# C API<!--c-arkgraphics2d-->

- Module<!--arkgraphics2d-module-->
    - [Vulkan](capi-vulkan.md)
    - [NativeWindow](capi-nativewindow.md)
    - [NativeColorSpaceManager](capi-nativecolorspacemanager.md)
    - [OH_NativeBuffer](capi-oh-nativebuffer.md)
    - [OH_NativeImage](capi-oh-nativeimage.md)
    - [NativeVsync](capi-nativevsync.md)
    - [NativeFence](capi-nativefence.md)
    - [NativeDisplaySoloist](capi-nativedisplaysoloist.md)
    - [Drawing](capi-drawing.md)
    - [effectKit](capi-effectkit.md)
- Header file<!--arkgraphics2d-headerfile-->
    - [vulkan_ohos.h](capi-vulkan-ohos-h.md)
    - [graphic_error_code.h](capi-graphic-error-code-h.md)
    - [buffer_handle.h](capi-buffer-handle-h.md)
    - [external_window.h](capi-external-window-h.md)
    - [native_color_space_manager.h](capi-native-color-space-manager-h.md)
    - [buffer_common.h](capi-buffer-common-h.md)
    - [native_buffer.h](capi-native-buffer-h.md)
    - [native_image.h](capi-native-image-h.md)
    - [native_vsync.h](capi-native-vsync-h.md)
    - [native_fence.h](capi-native-fence-h.md)
    - [native_display_soloist.h](capi-native-display-soloist-h.md)
    - [drawing_lattice.h](capi-drawing-lattice-h.md)
    - [drawing_point.h](capi-drawing-point-h.md)
    - [drawing_round_rect.h](capi-drawing-round-rect-h.md)
    - [drawing_shadow_layer.h](capi-drawing-shadow-layer-h.md)
    - [drawing_text_line.h](capi-drawing-text-line-h.md)
    - [drawing_color_space.h](capi-drawing-color-space-h.md)
    - [drawing_path_iterator.h](capi-drawing-path-iterator-h.md)
    - [drawing_text_run.h](capi-drawing-text-run-h.md)
    - [drawing_text_global.h](capi-drawing-text-global-h.md)
    - [drawing_pen.h](capi-drawing-pen-h.md)
    - [drawing_bitmap.h](capi-drawing-bitmap-h.md)
    - [drawing_matrix.h](capi-drawing-matrix-h.md)
    - [drawing_memory_stream.h](capi-drawing-memory-stream-h.md)
    - [drawing_canvas.h](capi-drawing-canvas-h.md)
    - [drawing_error_code.h](capi-drawing-error-code-h.md)
    - [drawing_gpu_context.h](capi-drawing-gpu-context-h.md)
    - [drawing_filter.h](capi-drawing-filter-h.md)
    - [drawing_text_blob.h](capi-drawing-text-blob-h.md)
    - [drawing_surface.h](capi-drawing-surface-h.md)
    - [drawing_brush.h](capi-drawing-brush-h.md)
    - [drawing_mask_filter.h](capi-drawing-mask-filter-h.md)
    - [drawing_text_declaration.h](capi-drawing-text-declaration-h.md)
    - [drawing_text_font_descriptor.h](capi-drawing-text-font-descriptor-h.md)
    - [drawing_record_cmd.h](capi-drawing-record-cmd-h.md)
    - [drawing_font_collection.h](capi-drawing-font-collection-h.md)
    - [drawing_sampling_options.h](capi-drawing-sampling-options-h.md)
    - [drawing_text_lineTypography.h](capi-drawing-text-linetypography-h.md)
    - [drawing_pixel_map.h](capi-drawing-pixel-map-h.md)
    - [drawing_typeface.h](capi-drawing-typeface-h.md)
    - [drawing_color.h](capi-drawing-color-h.md)
    - [drawing_shader_effect.h](capi-drawing-shader-effect-h.md)
    - [drawing_font.h](capi-drawing-font-h.md)
    - [drawing_rect.h](capi-drawing-rect-h.md)
    - [drawing_types.h](capi-drawing-types-h.md)
    - [drawing_font_mgr.h](capi-drawing-font-mgr-h.md)
    - [drawing_text_typography.h](capi-drawing-text-typography-h.md)
    - [drawing_path.h](capi-drawing-path-h.md)
    - [drawing_color_filter.h](capi-drawing-color-filter-h.md)
    - [drawing_register_font.h](capi-drawing-register-font-h.md)
    - [drawing_image.h](capi-drawing-image-h.md)
    - [drawing_image_filter.h](capi-drawing-image-filter-h.md)
    - [drawing_path_effect.h](capi-drawing-path-effect-h.md)
    - [drawing_region.h](capi-drawing-region-h.md)
    - [effect_types.h](capi-effect-types-h.md)
    - [effect_filter.h](capi-effect-filter-h.md)
- Struct<!--arkgraphics2d-struct-->
    - [VkSurfaceCreateInfoOHOS](capi-vulkan-vksurfacecreateinfoohos.md)
    - [VkNativeBufferOHOS](capi-vulkan-vknativebufferohos.md)
    - [VkSwapchainImageCreateInfoOHOS](capi-vulkan-vkswapchainimagecreateinfoohos.md)
    - [VkPhysicalDevicePresentationPropertiesOHOS](capi-vulkan-vkphysicaldevicepresentationpropertiesohos.md)
    - [VkNativeBufferUsageOHOS](capi-vulkan-vknativebufferusageohos.md)
    - [VkNativeBufferPropertiesOHOS](capi-vulkan-vknativebufferpropertiesohos.md)
    - [VkNativeBufferFormatPropertiesOHOS](capi-vulkan-vknativebufferformatpropertiesohos.md)
    - [VkImportNativeBufferInfoOHOS](capi-vulkan-vkimportnativebufferinfoohos.md)
    - [VkMemoryGetNativeBufferInfoOHOS](capi-vulkan-vkmemorygetnativebufferinfoohos.md)
    - [VkExternalFormatOHOS](capi-vulkan-vkexternalformatohos.md)
    - [NativeWindow](capi-vulkan-nativewindow.md)
    - [OH_NativeBuffer](capi-vulkan-oh-nativebuffer.md)
    - [BufferHandle](capi-nativewindow-bufferhandle.md)
    - [Region](capi-nativewindow-region.md)
    - [OHHDRMetaData](capi-nativewindow-ohhdrmetadata.md)
    - [OHExtDataHandle](capi-nativewindow-ohextdatahandle.md)
    - [OH_NativeBuffer](capi-nativewindow-oh-nativebuffer.md)
    - [OHIPCParcel](capi-nativewindow-ohipcparcel.md)
    - [NativeWindow](capi-nativewindow-nativewindow.md)
    - [NativeWindowBuffer](capi-nativewindow-nativewindowbuffer.md)
    - [ColorSpacePrimaries](capi-nativecolorspacemanager-colorspaceprimaries.md)
    - [WhitePointArray](capi-nativecolorspacemanager-whitepointarray.md)
    - [OH_NativeColorSpaceManager](capi-nativecolorspacemanager-oh-nativecolorspacemanager.md)
    - [OH_NativeBuffer_ColorXY](capi-oh-nativebuffer-oh-nativebuffer-colorxy.md)
    - [OH_NativeBuffer_Smpte2086](capi-oh-nativebuffer-oh-nativebuffer-smpte2086.md)
    - [OH_NativeBuffer_Cta861](capi-oh-nativebuffer-oh-nativebuffer-cta861.md)
    - [OH_NativeBuffer_StaticMetadata](capi-oh-nativebuffer-oh-nativebuffer-staticmetadata.md)
    - [OH_NativeBuffer_Config](capi-oh-nativebuffer-oh-nativebuffer-config.md)
    - [OH_NativeBuffer_Plane](capi-oh-nativebuffer-oh-nativebuffer-plane.md)
    - [OH_NativeBuffer_Planes](capi-oh-nativebuffer-oh-nativebuffer-planes.md)
    - [OHIPCParcel](capi-oh-nativebuffer-ohipcparcel.md)
    - [OH_OnFrameAvailableListener](capi-oh-nativeimage-oh-onframeavailablelistener.md)
    - [NativeWindow](capi-oh-nativeimage-nativewindow.md)
    - [NativeWindowBuffer](capi-oh-nativeimage-nativewindowbuffer.md)
    - [OH_NativeVSync_ExpectedRateRange](capi-nativevsync-oh-nativevsync-expectedraterange.md)
    - [DisplaySoloist_ExpectedRateRange](capi-nativedisplaysoloist-displaysoloist-expectedraterange.md)
    - [OH_DisplaySoloist](capi-nativedisplaysoloist-oh-displaysoloist.md)
    - [OH_NativeColorSpaceManager](capi-drawing-oh-nativecolorspacemanager.md)
    - [OH_Drawing_BitmapFormat](capi-drawing-oh-drawing-bitmapformat.md)
    - [OH_Drawing_GpuContextOptions](capi-drawing-oh-drawing-gpucontextoptions.md)
    - [OH_Drawing_RunBuffer](capi-drawing-oh-drawing-runbuffer.md)
    - [OH_Drawing_FontCollection](capi-drawing-oh-drawing-fontcollection.md)
    - [OH_Drawing_Typography](capi-drawing-oh-drawing-typography.md)
    - [OH_Drawing_LineTypography](capi-drawing-oh-drawing-linetypography.md)
    - [OH_Drawing_TextStyle](capi-drawing-oh-drawing-textstyle.md)
    - [OH_Drawing_TypographyStyle](capi-drawing-oh-drawing-typographystyle.md)
    - [OH_Drawing_TypographyCreate](capi-drawing-oh-drawing-typographycreate.md)
    - [OH_Drawing_TextBox](capi-drawing-oh-drawing-textbox.md)
    - [OH_Drawing_PositionAndAffinity](capi-drawing-oh-drawing-positionandaffinity.md)
    - [OH_Drawing_Range](capi-drawing-oh-drawing-range.md)
    - [OH_Drawing_FontParser](capi-drawing-oh-drawing-fontparser.md)
    - [OH_Drawing_TextShadow](capi-drawing-oh-drawing-textshadow.md)
    - [OH_Drawing_TextTab](capi-drawing-oh-drawing-texttab.md)
    - [OH_Drawing_TextLine](capi-drawing-oh-drawing-textline.md)
    - [OH_Drawing_Run](capi-drawing-oh-drawing-run.md)
    - [OH_Drawing_FontFullDescriptor](capi-drawing-oh-drawing-fontfulldescriptor.md)
    - [OH_Drawing_FontVariationAxis](capi-drawing-oh-drawing-fontvariationaxis.md)
    - [OH_Drawing_FontVariationInstance](capi-drawing-oh-drawing-fontvariationinstance.md)
    - [OH_Drawing_FontVariationInstanceCoordinate](capi-drawing-oh-drawing-fontvariationinstancecoordinate.md)
    - [NativePixelMap_](capi-drawing-nativepixelmap-.md)
    - [OH_PixelmapNative](capi-drawing-oh-pixelmapnative.md)
    - [OH_Drawing_TypefaceFallbackInfo](capi-drawing-oh-drawing-typefacefallbackinfo.md)
    - [OH_Drawing_Font_Metrics](capi-drawing-oh-drawing-font-metrics.md)
    - [OH_Drawing_Point2D](capi-drawing-oh-drawing-point2d.md)
    - [OH_Drawing_Point3D](capi-drawing-oh-drawing-point3d.md)
    - [OH_Drawing_RectStyle_Info](capi-drawing-oh-drawing-rectstyle-info.md)
    - [OH_Drawing_Image_Info](capi-drawing-oh-drawing-image-info.md)
    - [OH_Drawing_String](capi-drawing-oh-drawing-string.md)
    - [OH_Drawing_Canvas](capi-drawing-oh-drawing-canvas.md)
    - [OH_Drawing_Pen](capi-drawing-oh-drawing-pen.md)
    - [OH_Drawing_Region](capi-drawing-oh-drawing-region.md)
    - [OH_Drawing_Brush](capi-drawing-oh-drawing-brush.md)
    - [OH_Drawing_Path](capi-drawing-oh-drawing-path.md)
    - [OH_Drawing_PathIterator](capi-drawing-oh-drawing-pathiterator.md)
    - [OH_Drawing_Lattice](capi-drawing-oh-drawing-lattice.md)
    - [OH_Drawing_Bitmap](capi-drawing-oh-drawing-bitmap.md)
    - [OH_Drawing_Point](capi-drawing-oh-drawing-point.md)
    - [OH_Drawing_PixelMap](capi-drawing-oh-drawing-pixelmap.md)
    - [OH_Drawing_ColorSpace](capi-drawing-oh-drawing-colorspace.md)
    - [OH_Drawing_PathEffect](capi-drawing-oh-drawing-patheffect.md)
    - [OH_Drawing_Rect](capi-drawing-oh-drawing-rect.md)
    - [OH_Drawing_RoundRect](capi-drawing-oh-drawing-roundrect.md)
    - [OH_Drawing_Matrix](capi-drawing-oh-drawing-matrix.md)
    - [OH_Drawing_ShaderEffect](capi-drawing-oh-drawing-shadereffect.md)
    - [OH_Drawing_ShadowLayer](capi-drawing-oh-drawing-shadowlayer.md)
    - [OH_Drawing_Filter](capi-drawing-oh-drawing-filter.md)
    - [OH_Drawing_MaskFilter](capi-drawing-oh-drawing-maskfilter.md)
    - [OH_Drawing_ColorFilter](capi-drawing-oh-drawing-colorfilter.md)
    - [OH_Drawing_Font](capi-drawing-oh-drawing-font.md)
    - [OH_Drawing_FontFeatures](capi-drawing-oh-drawing-fontfeatures.md)
    - [OH_Drawing_MemoryStream](capi-drawing-oh-drawing-memorystream.md)
    - [OH_Drawing_FontArguments](capi-drawing-oh-drawing-fontarguments.md)
    - [OH_Drawing_Typeface](capi-drawing-oh-drawing-typeface.md)
    - [OH_Drawing_TextBlob](capi-drawing-oh-drawing-textblob.md)
    - [OH_Drawing_Image](capi-drawing-oh-drawing-image.md)
    - [OH_Drawing_ImageFilter](capi-drawing-oh-drawing-imagefilter.md)
    - [OH_Drawing_SamplingOptions](capi-drawing-oh-drawing-samplingoptions.md)
    - [OH_Drawing_TextBlobBuilder](capi-drawing-oh-drawing-textblobbuilder.md)
    - [OH_Drawing_GpuContext](capi-drawing-oh-drawing-gpucontext.md)
    - [OH_Drawing_Surface](capi-drawing-oh-drawing-surface.md)
    - [OH_Drawing_FontMgr](capi-drawing-oh-drawing-fontmgr.md)
    - [OH_Drawing_FontStyleSet](capi-drawing-oh-drawing-fontstyleset.md)
    - [OH_Drawing_RecordCmdUtils](capi-drawing-oh-drawing-recordcmdutils.md)
    - [OH_Drawing_RecordCmd](capi-drawing-oh-drawing-recordcmd.md)
    - [OH_Drawing_Array](capi-drawing-oh-drawing-array.md)
    - [OH_Drawing_PlaceholderSpan](capi-drawing-oh-drawing-placeholderspan.md)
    - [OH_Drawing_FontFallbackInfo](capi-drawing-oh-drawing-fontfallbackinfo.md)
    - [OH_Drawing_FontFallbackGroup](capi-drawing-oh-drawing-fontfallbackgroup.md)
    - [OH_Drawing_FontAdjustInfo](capi-drawing-oh-drawing-fontadjustinfo.md)
    - [OH_Drawing_FontAliasInfo](capi-drawing-oh-drawing-fontaliasinfo.md)
    - [OH_Drawing_FontGenericInfo](capi-drawing-oh-drawing-fontgenericinfo.md)
    - [OH_Drawing_FontConfigInfo](capi-drawing-oh-drawing-fontconfiginfo.md)
    - [OH_Drawing_FontDescriptor](capi-drawing-oh-drawing-fontdescriptor.md)
    - [OH_Drawing_LineMetrics](capi-drawing-oh-drawing-linemetrics.md)
    - [OH_Drawing_FontStyleStruct](capi-drawing-oh-drawing-fontstylestruct.md)
    - [OH_Drawing_FontFeature](capi-drawing-oh-drawing-fontfeature.md)
    - [OH_Drawing_StrutStyle](capi-drawing-oh-drawing-strutstyle.md)
    - [OH_Drawing_RectSize](capi-drawing-oh-drawing-rectsize.md)
    - [OH_Filter_ColorMatrix](capi-effectkit-oh-filter-colormatrix.md)
    - [OH_Filter](capi-effectkit-oh-filter.md)
    - [OH_PixelmapNative](capi-effectkit-oh-pixelmapnative.md)
