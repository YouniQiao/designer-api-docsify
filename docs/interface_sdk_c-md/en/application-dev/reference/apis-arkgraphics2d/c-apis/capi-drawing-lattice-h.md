...
| [const OH_Drawing_Rect](capi-drawing-oh-drawing-rect.md)* bounds | The original bounding rectangle to be drawn, which defaults to the size of the original imagerectangle. The value must be an integer and is rounded down. |
...
| [OH_Drawing_Lattice](capi-drawing-oh-drawing-lattice.md)** lattice | Double pointer to an [OH_Drawing_Lattice](capi-drawing-oh-drawing-lattice.md) object, which serves as an output parameter returnedto the caller. |
