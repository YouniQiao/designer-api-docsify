...
| Parameter | Description |
| -- | -- |
| [struct OH_Huks_ParamSet](capi-hukstypeapi-oh-huks-paramset.md) *paramSet | Pointer to the parameter set to which parameters are to be added. This parameter needs to beinitialized by using [OH_Huks_InitParamSet](capi-native-huks-param-h.md#oh_huks_initparamset). |
...
| bool isCopy | If the value is **true**, the address of the [OH_Huks_Blob](capi-hukstypeapi-oh-huks-blob.md) data is refreshed and copied to theparameter set. If the value is **false**, only the address of the [OH_Huks_Blob](capi-hukstypeapi-oh-huks-blob.md) data is refreshed. |
