...
| -- | -- |
| OH_AVPlayer \*player | Pointer to an OH_AVPlayer instance. |
| int32_t errorCode | Error code.**AV_ERR_NO_MEMORY**: No memory. The value is **1**.{@link AV_ERR_OPERATE_NOT_PERMIT}: The operation is not allowed. The value is **2**.{@link AV_ERR_INVALID_VA}: Invalid value. The value is **3**.**AV_ERR_IO**: I/O error. The value is **4**.**AV_ERR_TIMEOUT**: Timeout. The value is **5**.**AV_ERR_UNKNOWN**: Unknown error. The value is **6**.{@link AV_ERR_SERVICE_DIED}: The service is dead. The value is **7**.**AV_ERR_INVALID_STATE**: The operation is not supported in the current state. The value is **8**.{@link AV_ERR_UNSUPPORT}: The function is not supported. The value is **9**.**AV_ERR_EXTEND_START**: Initial value for extended error codes. The value is **100**. |
...
| -- | -- |
| OH_AVPlayer \*player | Pointer to an OH_AVPlayer instance. |
| int32_t errorCode | Error code.**AV_ERR_NO_MEMORY**: No memory. The value is **1**.{@link AV_ERR_OPERATE_NOT_PERMIT}: The operation is not allowed. The value is **2**.{@link AV_ERR_INVALID_VA}: Invalid value. The value is **3**.**AV_ERR_IO**: I/O error. For API versions 12 and 13, the value is **4**. Starting from API version 14, itcorresponds to more specific error codes ranging from 5411001 to 5411011.**AV_ERR_TIMEOUT**: Timeout. The value is **5**.**AV_ERR_UNKNOWN**: Unknown error. The value is **6**.{@link AV_ERR_SERVICE_DIED}: The service is dead. The value is **7**.**AV_ERR_INVALID_STATE**: The operation is not supported in the current state. The value is **8**.{@link AV_ERR_UNSUPPORT}: The function is not supported. The value is **9**.**AV_ERR_EXTEND_START**: Initial value for extended error codes. The value is **100**. |
