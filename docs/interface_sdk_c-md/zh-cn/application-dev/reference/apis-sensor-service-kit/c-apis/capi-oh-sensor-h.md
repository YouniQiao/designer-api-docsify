...
| 参数项 | 描述 |
| -- | -- |
| [Sensor_Info](capi-sensor-sensor-info.md) **infos | 输出参数，双指针指向设备上所有传感器的信息。调用者不需要预先分配内存，函数内部会分配内存，调用者需要在使用完毕后调用相应的释放函数释放内存。调用后，*infos指向传感器信息数组，请参考[Sensor_Info](capi-sensor-sensor-info.md)。该参数不应为NULL。 |
...
| 类型 | 说明 |
| -- | -- |
| [Sensor_Result](capi-oh-sensor-type-h.md#sensor_result) | 返回[Sensor_Result](capi-oh-sensor-type-h.md#sensor_result)枚举值。主要枚举值如下：<br>     <br>SENSOR_SUCCESS：操作成功，表示成功获取传感器信息。<br>     <br>[SENSOR_PARAMETER_ERROR](capi-oh-sensor-type-h.md#sensor_result)：参数检查失败。例如，传入的infos或count参数为NULL指针。<br>     <br>[SENSOR_SERVICE_EXCEPTION](capi-oh-sensor-type-h.md#sensor_result)：传感器服务异常。例如，传感器服务未启动或发生内部错误。<br>     <br>其他可能的错误代码请参考[Sensor_Result](capi-oh-sensor-type-h.md#sensor_result)。 |
...
**需要权限：** ohos.permission.ACCELEROMETER or ohos.permission.GYROSCOPE or ohos.permission.ACTIVITY_MOTION or
...
| 类型 | 说明 |
| -- | -- |
| [Sensor_Result](capi-oh-sensor-type-h.md#sensor_result) | 返回[Sensor_Result](capi-oh-sensor-type-h.md#sensor_result)枚举值。主要枚举值如下：<br>     <br>SENSOR_SUCCESS：操作成功，表示成功订阅传感器数据。<br>     <br>[SENSOR_PERMISSION_DENIED](capi-oh-sensor-type-h.md#sensor_result)：权限验证失败。缺少订阅传感器所需权限时返回此错误，请参考函数描述部分了解不同传感器所需的权限，并按照要求申请相应权限后再试。<br>     <br>[SENSOR_PARAMETER_ERROR](capi-oh-sensor-type-h.md#sensor_result)：参数检查失败。例如，传入的id、attribute或subscriber参数为NULL指针。<br>     <br>[SENSOR_SERVICE_EXCEPTION](capi-oh-sensor-type-h.md#sensor_result)：传感器服务异常。例如，传感器服务未启动或发生内部错误。<br>     <br>其他可能的错误代码请参考[Sensor_Result](capi-oh-sensor-type-h.md#sensor_result)。 |
...
| 类型 | 说明 |
| -- | -- |
| [Sensor_Result](capi-oh-sensor-type-h.md#sensor_result) | 返回[Sensor_Result](capi-oh-sensor-type-h.md#sensor_result)枚举值。主要枚举值如下：<br>     <br>SENSOR_SUCCESS：操作成功，表示成功取消订阅传感器数据。<br>     <br>[SENSOR_PERMISSION_DENIED](capi-oh-sensor-type-h.md#sensor_result)：权限验证失败。缺少取消订阅传感器所需权限时返回此错误，请参考函数描述部分了解不同传感器所需的权限，并按照要求申请相应权限后再试。<br>     <br>[SENSOR_PARAMETER_ERROR](capi-oh-sensor-type-h.md#sensor_result)：参数检查失败。例如，传入的id或subscriber参数为NULL指针。<br>     <br>[SENSOR_SERVICE_EXCEPTION](capi-oh-sensor-type-h.md#sensor_result)：传感器服务异常。例如，传感器服务未启动或发生内部错误。<br>     <br>其他可能的错误代码请参考[Sensor_Result](capi-oh-sensor-type-h.md#sensor_result)。 |
