...
文件中定义了与字体相关的功能函数。 <br>本模块为单线程模型策略，需要调用方自行管理线程安全和上下文状态的切换。
...
| 名称 | typedef关键字 | 描述 |
| -- | -- | -- |
| [OH_Drawing_TypefaceFallbackInfo](capi-drawing-oh-drawing-typefacefallbackinfo.md) | OH_Drawing_TypefaceFallbackInfo | 定义字体回退信息结构体，包含一组使用相同回退字体的字形数组。 |
...
| [OH_Drawing_ErrorCode OH_Drawing_FontMeasureTextWithFallback(const OH_Drawing_Font *font, const void *text, uint32_t byteLength, OH_Drawing_TextEncoding encoding, OH_Drawing_Rect *bounds, float *textWidth)](#oh_drawing_fontmeasuretextwithfallback) | 获取文本的宽度和边界框，支持字体回退。 若当前字型的字体不支持某些字符时，会自动从系统中查找回退字体。如果未找到任何回退字体，则仍使用当前字型的字体。 |
...
| [OH_Drawing_ErrorCode OH_Drawing_FontMeasureTextWithBrushOrPenWithFallback(const OH_Drawing_Font *font, const void *text, uint32_t byteLength, OH_Drawing_TextEncoding encoding, const OH_Drawing_Brush *brush, const OH_Drawing_Pen *pen, OH_Drawing_Rect *bounds, float *textWidth)](#oh_drawing_fontmeasuretextwithbrushorpenwithfallback) | 使用画刷或画笔获取文本的宽度和边界框，支持字体回退。 若当前字型的字体不支持某些字符时，会自动从系统中查找回退字体。如果未找到任何回退字体，则仍使用当前字型的字体。 |
...
| [OH_Drawing_ErrorCode OH_Drawing_FontTypefaceFallbackInfoDestroy(OH_Drawing_TypefaceFallbackInfo *infos, uint32_t count)](#oh_drawing_fonttypefacefallbackinfodestroy) | 释放OH_Drawing_TypefaceFallbackInfo对象数组并回收该数组占用的内存。 本函数会销毁数组中的字体和字形IDs，并释放数组本身。count必须与创建数组时返回的数量完全一致，传入其他值将导致未定义行为。 |
...
| 枚举项 | 描述 |
| -- | -- |
| FONT_HINTING_NONE | 不修改字型轮廓。<br>**起始版本：** 12 |
| FONT_HINTING_SLIGHT | 最小限度修改字型轮廓以改善对比度。<br>**起始版本：** 12 |
| FONT_HINTING_NORMAL | 修改字型轮廓以提高对比度。<br>**起始版本：** 12 |
| FONT_HINTING_FULL | 修改字型轮廓以获得最大对比度。<br>**起始版本：** 12 |
...
| 枚举项 | 描述 |
| -- | -- |
| FONT_EDGING_ALIAS | 无抗锯齿处理。<br>**起始版本：** 12 |
| FONT_EDGING_ANTI_ALIAS | 使用抗锯齿来平滑字型边缘。<br>**起始版本：** 12 |
| FONT_EDGING_SUBPIXEL_ANTI_ALIAS | 使用次像素级别的抗锯齿来平滑字型边缘，可以获得更加平滑的字型渲染效果。<br>**起始版本：** 12 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_Font* | 函数会返回一个指针，指针指向创建的字型对象OH_Drawing_Font。 |
...
| 参数项 | 描述 |
| -- | -- |
| OH_Drawing_Font* font | 指向字型对象OH_Drawing_Font的指针。 |
...
| 参数项 | 描述 |
| -- | -- |
| const OH_Drawing_Font* font | 指向字型对象OH_Drawing_Font的指针。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_Typeface* | 函数返回一个指针，指向字体对象OH_Drawing_Typeface。 |
...
| OH_Drawing_TextEncoding encoding | 文本编码类型OH_Drawing_TextEncoding。 |
...
将文本转换为字形索引，支持字体回退。若当前字型的字体不支持某些字符时，会自动从系统中查找回退字体。 文本会被切分为多个使用相同字体的分段。如果未找到任何回退字体，则仍使用当前字型的字体。
...
| const void *text | [in] 文本存储首地址。 |
| uint32_t byteLength | [in] 文本长度，单位为字节。 |
| OH_Drawing_TextEncoding encoding | [in] 文本编码类型OH_Drawing_TextEncoding。 |
| [OH_Drawing_TypefaceFallbackInfo](capi-drawing-oh-drawing-typefacefallbackinfo.md) **typefaceFallbackInfo | [out] 指向OH_Drawing_TypefaceFallbackInfo数组首元素的指针。 作为出参使用。当不再需要时，使用[OH_Drawing_FontTypefaceFallbackInfoDestroy](capi-drawing-font-h.md#oh_drawing_fonttypefacefallbackinfodestroy)释放该数组。 |
| uint32_t *infosCount | [out] 存储OH_Drawing_TypefaceFallbackInfo对象数组的大小。作为出参使用。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INCORRECT_PARAMETER，表示参数font、text、typefaceFallbackInfo、infosCount任意一个为空，或者byteLength为0。      <br>返回OH_DRAWING_ERROR_PARAMETER_OUT_OF_RANGE，表示encoding不在枚举范围内。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font、str、textWidth任意一个为NULL或者str的长度为0。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行结果。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font、str、  fontFeatures或者textWidth任意一个为空指针或者str的长度为0。 |
...
| OH_Drawing_Rect* bounds | 用于承载获取的边界框，可以为NULL，为NULL时不返回边界框信息，仅返回文本宽度。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font，text，textWidth至少有一个为空，或者byteLength为0。 |
...
获取文本的宽度和边界框，支持字体回退。 若当前字型的字体不支持某些字符时，会自动从系统中查找回退字体。如果未找到任何回退字体，则仍使用当前字型的字体。
...
| uint32_t byteLength | [in] 文本长度，单位为字节。 |
| OH_Drawing_TextEncoding encoding | [in] 文本编码类型。 |
| OH_Drawing_Rect *bounds | [out] 用于承载获取的边界框，可以为NULL，为NULL时不返回边界框信息，仅返回文本宽度。作为出参使用。 |
| float *textWidth | [out] 用于存储得到的文本宽度，单位为物理像素px。作为出参使用。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INCORRECT_PARAMETER，表示参数font、text、textWidth至少有一个为空，或者byteLength为0。      <br>返回OH_DRAWING_ERROR_PARAMETER_OUT_OF_RANGE，表示encoding不在枚举范围内。 |
...
| OH_Drawing_TextEncoding encoding | 文本编码类型。 |
| const OH_Drawing_Brush* brush | 指向画刷对象OH_Drawing_Brush的指针。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font、text、textWidth至少有一个为空，或者byteLength为0，  或者brush和pen同时不为空。 |
...
使用画刷或画笔获取文本的宽度和边界框，支持字体回退。 若当前字型的字体不支持某些字符时，会自动从系统中查找回退字体。如果未找到任何回退字体，则仍使用当前字型的字体。
...
| uint32_t byteLength | [in] 文本长度，单位为字节。 |
| OH_Drawing_TextEncoding encoding | [in] 文本编码类型。 |
| const OH_Drawing_Brush *brush | [in] 指向画刷对象OH_Drawing_Brush的指针。 |
| const OH_Drawing_Pen *pen | [in] 指向画笔对象OH_Drawing_Pen的指针。 |
| OH_Drawing_Rect *bounds | [out] 用于承载获取的边界框，可以为NULL，为NULL时不返回边界框信息，仅返回文本宽度。作为出参使用。 |
| float *textWidth | [out] 用于存储得到的文本宽度，单位为物理像素px。作为出参使用。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INCORRECT_PARAMETER，表示参数font、text、textWidth至少有一个为空，或者byteLength为0，或者brush和pen同时存在。      <br>返回OH_DRAWING_ERROR_PARAMETER_OUT_OF_RANGE，表示encoding不在枚举范围内。 |
...
| const uint16_t* glyphs | 字形索引存储首地址。 |
| int count | 字形索引的数量，大小与glyphs数组大小保持一致。 |
| const OH_Drawing_Brush* brush | 指向画刷对象OH_Drawing_Brush的指针，用于指定画刷样式以获取字形的宽度和边界框。 |
| const OH_Drawing_Pen* pen | 指向画笔对象OH_Drawing_Pen的指针，用于指定画笔样式以获取字形的宽度和边界框。 |
| float* widths | 字形宽度存储首地址，用于存储得到的字形宽度，作为返回值返回给调用者。单位为物理像素px。 |
| OH_Drawing_Array* bounds | 字形边界框存储首地址，用于存储得到的字形边界框，作为出参使用。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font、glyphs至少有一个为空，或者count不大于0，  或者brush和pen同时不为空，或者widths和bounds同时为空。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font、glyphs、points至少有一个为空，或者count不大于0。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font、spacing至少有一个为空。 |
...
| -- | -- |
| OH_Drawing_Font* font | 指向字型对象OH_Drawing_Font的指针。 |
| [OH_Drawing_FontHinting](capi-drawing-font-h.md#oh_drawing_fonthinting) fontHinting | 字型轮廓效果枚举类型OH_Drawing_FontHinting。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_Drawing_FontHinting](capi-drawing-font-h.md#oh_drawing_fonthinting) | 返回字型轮廓效果枚举类型OH_Drawing_FontHinting。 |
...
| -- | -- |
| OH_Drawing_Font* font | 指向字型对象OH_Drawing_Font的指针。 |
| [OH_Drawing_FontEdging](capi-drawing-font-h.md#oh_drawing_fontedging) fontEdging | 字型边缘效果枚举类型OH_Drawing_FontEdging。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_Drawing_FontEdging](capi-drawing-font-h.md#oh_drawing_fontedging) | 返回字型边缘效果。 |
...
| -- | -- |
| OH_Drawing_Font* font | 指向字型对象OH_Drawing_Font的指针。 |
| [OH_Drawing_Font_Metrics](capi-drawing-oh-drawing-font-metrics.md)* fontMetrics | 指向字体度量信息对象OH_Drawing_Font_Metrics的指针。 |
...
| OH_Drawing_Array* bounds | 矩形边界数组，用于存储得到的字形矩形边界，作为出参使用。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font、glyphs或bounds为空，或者count为零。 |
...
| OH_Drawing_Path* path | 指向路径对象OH_Drawing_Path的指针，用于存储得到的字形路径。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font或者path为空， 或者指定glyph不存在。 |
...
| OH_Drawing_TextEncoding encoding | 指示文本编码格式，支持 UTF-8、UTF-16、UTF-32，以及字形索引，具体类型格式可见OH_Drawing_TextEncoding。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font、text或path任意一个为空指针。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行结果。      <br>如果操作成功，则返回OH_DRAWING_SUCCESS。      <br>如果font、text或path中的任何一个为空指针，或者byteLength为0，则返回OH_DRAWING_ERROR_INCORRECT_PARAMETER。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_FontFeatures* | 函数会返回一个指针，指向创建的字体特征容器对象OH_Drawing_FontFeatures。      <br>如果返回的对象指针为空，则表示字体特征容器对象创建失败。失败的原因可能为：没有可用的内存。 |
...
| 参数项 | 描述 |
| -- | -- |
| OH_Drawing_FontFeatures* fontFeatures | 指向字体特征容器对象OH_Drawing_FontFeatures的指针。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行结果。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数fontFeatures或name为空指针。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行结果。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数fontFeatures为空指针。 |
...
释放OH_Drawing_TypefaceFallbackInfo对象数组并回收该数组占用的内存。 本函数会销毁数组中的字体和字形IDs，并释放数组本身。count必须与创建数组时返回的数量完全一致，传入其他值将导致未定义行为。
...
| 参数项 | 描述 |
| -- | -- |
| [OH_Drawing_TypefaceFallbackInfo](capi-drawing-oh-drawing-typefacefallbackinfo.md) *infos | [in] 指向OH_Drawing_TypefaceFallbackInfo对象数组的指针。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 返回[OH_DRAWING_SUCCESS](capi-drawing-error-code-h.md#oh_drawing_errorcode)，表示执行成功。      <br>返回[OH_DRAWING_ERROR_INCORRECT_PARAMETER](capi-drawing-error-code-h.md#oh_drawing_errorcode)，表示infos为空或者count为0。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font为空。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_ErrorCode | 函数返回执行错误码。      <br>返回OH_DRAWING_SUCCESS，表示执行成功。      <br>返回OH_DRAWING_ERROR_INVALID_PARAMETER，表示参数font或者followed其中一个为空。 |
