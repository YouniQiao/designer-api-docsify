...
| 名称 | 描述 |
| -- | -- |
| [OH_Drawing_LineTypography* OH_Drawing_CreateLineTypography(OH_Drawing_TypographyCreate* handler)](#oh_drawing_createlinetypography) | 创建一个排版行对象{@link OH_Drawing_LineTypography}的指针，排版行对象是保存文本内容以及样式的载体，可以用于计算单行排版信息。 |
| [void OH_Drawing_DestroyLineTypography(OH_Drawing_LineTypography* lineTypography)](#oh_drawing_destroylinetypography) | 释放排版行对象{@link OH_Drawing_LineTypography}占用的内存。 |
...
| [OH_Drawing_TextLine* OH_Drawing_LineTypographyCreateLine(OH_Drawing_LineTypography* lineTypography, size_t startIndex, size_t count)](#oh_drawing_linetypographycreateline) | 根据指定区间文本内容创建一个指向文本行对象{@link OH_Drawing_TextLine}的指针。 |
...
创建一个排版行对象{@link OH_Drawing_LineTypography}的指针，排版行对象是保存文本内容以及样式的载体，可以用于计算单行排版信息。
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_LineTypography* | 返回一个指向排版行对象{@link OH_Drawing_LineTypography}的指针。 |
...
释放排版行对象{@link OH_Drawing_LineTypography}占用的内存。
...
根据指定区间文本内容创建一个指向文本行对象{@link OH_Drawing_TextLine}的指针。
...
| 类型 | 说明 |
| -- | -- |
| OH_Drawing_TextLine* | 返回一个指向文本行对象{@link OH_Drawing_TextLine}的指针。 |
