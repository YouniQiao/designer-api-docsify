...
判断矩阵是否是单位矩阵。单位矩阵为：`[1 0 0; 0 1 0; 0 0 1]`。<br>如需判断两个矩阵是否相等，请使用[OH_Drawing_MatrixIsEqual](capi-drawing-matrix-h.md#oh_drawing_matrixisequal)。<br>本接口会产生错误码，可以通过{@link OH_Drawing_ErrorCodeGet}查看错误码的取值。<br>matrix为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER。
