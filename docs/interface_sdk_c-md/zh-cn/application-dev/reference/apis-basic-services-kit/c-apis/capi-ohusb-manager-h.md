...
连接USB设备并打开用于通信的管道。返回的管道必须通过调用<br>[OH_UsbManager_ClosePipe](capi-ohusb-manager-h.md#oh_usbmanager_closepipe)关闭，以避免资源泄漏。<br>仅需要设备结构体中的busNum和devAddress字段；其他字段将被忽略。
