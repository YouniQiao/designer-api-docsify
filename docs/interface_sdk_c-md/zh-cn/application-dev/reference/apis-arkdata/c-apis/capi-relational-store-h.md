...
| bool isPersistent | 指示数据库是否需要持久化。 |
...
| OH_Rdb_Transaction **trans | 输出参数，表示执行成功时指向{@link OH_Rdb_Transaction}实例的指针。否则返回nullptr。<br>使用完成后，必须通过{@link OH_RdbTrans_Destroy}接口释放内存。 |
