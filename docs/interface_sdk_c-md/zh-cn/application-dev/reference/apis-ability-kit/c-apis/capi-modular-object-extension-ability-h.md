...
| 类型 | 说明 |
| -- | -- |
| AbilityRuntime_ErrorCode | 返回特定的错误码。<br>     <br>{@link ABILITY_RUNTIME_ERROR_CODE_NO_ERROR} 接口调用成功。<br>     <br>{@link ABILITY_RUNTIME_ERROR_CODE_PARAM_INVALID} 参数校验失败。<br>     <br>{@link ABILITY_RUNTIME_ERROR_CODE_INCORRECT_ABILITY_TYPE}<br>     ExtensionAbility实例不是ModularObjectExtensionAbility类型。 |
