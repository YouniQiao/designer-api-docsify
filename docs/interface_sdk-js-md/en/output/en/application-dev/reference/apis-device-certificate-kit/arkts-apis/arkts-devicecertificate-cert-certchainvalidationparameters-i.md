...
Whether to allow the application to download the missing intermediate CA certificate from the network. **true** means yes; **false** otherwise. The default value is **false**. \_\_\_HTML\_TAG\_DESC\_USD\_1\_\_\_The download address is obtained from the certificate AIA extension. Only HTTP is supported. To use the network for download, you need to request the **ohos.permission.INTERNET** permission. For details about the permission configuration, see \_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_.
