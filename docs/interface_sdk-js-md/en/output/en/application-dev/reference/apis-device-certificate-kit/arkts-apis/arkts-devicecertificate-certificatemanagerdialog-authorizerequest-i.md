...
Certificate usage. If the **certTypes** parameter contains the **CertificateType.CREDENTIAL\_UKEY** type, the **certPurpose** parameter takes effect , indicating that the certificate credentials of the USB key are filtered based on the specified certificate usage.
