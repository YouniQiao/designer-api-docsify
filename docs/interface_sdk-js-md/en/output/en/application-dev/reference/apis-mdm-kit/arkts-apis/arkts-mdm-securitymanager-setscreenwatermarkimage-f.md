...
| pixelMap | image.PixelMap | Yes | Image object. The image width must not exceed twice the screen width, and the image height must not exceed twice the screen height. The size of the image pixel data cannot exceed 128 MB.The size of the image pixel data is calculated as follows: Image width (pixels) × Image height (pixels) ×Number of bytes per pixel (typically 4). For example, the size of a 100 × 100 image is 100 × 100 × 4 = 40,000bytes. For a 1920 × 1080 screen, using an image of the same resolution results in a pixel data size of 1920 × 1080 × 4 = 8,294,400 bytes (approximately 7.9 MB). |
