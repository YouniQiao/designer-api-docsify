...
Device printing capability. If the device printing capability is disabled for a specific user, it remains disabled for that user even if the device printing capability ( [FeatureForDevice.PRINTER]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_) capability is enabled globally.
...
USB storage device write capability. Currently, it is supported only on enterprise PCs/2-in-1 devices. Disabling the USB storage device write capability for a specific user in any of the following scenarios will result in a policy conflict: 1. The device USB capability ([FeatureForDevice.USB]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_) has been disabled. 2. USB storage device access policy has been set to read-only or disabled via the setUsbStorageDeviceAccessPolicy API. 3. Storage USB devices have been disabled via the addDisallowedUsbDevices API.
