...
Removes an application from the permission usage exception list. After the application is removed, it cannot use the corresponding permission any more. > **NOTE** > > The permission must first be disabled via the > [setDisallowedPermission]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ API before an application can be removed > from the permission usage exception list. Otherwise, error code 9201044 is returned.
