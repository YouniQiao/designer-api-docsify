...
Allows the VoIP capture stream of the current application to run concurrently with other existing VoIP capture streams. When a later VoIP capture stream arrives, it can interrupt the current stream. This flag only takes effect when used in \_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_. When using this flag, the permission ohos.permission.VOIP\_CAPTURE\_CONCURRENCY must be verified.
