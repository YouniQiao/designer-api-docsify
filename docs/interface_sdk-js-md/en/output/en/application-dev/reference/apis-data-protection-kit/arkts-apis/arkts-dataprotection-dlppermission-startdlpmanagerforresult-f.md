...
Starts the DLP manager application on the current [UIAbility]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ page in borderless mode. This API uses a promise to return the result. This API starts the DLP manager application to configure file permissions and return the user operation result to the caller. > **NOTE** > > This API can be called only by domain accounts.
