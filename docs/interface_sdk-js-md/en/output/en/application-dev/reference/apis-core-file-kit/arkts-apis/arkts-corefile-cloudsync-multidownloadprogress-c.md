...
| Type | Description |
| --- | --- |
| Array&lt;FailedFileInfo&gt; | List of file URIs that fail to be downloaded and the corresponding error |
...
Size of the downloaded file, in bytes. The value range is [0, INT64\_MAX). If the progress is abnormal, the value **INT64\_MAX** is returned.
...
ID of a batch download task. The value ranges from 0 to INT64\_MAX. If the progress is abnormal, the value **-1** is returned.
...
Total size of the files to be downloaded, in bytes. The value range is [0, INT64\_MAX). If the progress is abnormal, the value **INT64\_MAX** is returned.
