...
Implements a window manager, which manages each basic window unit, that is, [Window]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ instance. Before calling any of the following APIs, you must use [onWindowStageCreate()]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ to create a WindowStage instance.
...
| [1300005](../errorcode-window.md#1300005-abnormal-windowstage) | This window stage is abnormal.\_\_\_HTML\_TAG\_USD\_0\_\_\_**Applicable version:** 9 and later |
...
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;WindowStageEventType&gt; | No | Callback used to return the window stage lifecycle state. |
...
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;WindowStageLifecycleEventType&gt; | No | Callback used to |
...
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;void&gt; | No | Callback function requires a boolean return value to determine whether to |
...
| --- | --- | --- | --- |
| eventType | 'windowStageLifecycleEvent' | Yes | Event type. |
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;WindowStageLifecycleEventType&gt; | Yes | Callback used to |
...
| --- | --- | --- | --- |
| eventType | 'windowStageClose' | Yes | The value is fixed at 'windowStageClose', indicating the window stage |
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;void&gt; | Yes | Callback function requires a boolean return value to determine whether to |
...
| [202](../../errorcode-universal.md#202-permission-verification-failed-for-calling-a-system-api) | Permission verification failed. A non-system application calls a system API.\_\_\_HTML\_TAG\_USD\_0\_\_\_**Applicable version:** 22 - 24 |
| [801](../../apis-ads-kit/errorcode-ads.md#801-ad-request-failure) | Capability not supported.Failed to call the API due to limited device capabilities. |
...
Allows the main window of the application to customize its display size scale factor. Existing child windows and system windows do not immediately re-layout to match the main window's new scale factor. They will re-layout to reflect this change only when their layout information (such as position, size, and system scale size) changes. If both this API and [setDefaultDensityEnabled(true)]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ are called, the setting from the last called API will be applied.
...
Allows the main window of the application to customize its display size scale factor and control when child windows and system windows re-layout to match the main window. If both this API and [setDefaultDensityEnabled(true)]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ are called, the setting from the last called API will be applied.
...
Sets whether the main window of the application uses the system's default density. Child windows and system windows will follow the main window's setting. Before calling this API, call [WindowStage.loadContent()]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ to initialize the layout to ensure the correct call sequence. If this API is not called, the default density is not used. When the default density is not used, if [setCustomDensity()]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ has been called, the window will be re-laid out according to the custom display size changes. Otherwise, it will be re-laid out according to the system display size changes.
...
Sets the image displayed in the multitasking view and on dock hover. This API uses a promise to return the result. > **NOTE** > > Before calling this API, you are advised to complete page loading via > [loadContent]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ or > [setUIContent]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_. If this API is called before the application > completes page loading, the intended functionality does not take effect. As a result, only the application's > launch page is displayed in the multitasking view.
