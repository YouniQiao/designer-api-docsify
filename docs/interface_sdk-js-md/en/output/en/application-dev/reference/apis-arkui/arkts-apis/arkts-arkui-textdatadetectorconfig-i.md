...
Whether to enable the preview menu displayed when long-pressing recognized text. The value **true** means to enable the preview menu, and **false** means the opposite. Default value: **false** When [copyOptions]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ is set to **None**, even if **enablePreviewMenu** is set to **true**, long-pressing AI entities will not display the preview menu. This API can be properly called on phones and tablets, but has no effect on other devices such as PCs, 2-in-1 devices, TVs, and wearables.
