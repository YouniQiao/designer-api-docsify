...
List of window IDs that are not displayed during screen capture. By default, this list is empty. Valid window IDs must be positive integers. Currently, this parameter applies only to [floating ball windows]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_. If a window ID does not correspond to a floating ball window, is not a positive integer, or does not exist, error code 401 is reported. You are advised to call [getFloatingBallWindowInfo()]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ to obtain the window ID of a floating ball window.
