...
Transition effect to apply when the video preview image changes. This parameter does not take effect if **showFirstFrame** is **true**, or if a valid **previewUri** in [VideoOptions]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ is not provided. Default value: **ContentTransitionEffect.IDENTITY**. If this parameter is set to **undefined** or **null**, it defaults to **ContentTransitionEffect.IDENTITY**.
...
Whether to enable first frame display, showing the first frame of the video as a preview. When first frame display is enabled, the previewUri field in [VideoOptions]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ has no effect. **true**: Enable first frame display. **false**: Disable first frame display. Default value: **false**.
