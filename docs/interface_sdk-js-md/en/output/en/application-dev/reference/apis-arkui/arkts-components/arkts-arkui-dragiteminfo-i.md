...
Custom component to display during dragging. If **pixelMap** is set, this parameter is ignored. **NOTE** Global builder definition is not supported. If the [Image]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ component is used in the builder, enable synchronous loading whenever possible, that is, set the [syncLoad]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_ attribute of the component to **true**. The builder is used only to generate the image displayed during the current dragging. Changes to the builder, if any, apply to the next dragging, but not to the current dragging. When passing the builder as a parameter, the format builder: ()=>{this.customBuilder()} is recommended to ensure correctness of this binding. For details, see \_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_.
