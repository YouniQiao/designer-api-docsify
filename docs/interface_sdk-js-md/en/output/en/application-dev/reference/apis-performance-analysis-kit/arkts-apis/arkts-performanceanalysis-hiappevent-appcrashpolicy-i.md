...
The policy for APP\_CRASH event. Whether to print the mapping information of all virtual memory areas (VMAs) in the crash log, that is, the **Maps** field in the crash log. The value **true** means to print only the VMA mapping information of the addresses in the crash log to reduce the log size. The value **false** means to print all VMA mapping information. The default value is **false**.
