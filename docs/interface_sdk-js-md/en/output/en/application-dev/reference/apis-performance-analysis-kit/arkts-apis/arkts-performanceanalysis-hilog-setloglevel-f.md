...
Sets the minimum log level of the current application process. You can configure different preference strategies using the **prefer** parameter. The **PREFER\_CLOSE\_LOG** strategy has the same effect as the **setMinLogLevel()** function. > **NOTE** > > This function does not take effect for debug applications.
