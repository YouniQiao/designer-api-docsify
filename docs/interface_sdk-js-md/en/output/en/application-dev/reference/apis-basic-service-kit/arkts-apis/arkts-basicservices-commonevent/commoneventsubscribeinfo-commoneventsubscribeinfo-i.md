...
Device ID, which is used to restrict the subscriber to receive only public events published by the specified device. Use [@ohos.deviceInfo]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ to obtain the UDID as the device ID of the publisher. Not supported currently.
...
User ID, which is used to restrict the subscriber to receive only public events related to the specified user ID. If this parameter is not specified, the default value, which is the ID of the current user, will be used. The value must be an existing user ID in the system. Use [getOsAccountLocalId]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ to obtain the system user ID and use it as the user ID of the publisher.
