...
Path where the downloaded file is stored. The default value is the cache directory of the caller (that is, the input **context**). The default file name is the part truncated from the last slash (/) in the URL. - In the FA model, use the \_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_ method to obtain the application storage path. - In the Stage model, use the **AbilityContext** class in [Context (Context Base Class of the Stage Model)]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ to obtain the file path.
...
Network type that can be used for download. The allowed network type is determined by bitwise operation of \_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_ . The following settings are supported: - Only the cellular network is supported. The parameter is **NETWORK\_MOBILE** or **0x00000001**. - Only WLAN is supported. The parameter is **NETWORK\_WIFI** or **0x00010000**. - Both cellular network and WLAN are supported, which is the default settings. The parameter is **NETWORK\_MOBILE **
