...
Indicates that the default application for opening a file has changed. This common event is sent when the default application for opening a file changes. To subscribe to this common event, your application must have the ohos.permission.CHANGE\_DEFAULT\_APPLICATION permission.
...
Indicates that the application shortcut has changed. This common event is sent when the shortcut is changed (for example, when \_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_ of the shortcutManager module is successfully called). To subscribe to this common event, your application must have the ohos.permission.MANAGE\_SHORTCUTS permission.
...
Indicates that a screen sharing event has occurred in the system. This is a protected common event and can be sent only by the system. To subscribe to this common event, your application must have the ohos.permission.RECEIVE\_SMS permission.(This permission is available only for system applications.)
