...
Upgrade status, which indicates the current execution phase of the upgrade task. The value can be a download status (from **WAITING\_DOWNLOAD** to **DOWNLOAD\_FAIL**), installation status (from **WAITING\_INSTALL** to **UPDATING**), effective status (from **WAITING\_APPLY** to **APPLYING**), or the final result ( **UPGRADE\_SUCCESS** or **UPGRADE\_FAIL**). This parameter is used for status monitoring, progress display, and exception handling.
