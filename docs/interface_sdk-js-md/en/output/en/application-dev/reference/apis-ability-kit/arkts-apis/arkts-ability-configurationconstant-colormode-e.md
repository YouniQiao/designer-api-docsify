# ColorMode

Enumerates the dark/light color modes, which are used in the [Configuration.colorMode]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ field. You can use these predefined enumerated values to set or obtain the dark/light color mode of the system or application.

**Since:** 9

**ArkTS mode:** ArkTS-Dyn since version 9; ArkTS-Sta since version 23.

<!--Device-ConfigurationConstant-export enum ColorMode--><!--Device-ConfigurationConstant-export enum ColorMode-End-->

**System capability:** SystemCapability.Ability.AbilityBase

## COLOR_MODE_NOT_SET

```TypeScript
COLOR_MODE_NOT_SET = -1
```

Unspecified color mode.

**Since:** 9

**ArkTS mode:** ArkTS-Dyn since version 9; ArkTS-Sta since version 23.

**Atomic service API:** This API can be used in atomic services since API version 11.

<!--Device-ColorMode-COLOR_MODE_NOT_SET = -1--><!--Device-ColorMode-COLOR_MODE_NOT_SET = -1-End-->

**System capability:** SystemCapability.Ability.AbilityBase

## COLOR_MODE_DARK

```TypeScript
COLOR_MODE_DARK = 0
```

Dark mode.

**Since:** 9

**ArkTS mode:** ArkTS-Dyn since version 9; ArkTS-Sta since version 23.

**Atomic service API:** This API can be used in atomic services since API version 11.

<!--Device-ColorMode-COLOR_MODE_DARK = 0--><!--Device-ColorMode-COLOR_MODE_DARK = 0-End-->

**System capability:** SystemCapability.Ability.AbilityBase

## COLOR_MODE_LIGHT

```TypeScript
COLOR_MODE_LIGHT = 1
```

Light mode.

**Since:** 9

**ArkTS mode:** ArkTS-Dyn since version 9; ArkTS-Sta since version 23.

**Atomic service API:** This API can be used in atomic services since API version 11.

<!--Device-ColorMode-COLOR_MODE_LIGHT = 1--><!--Device-ColorMode-COLOR_MODE_LIGHT = 1-End-->

**System capability:** SystemCapability.Ability.AbilityBase

