...
Obtains information about the running processes of the current application. This API uses a promise to return the result. > **NOTE** > > - In versions earlier than API version 11, this API requires the ohos.permission.GET\_RUNNING\_INFO permission, > which is available only for system applications. > > - Starting from API version 11, this API is used only to obtain the process information of the caller. No > permission is required.
...
Obtains information about the running processes of the current application. This API uses an asynchronous callback to return the result. > **NOTE** > > - In versions earlier than API version 11, this API requires the ohos.permission.GET\_RUNNING\_INFO permission, > which is available only for system applications. > > - Starting from API version 11, this API is used only to obtain the process information of the caller. No > permission is required.
