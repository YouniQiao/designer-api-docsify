...
The asset can be accessed only after the device is unlocked for the first time. **Note**: If no lock screen password is set, this option is equivalent to **DEVICE\_POWERED\_ON**.
...
The asset can be accessed only when the device is unlocked. **Note**: If no lock screen password is set, this option is equivalent to **DEVICE\_POWERED\_ON**.
