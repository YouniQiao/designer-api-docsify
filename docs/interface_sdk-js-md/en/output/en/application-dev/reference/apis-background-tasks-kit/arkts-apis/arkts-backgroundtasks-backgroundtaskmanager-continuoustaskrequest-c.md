...
Checks whether **BackgroundTaskMode** specified in [ContinuousTaskRequest]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ is supported. For details, see [BackgroundTaskMode]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_.
...
| Type | Description |
| --- | --- |
| boolean | Whether **BackgroundTaskMode** is supported. The value **true** means it is supported, and |
...
Requests user authorization to run tasks continuously in the background. This API uses an asynchronous callback to return the result. If the API call is successful, a banner notification with a sound is sent. This API is applicable only to continuous tasks of the [MODE\_SPECIAL\_SCENARIO\_PROCESSING]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ type.
...
Whether to combine notifications. The value **true** means to combine notifications, and the value **false** ( default) means the opposite. Note: This property does not take effect in [updateBackgroundRunning]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ API. If notifications need to be combined for an existing task, request the task again and set the value to **true**.
