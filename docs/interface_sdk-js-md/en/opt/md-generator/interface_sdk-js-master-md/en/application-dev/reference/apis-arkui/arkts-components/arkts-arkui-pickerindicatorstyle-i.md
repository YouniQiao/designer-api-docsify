...
Background border radius of the selected item. Value range: no more than half of the smaller value between the width and height of the selected item. If the value Default value: { value:12, unit:LengthUnit.vp }, meaning 12 vp for all corners is less than 0, the default value is used. If the value is greater than the maximum value, the maximum value is used. NOTE 1. This parameter takes effect only when **type** is set to **PickerIndicatorType.BACKGROUND**. 2. [LengthMetrics](../arkts-apis/arkts-arkui-graphics-lengthmetrics-c.md#LengthMetrics): Sets the size and unit of the four corner radii in a unified manner. 3. BorderRadiuses: Sets the size (unit: vp) of the four corner radii individually. 4. [LocalizedBorderRadiuses](../arkts-apis/arkts-arkui-localizedborderradiuses-i.md#LocalizedBorderRadiuses): Sets the size and unit of the four corner radii individually.
