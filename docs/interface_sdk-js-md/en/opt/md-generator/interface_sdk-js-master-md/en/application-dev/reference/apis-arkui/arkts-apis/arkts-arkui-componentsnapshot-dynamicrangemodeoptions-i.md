...
1. If SDR is used for the component, SDR is applied for the snapshot even if the dynamic range mode is set to **HIGH**.2. If HDR is used for the component, the specified dynamic range mode is applied for the screenshot.3. If the [color space](arkts-arkui-componentsnapshot-colormodeoptions-i.md) is set to **SRGB** or **DISPLAY_P3**, SDR is applied for the snapshot.4. If both SDR and HDR are used for the child components, HDR is applied for the snapshot.5. If both conditions 3 and 4 are met, SDR is applied for the snapshot.
...
[SnapshotOptions](arkts-arkui-componentsnapshot-snapshotoptions-i.md) to **true** to ensure that the system can properly detect the used dynamic range mode.
