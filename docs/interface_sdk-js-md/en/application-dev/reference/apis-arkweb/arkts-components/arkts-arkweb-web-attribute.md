...
Triggered when an error occurs during web page loading. The error may occur on the main resource or sub-resource.You can use [isMainFrame](arkts-arkweb-webresourcerequest-c.md#ismainframe) to determine whether the error occurs on the main resource. This API is called when there is no network connection.
