# @ohos.multimedia.avInputCastPicker

## Modules to Import

```TypeScript
import { AVInputCastPicker } from '@kit.AVSessionKit';
```

## Summary

### Structs

| Name | Description |
| --- | --- |
| [AVInputCastPicker](arkts-avsession-multimediaavinputcastpicker-avinputcastpicker-s.md) | Picker used to show available input devices. @struct { AVInputCastPicker } |

### Types

| Name | Description |
| --- | --- |
| [OnPickerStateCallback](arkts-avsession-onpickerstatecallback-t.md) | Callback for picker state |

