# @ohos.app.ability.InsightIntentDecorator

## Modules to Import

```TypeScript
```

## Summary

### Interfaces

| Name | Description |
| --- | --- |
| [LinkIntentParamMapping](arkts-app-ability-insightintentdecorator-linkintentparammapping-i.md) | Declare interface of LinkIntentParamMapping. |

### Enums

| Name | Description |
| --- | --- |
| [LinkParamCategory](arkts-app-ability-insightintentdecorator-linkparamcategory-e.md) | Enum definition of the paramCategory [paramCategory](arkts-app-ability-insightintentdecorator-linkintentparammapping-i.md#paramcategory), paramCategory is an attribute of LinkIntentParamMapping and used in InsightIntentLink InsightIntentLink. |

