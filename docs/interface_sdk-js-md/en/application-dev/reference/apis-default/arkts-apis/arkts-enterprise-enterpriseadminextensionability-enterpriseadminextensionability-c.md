...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| event | common.PolicyChangedEvent | Yes | 策略变更事件，包含策略变更的详细信息，如变更的应用包名（bundleName）、变更的函数名（functionName）、变更的参数（ parameters）和变更时间（time），超级设备管理应用可根据这些信息进行策略审计或同步。 |
...
| accountId | number | Yes | 被更新应用所在的用户ID。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| keyEvent | systemManager.KeyEvent | Yes | 当前发生的按键事件信息，包含按键码（keyCode）、按键动作（keyAction，如按下/抬起）、触发时间（actionTime）、已按下 按键列表（keyItems）等，用于识别和处理用户的按键操作。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| result | common.Result | Yes | 日志收集结果，用于标识日志收集是否成功，常见值为SUCCESS（收集成功）或FAIL（收集失败），开发者可根据结果判断是否执行后续的日志获取操作。 |
...
| result | common.InstallationResult | Yes | 安装结果，表示应用市场应用的安装状态，包含安装成功或失败的状态信息。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| scene | common.StartupScene | Yes | 开机向导完成场景，标识触发回调的具体场景类型，如用户设置完成（USER_SETUP）、OTA升级完成（OTA）、设备配置完成（ DEVICE_PROVISION）等，开发者可根据不同场景执行相应的业务逻辑。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| systemUpdateInfo | systemManager.SystemUpdateInfo | Yes | 系统更新的版本信息，用于通知设备管理应用系统版本更新情况。 |
...
**Type:** [EnterpriseAdminExtensionContext](../../apis-mdm-kit/arkts-apis/arkts-mdm-enterpriseadminextensioncontext-c.md)
