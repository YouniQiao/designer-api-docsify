...
Defines a readable stream. You need to use [fileIo.createReadStream](arkts-fileio-createreadstream-f.md) to create a **ReadStream** instance, which is inherited from [stream.Readable](../../apis-arkts/arkts-apis/arkts-arkts-stream-readable-c.md).The data obtained by **ReadStream** is a decoded string. Currently, only the UTF-8 format is supported.
