# SecurityDpiFollowStrategy (System API)

Enumeration of different types of SecurityDpiFollowStrategy.

**Since:** 26.0.0

**ArkTS mode:** ArkTS-Sta since version 26.0.0.

<!--Device-unnamed-export declare enum SecurityDpiFollowStrategy--><!--Device-unnamed-export declare enum SecurityDpiFollowStrategy-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

**System API:** This is a system API.

## FOLLOW_HOST_DPI

```TypeScript
FOLLOW_HOST_DPI = 0
```

Follow host DPI

**Since:** 26.0.0

**ArkTS mode:** ArkTS-Sta since version 26.0.0.

**Model restriction:** This API can be used only in the stage model.

<!--Device-SecurityDpiFollowStrategy-FOLLOW_HOST_DPI = 0--><!--Device-SecurityDpiFollowStrategy-FOLLOW_HOST_DPI = 0-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

**System API:** This is a system API.

## FOLLOW_UI_EXTENSION_ABILITY_DPI

```TypeScript
FOLLOW_UI_EXTENSION_ABILITY_DPI = 1
```

DPI following provider UIExtensionAbility

**Since:** 26.0.0

**ArkTS mode:** ArkTS-Sta since version 26.0.0.

**Model restriction:** This API can be used only in the stage model.

<!--Device-SecurityDpiFollowStrategy-FOLLOW_UI_EXTENSION_ABILITY_DPI = 1--><!--Device-SecurityDpiFollowStrategy-FOLLOW_UI_EXTENSION_ABILITY_DPI = 1-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

**System API:** This is a system API.

