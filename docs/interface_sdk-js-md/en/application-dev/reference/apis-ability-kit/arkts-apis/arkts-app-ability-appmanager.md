...
| off | Unregisters the observer used to listen for application start or exit events. |
| off | Deregisters the observer used to listen for the complete of the first frame rendering of a given ability. |
| on | Registers an application state observer, which allows you to filter for specific application lifecycle changes by setting filter criteria. |
| on | Registers an observer to listen for application start or exit events. The observer can be used by a system application to observe the start or event events of all applications. |
| on | Registers an observer to listen for the complete of the first frame rendering of a given ability. |
...
| Name | Description |
| --- | --- |
| [AppStateFilter](arkts-ability-appmanager-appstatefilter-i-sys.md) | Describes the filter for application lifecycle change events. It can be used as a parameter of [on](arkts-ability-appmanager-on-f.md#onapplicationstate) to filter application lifecycle change events you want to listen for. |
