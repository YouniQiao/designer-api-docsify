...
| --- | --- | --- | --- |
| key | string | Yes | 目标UDMF数据唯一标识。key必须指定应用（即callerTokenId对应的应用）通过 [unifiedDataChannel.insertData](../../apis-arkdata/arkts-apis/arkts-arkdata-unifieddatachannel-insertdata-f.md/arkts-arkdata-unifieddatachannel-insertdata-f.md#insertdata) 创建，且写入的数据均为有权限授权的文件URI。&lt;br&gt;当前仅支持SYSTEM_SHARE、PICKER和MENU类型的 [UDMF数据通路](../../apis-arkdata/arkts-apis/arkts-arkdata-unifieddatachannel-intention-e.md/arkts-arkdata-unifieddatachannel-intention-e.md)的key。key的创建与使用方法详见 [标准化数据通路实现数据共享](../../../database/unified-data-channels.md)。 |
...
| callerTokenId | ArkTS-Dyn: number  <br>ArkTS-Sta：int | Yes | 拉起方应用的身份标识，可以通过[want](arkts-ability-app-ability-want-want-c.md)中的"ohos.aafwk.param.callerToken"字 段获取。 |
