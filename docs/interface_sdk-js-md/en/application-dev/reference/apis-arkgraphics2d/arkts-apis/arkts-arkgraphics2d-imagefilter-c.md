...
| background | [ImageFilter](arkts-arkgraphics2d-imagefilter-c.md) | Yes | Filter that serves as the destination color in blend mode. |
| foreground | [ImageFilter](arkts-arkgraphics2d-imagefilter-c.md) | Yes | Filter that serves as the source color in blend mode. |

**Return value:**

| Type | Description |
| --- | --- |
| [ImageFilter](arkts-arkgraphics2d-imagefilter-c.md) | Image filter created. |
...
| tileMode | [TileMode](arkts-arkgraphics2d-tilemode-e.md) | Yes | Tile mode to apply to the edges. |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| cOuter | [ImageFilter](arkts-arkgraphics2d-imagefilter-c.md) | Yes | The second filter in the cascade, which processes the first filter's output. If the second filter is empty and the first filter is not empty, the final result is the first filter's output.The two filters cannot be empty at the same time. |
| cInner | [ImageFilter](arkts-arkgraphics2d-imagefilter-c.md) | Yes | The first filter in the cascade, which directly processes the original image content. If the first filter is empty and the second filter is not empty, the final result is the second filter's output. The two filters cannot be empty at the same time. |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| shader | [ShaderEffect](arkts-arkgraphics2d-shadereffect-c.md) | Yes | Shader effect to be applied to the image. |
