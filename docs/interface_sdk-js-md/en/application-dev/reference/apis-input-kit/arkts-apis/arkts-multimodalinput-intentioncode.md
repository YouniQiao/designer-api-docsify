# @ohos.multimodalInput.intentionCode(Intention Code)

## Modules to Import

```TypeScript
import { IntentionCode } from '@kit.InputKit';
```

## Summary

### Enums

| Name | Description |
| --- | --- |
| [IntentionCode(Intention Code)](arkts-input-multimodalinputintentioncode-intentioncode-e.md) | The **intentionCode** module maps the original events of the keyboard to intention codes for normalized interaction. For example, if the mapped event of the space bar on the keyboard is **INTENTION_SELECT**, the intent is to select an item. |

