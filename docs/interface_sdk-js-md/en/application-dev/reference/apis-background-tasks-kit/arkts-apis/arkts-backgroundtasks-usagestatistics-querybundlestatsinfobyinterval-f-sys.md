...
| --- | --- | --- | --- |
| byInterval | [IntervalType](arkts-backgroundtasks-bundlestate-intervaltype-e.md) | Yes | 查询类型。 |
| begin | ArkTS-Dyn: number  <br>ArkTS-Sta：long | Yes | 起始时间，单位：ms。 |
| end | ArkTS-Dyn: number  <br>ArkTS-Sta：long | Yes | 结束时间，单位：ms。 |
| callback | [AsyncCallback](../../apis-basic-service-kit/arkts-apis/arkts-basicservices-base-asynccallback-i.md)&lt;Array&lt;BundleStatsInfo&gt;&gt; | Yes | 回调函数。 当查询成功，err为undefined，data为指定时间段间隔内，应用使用时长的统计信息；否则为错误对象。 |
