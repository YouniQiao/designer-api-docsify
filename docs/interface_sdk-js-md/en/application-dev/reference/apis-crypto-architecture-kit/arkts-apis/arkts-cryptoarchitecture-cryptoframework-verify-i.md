...
[createVerify(algName: string): Verify]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_. Invoke **init()**, **update()**, and  
**verify()** in this class in sequence to complete the signature verification. For details about the sample code, see  
\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_.

\_\_\_HTML\_TAG\_DESC\_USD\_2\_\_\_The **Verify** class does not support repeated initialization. When a new key is used for signature verification,you must create a new **Verify** instance and call **init()** for initialization.

\_\_\_HTML\_TAG\_DESC\_USD\_3\_\_\_The signature verification mode is determined in **createVerify()**, and the key is set by **init()**.

\_\_\_HTML\_TAG\_DESC\_USD\_4\_\_\_If the signed message is short, you can call **verify()** to pass in the signed message and signature (  
**signatureData**) for signature verification after **init()**. That is, you do not need to use **update()**.

\_\_\_HTML\_TAG\_DESC\_USD\_5\_\_\_If the signed message is too long, you can call **update()** multiple times to pass in the signed message by segment, and then call **verify()** to verify the full text of the message. In versions earlier than API version 10, the input parameter **data** of **verify()** supports only **DataBlob**. Since API version 10, **data** also supports **null**. After all the data is passed in by using **update()**, **verify()** can be called to verify the signature data.

\_\_\_HTML\_TAG\_DESC\_USD\_6\_\_\_If the DSA algorithm is used for signature verification and the digest algorithm is **NoHash**, **update()** is not supported. If **update()** is called in this case, **ERR\_CRYPTO\_OPERATION** will be returned.
...
Sets signature verification specifications. You can use this API to set signature verification parameters that cannot be set by [createVerify]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_.

\_\_\_HTML\_TAG\_DESC\_USD\_1\_\_\_Currently, only RSA and SM2 are supported. Since API version 11, SM2 signature verification parameters can be set.

\_\_\_HTML\_TAG\_DESC\_USD\_2\_\_\_The parameters for signature verification must be the same as those for signing.
...
\_\_\_HTML\_TAG\_DESC\_USD\_0\_\_\_Currently, only PSS\_SALT\_LEN in RSA and USER\_ID in SM2 are supported.

\_\_\_HTML\_TAG\_DESC\_USD\_1\_\_\_The parameters for signature verification must be the same as those for signing.
...
\_\_\_HTML\_TAG\_DESC\_USD\_0\_\_\_Currently, only PSS\_SALT\_LEN in RSA, USER\_ID in SM2, and ML\_DSA\_DETERMINISTIC, ML\_DSA\_MU and ML\_DSA\_CONTEXT in ML-DSA are supported.
...
\_\_\_HTML\_TAG\_DESC\_USD\_1\_\_\_Currently, only ML\_DSA\_DETERMINISTIC and ML\_DSA\_MU in ML-DSA are supported. For ML\_DSA\_CONTEXT parameter,use [setVerifySpec()]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_.\_\_\_HTML\_TAG\_DESC\_USD\_2\_\_\_The parameters for signature verification must be the same as those for signing.
...
\_\_\_HTML\_TAG\_DESC\_USD\_6\_\_\_This API can be called only after the [Verify]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ instance is initialized using  
[init]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_ or [initSync]\_\_\_JSDOC\_LINK\_DESC\_USD\_3\_\_\_.
    **NOTE**  
    
    You can call **update** multiple times or do not use **update** (call  
    [verify]\_\_\_JSDOC\_LINK\_DESC\_USD\_4\_\_\_  
    after [init]\_\_\_JSDOC\_LINK\_DESC\_USD\_5\_\_\_), depending on  
    the data volume.  
    ...
    **OnlyVerify** cannot be used with **update()**. If **OnlyVerify** is specified, use **verify()** to pass in  
    ...
    not supported. If **update()** is called in this case, **ERR\_CRYPTO\_OPERATION** will be returned.
...
\_\_\_HTML\_TAG\_DESC\_USD\_5\_\_\_This API can be called only after the [Verify]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ instance is initialized using  
[init()]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_.
    **NOTE**  
    
    You can call **update** multiple times or do not use **update** (call  
    [verify]\_\_\_JSDOC\_LINK\_DESC\_USD\_3\_\_\_ after  
    [init]\_\_\_JSDOC\_LINK\_DESC\_USD\_4\_\_\_), depending on the data volume.
    The amount of the data to be passed in by **update()** (one-time or accumulative) is not limited. If there is a  
    large amount of data, you are advised to call **update()** multiple times to pass in the data by segment. This  
    prevents too much memory from being requested at a time.
    For details about the sample code for calling **update()** multiple times in signature verification, see  
...
\_\_\_HTML\_TAG\_DESC\_USD\_6\_\_\_This API can be called only after the [Verify]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ instance is initialized by using  
[initSync()]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_.
    **NOTE**  
    
    You can call **updateSync** multiple times or do not use **updateSync** (call  
    [verifySync]\_\_\_JSDOC\_LINK\_DESC\_USD\_3\_\_\_ after [initSync]\_\_\_JSDOC\_LINK\_DESC\_USD\_4\_\_\_),  
    ...
    **OnlyVerify** cannot be used with **updateSync()**. If **OnlyVerify** is specified, use **verifySync()** to pass  
    ...
    is not supported. If **updateSync** is called in this case, **ERR\_CRYPTO\_OPERATION** will be returned.
