...
Number used once. <br>For AES-CCM, the nonce length ranges from 7 to 13 bytes. For AES-GCM, the nonce length ranges from 1 to 128 bytes, 12 bytes are recommended. For SM4-GCM, the nonce length ranges from 1 to 128 bytes, 12 bytes are recommended. For ChaCha20-Poly1305, the nonce length must be 12 bytes.
