...
- [LiteResultSet](arkts-data-relationalstore.md): provides APIs for accessing the result set obtained from the RDB store, such as [queryWithoutRowCount](arkts-arkdata-rdbstore-i.md#querywithoutrowcount-1) and [querySqlWithoutRowCount](arkts-arkdata-rdbstore-i.md#querysqlwithoutrowcount-1). Unlike [ResultSet](arkts-data-relationalstore.md), **LiteResultSet** does not include the total number of rows in the query result.  
...
| [SecurityLevel](arkts-arkdata-securitylevel-e.md) | Enumerates the KV store security levels. Use the enum name rather than the enum value. You cannot change the security level of an RDB store from a higher level to a lower one. |
