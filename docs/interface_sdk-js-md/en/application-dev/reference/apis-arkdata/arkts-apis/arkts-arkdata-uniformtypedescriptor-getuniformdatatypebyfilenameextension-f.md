...
| --- | --- | --- | --- |
| filenameExtension | string | Yes | 文件后缀名称，需要包含点号，如'.ts'、'.jpg'等。 |
| belongsTo | string | No | 要查询的标准化数据类型所归属类型ID，用于限定查询范围。当需要查询特定归属类型下的数据类型时传入此参数，无默认值，若不传入此参数则只按照文件后缀名称查询[标准化数据类型ID]。 |
