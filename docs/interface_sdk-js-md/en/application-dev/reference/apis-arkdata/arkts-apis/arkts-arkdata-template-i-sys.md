...
Predicates to use. When [**on**](arkts-arkdata-datasharehelper-i-sys.md#on-3)is called, the predicates are used to generate data. This parameter applies only to RDB data storage.
...
Update SQL statement of a specified template. The default value is an empty string. When [on](arkts-arkdata-datasharehelper-i-sys.md#on-3)is called, the **update** parameter is used to update data. This parameter applies only to RDB data storage.
