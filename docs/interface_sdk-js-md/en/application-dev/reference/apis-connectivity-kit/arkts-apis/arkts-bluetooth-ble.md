...
| off_advertisingStateChange | Unsubscribe from advertising state change event. |
...
| [on_BLEDeviceFind](arkts-connectivity-ble-onbledevicefind-f.md#on_BLEDeviceFind) | Subscribe BLE scan result. On API 26.0.0 and above, if the application has ohos.permission.GET_BLUETOOTH_PEERS_MAC, the type of the peer device address is real. Otherwise, the type of the peer device address is virtual. |
| on_advertisingStateChange | Subscribing to advertising state change event. |
