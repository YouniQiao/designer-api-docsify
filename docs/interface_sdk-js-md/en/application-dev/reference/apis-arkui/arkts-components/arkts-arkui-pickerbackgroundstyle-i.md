...
Border radius of the selected item. Default value: **{ value:24, unit:LengthUnit.VP }**, meaning 24 vp for all corners. **NOTE：**1. [LengthMetrics](../arkts-apis/arkts-arkui-graphics-lengthmetrics-c.md): uniform radius with a customizable unit 2. BorderRadiuses: per-corner radius values (vp units only) 3. LocalizedBorderRadiuses: per-corner radius values with individual units
