# LayoutSafeAreaType

Enumerates the types for expanding layout safe areas.

**Since:** 12

<!--Device-unnamed-declare enum LayoutSafeAreaType--><!--Device-unnamed-declare enum LayoutSafeAreaType-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## SYSTEM

```TypeScript
SYSTEM = 0
```

Default non-safe area of the system, including the status bar and navigation bar.The component's layout range can be expanded to include both component-level safe areas([safeAreaPadding](arkts-arkui-common-commonmethod-c.md#safeareapadding-1)) and page-level safe areas (status bar, navigation bar, and cutout area).

**Since:** 12

**Model restriction:** This API can be used only in the stage model.

**Atomic service API:** This API can be used in atomic services since API version 12.

<!--Device-LayoutSafeAreaType-SYSTEM = 0--><!--Device-LayoutSafeAreaType-SYSTEM = 0-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

