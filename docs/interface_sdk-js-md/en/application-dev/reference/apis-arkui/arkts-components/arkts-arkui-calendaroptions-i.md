...
Style of the background of the selected state.Value range: [0.0, 16.0]Unit: vp.Default value: **16.0** (the background is a circle).  
**NOTE：**If the value is **0.0**, the background is a right-angled rectangle. If the value is in the (0.0, 16.0) range, the background is a rounded rectangle. If the value is a negative number or greater than 16.0, the default value **16.0** is used, which means the background is a circle.
