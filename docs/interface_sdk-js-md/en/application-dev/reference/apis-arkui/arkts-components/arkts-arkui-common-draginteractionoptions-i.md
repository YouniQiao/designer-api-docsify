...
**false**: Disable haptic feedback during dragging. This parameter is effective only for previews with masks (configured using [bindContextMenu](arkts-arkui-common-commonmethod-c.md#bindcontextmenu-2)).
...
With the value **true**, only the custom menu preview (set using [bindContextMenu](arkts-arkui-common-commonmethod-c.md#bindcontextmenu-1)), also known as the long-press preview, is displayed if both the long-press preview and drag preview are configured.
...
Whether to enable multi-select clustering during drag operations. **true** to enable, **false** otherwise. This parameter takes effect only for the [grid items](./gridItem) and [list items](./list_item) in the [Grid](./grid) and [List](./list) containers.

When this feature is enabled, child components cannot be dragged individually. Preview priority: string in [dragPreview](arkts-arkui-common-commonmethod-c.md#dragpreview-1) > PixelMap in **dragPreview** > component snapshot. Builder previews not supported.
