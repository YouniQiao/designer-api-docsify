# LongPressGestureEvent

Inherits from [BaseGestureEvent](arkts-arkui-gesture-basegestureevent-i.md). This object can be passed as the **event** parameter of [onGestureJudgeBegin](../arkts-components/arkts-arkui-common-commonmethod-c.md#ongesturejudgebegin-1).

**Inheritance/Implementation:** LongPressGestureEvent extends [BaseGestureEvent](arkts-arkui-gesture-basegestureevent-i.md)

**Since:** 11

<!--Device-unnamed-interface LongPressGestureEvent extends BaseGestureEvent--><!--Device-unnamed-interface LongPressGestureEvent extends BaseGestureEvent-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## repeat

```TypeScript
repeat: boolean
```

Whether the event is a repeated trigger event. **true**: The event is repeated. **false**: The event is not repeated.

**Type:** boolean

**Since:** 11

**Model restriction:** This API can be used only in the stage model.

**Atomic service API:** This API can be used in atomic services since API version 12.

<!--Device-LongPressGestureEvent-repeat: boolean--><!--Device-LongPressGestureEvent-repeat: boolean-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

