# ColorShaderStyle

Displays a solid color. **ColorShaderStyle** inherits from [ShaderStyle](arkts-arkui-text-common-shaderstyle-c.md).

**Inheritance/Implementation:** ColorShaderStyle extends [ShaderStyle](arkts-arkui-text-common-shaderstyle-c.md)

**Since:** 20

<!--Device-unnamed-declare class ColorShaderStyle extends ShaderStyle--><!--Device-unnamed-declare class ColorShaderStyle extends ShaderStyle-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## constructor

```TypeScript
constructor(color: ResourceColor)
```

A constructor used to create a **ResourceColor** object.

**Since:** 20

**Model restriction:** This API can be used only in the stage model.

**Atomic service API:** This API can be used in atomic services since API version 20.

<!--Device-ColorShaderStyle-constructor(color: ResourceColor)--><!--Device-ColorShaderStyle-constructor(color: ResourceColor)-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

**Parameters:**

| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| color | [ResourceColor](arkts-arkui-resourcecolor-t.md) | Yes | Options for displaying a solid color. |

## color

```TypeScript
color: ResourceColor
```

Options for displaying a solid color.

**Type:** ResourceColor

**Since:** 20

**Model restriction:** This API can be used only in the stage model.

**Atomic service API:** This API can be used in atomic services since API version 20.

<!--Device-ColorShaderStyle-color: ResourceColor--><!--Device-ColorShaderStyle-color: ResourceColor-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

