...
> 
> - **setMenuOptions** is a non-static API. You need to first use the [getTextMenuController()](arkts-arkui-arkui-uicontext-uicontext-c.md#gettextmenucontroller) method in UIContext to obtain a TextMenuController instance, and then call the corresponding method through this instance. **disableSystemServiceMenuItems** and
...
> 
> - Disabling a first-level menu item also disables all its second-level menu items. For example, disabling the first-level menu item autoFill (parent menu item) in [TextMenuItemId](arkts-arkui-textmenuitemid-c.md) also disables the second-level menu item passwordVault (child menu item).
> 
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| options | [TextMenuOptions](arkts-arkui-textmenuoptions-i.md) | Yes | Menu options for controlling the display mode of the text selection menu. |
