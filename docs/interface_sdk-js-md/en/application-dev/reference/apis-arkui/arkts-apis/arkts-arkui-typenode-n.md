...
| [getAttribute](arkts-arkui-typenode-getattribute-f.md#getattribute-9) | Obtains the attributes of a LoadingProgress node. If the node is not created using ArkTS, cross-language access must be enabled; otherwise, **undefined** is returned. This API does not support declaratively created nodes. |
