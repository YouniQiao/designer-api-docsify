...
| --- | --- | --- | --- |
| builder | [CustomBuilder](../arkts-components/arkts-arkui-custombuilder-t.md) | Yes | 创建对应节点树的时候所需的无状态UI方法[@Builder](../../../ui/state-management/arkts-builder.md)。 |
| options | [BuildOptions](arkts-arkui-buildernode-buildoptions-i.md) | No | build相关的配置项。&lt;br/&gt;默认值：参考[BuildOptions](arkts-arkui-buildernode-buildoptions-i.md)各个成员的默认值。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| uiContext | [UIContext](arkts-arkui-arkui-uicontext-uicontext-c.md) | Yes | UI上下文，获取方式可参考 [UIContext获取方法](../../../reference/apis-arkui/js-apis-arkui-node.md#uicontext获取方法)。uiContext需要为一个有效的值，即UI上下文正 确，如果传入非法值或者未设置，会导致创建失败。 |
| options | [RenderOptions](arkts-arkui-buildernode-renderoptions-i.md) | No | ReactiveBuilderNode的构造可选参数，参数用于构造节点的理想大小和节点的渲染类型。&lt;br/&gt;默认值：undefined |
...
> 不建议同一个事件转发多次。&lt;!--Del--&gt;
> 
> postInputEvent的参数不支持[UIExtensionComponent](ui_extension_component)。&lt;!--DelEnd--&gt;
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| event | [InputEventType](arkts-arkui-inputeventtype-t.md) | Yes | 待分发的输入事件。 |
...
| competitionStrategy | [CompetitionStrategy](arkts-arkui-competitionstrategy-e.md) | No | 分发事件的手势是否为竞争场景，默认为非竞争。 |
...
> 
> postTouchEvent的参数不支持[UIExtensionComponent](ui_extension_component)。
