...
Foreground color of the progress indicator. If the component's background color ( [backgroundColor](arkts-arkui-arkuiadvancedarcbutton-arcbuttonoptions-c.md)) is set, it is used as the default foreground color of the progress indicator. The foreground color of the progress indicator is not affected by the button style ( [ArcButtonStyleMode](arkts-arkui-arkuiadvancedarcbutton-arcbuttonstylemode-e.md)). The progress indicator's background color is derived solely from its foreground color, with an opacity value of 25%.
