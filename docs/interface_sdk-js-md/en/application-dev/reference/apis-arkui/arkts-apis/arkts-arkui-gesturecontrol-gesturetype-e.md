# GestureType

Enumerates gesture recognizer types.

**Since:** 11

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## TAP_GESTURE

```TypeScript
TAP_GESTURE = 0
```

Tap gesture.

**Since:** 11

**Atomic service API:** This API can be used in atomic services since API version 12.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## LONG_PRESS_GESTURE

```TypeScript
LONG_PRESS_GESTURE = 1
```

Long press gesture.

**Since:** 11

**Atomic service API:** This API can be used in atomic services since API version 12.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## PAN_GESTURE

```TypeScript
PAN_GESTURE = 2
```

Pan gesture.

**Since:** 11

**Atomic service API:** This API can be used in atomic services since API version 12.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## PINCH_GESTURE

```TypeScript
PINCH_GESTURE = 3
```

Pinch gesture.

**Since:** 11

**Atomic service API:** This API can be used in atomic services since API version 12.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## SWIPE_GESTURE

```TypeScript
SWIPE_GESTURE = 4
```

Swipe gesture.

**Since:** 11

**Atomic service API:** This API can be used in atomic services since API version 12.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## ROTATION_GESTURE

```TypeScript
ROTATION_GESTURE = 5
```

Rotation gesture.

**Since:** 11

**Atomic service API:** This API can be used in atomic services since API version 12.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## DRAG

```TypeScript
DRAG = 6
```

Drag.

**Since:** 11

**Atomic service API:** This API can be used in atomic services since API version 12.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## CLICK

```TypeScript
CLICK = 7
```

Click.

**Since:** 11

**Atomic service API:** This API can be used in atomic services since API version 12.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## BOX_SELECT_GESTURE

```TypeScript
BOX_SELECT_GESTURE = 8
```

Swipe gesture used to create a selection area by dragging with the mouse within a scroll container, enabling the batch selection of multiple elements.

**Since:** 23

**Model restriction:** This API can be used only in the stage model.

**Atomic service API:** This API can be used in atomic services since API version 23.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## WEB_SCROLL_GESTURE

```TypeScript
WEB_SCROLL_GESTURE = 9
```

Swipe gesture used to control the scrolling behavior of a **Web** component.

**Since:** 23

**Model restriction:** This API can be used only in the stage model.

**Atomic service API:** This API can be used in atomic services since API version 23.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## TEXT_FIELD_SELECT_GESTURE

```TypeScript
TEXT_FIELD_SELECT_GESTURE = 10
```

Swipe gesture used to select text content by dragging within an input box component.

**Since:** 23

**Model restriction:** This API can be used only in the stage model.

**Atomic service API:** This API can be used in atomic services since API version 23.

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## CONTEXT_MENU_HOVER_GESTURE

```TypeScript
CONTEXT_MENU_HOVER_GESTURE = 11
```

Context menu hover gesture. It is a special type of number-press gesture and triggers the **hoverScale** animation effect of the menu during the number-press process (this behavior requires enabling the **hoverScaleInterruption** attribute of [ContextMenuAnimationOptions](../arkts-components/arkts-arkui-contextmenuanimationoptions-i.md) for support).

**Since:** 23

**Model restriction:** This API can be used only in the stage model.

**Atomic service API:** This API can be used in atomic services since API version 23.

**System capability:** SystemCapability.ArkUI.ArkUI.Full
