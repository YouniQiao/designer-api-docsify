...
依照传入的无参数[CustomBuilder](../../../reference/apis-arkui/arkui-ts/ts-types.md#custombuildertt23)创建组件树，并持有组件树的根节点。
...
| --- | --- | --- | --- |
| builder | [WrappedBuilder](../arkts-components/arkts-arkui-wrappedbuilder-c.md)&lt;[CustomBuilder](../arkts-components/arkts-arkui-custombuilder-t.md)&gt; | Yes | 创建对应节点树的时候所需的无状态UI方法 [@Builder（ArkTS-Sta）](../../../ui/arkts-v1.2-user-defined-arktsNode-builderNode.md)封装的WrappedBuilder对象。 |
...
依照传入的带参数的[CustomBuilderT\&lt;T&gt;](../../../reference/apis-arkui/arkui-ts/ts-types.md#custombuildertt23)创建组件树，并持有组件树的根节点。无状态的UI方法[@Builder（ArkTS-Sta）](../../../ui/arkts-v1.2-user-defined-arktsNode-builderNode.md)最多拥有一个根节点。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| builder | [WrappedBuilder](../arkts-components/arkts-arkui-wrappedbuilder-c.md)&lt;CustomBuilderT&lt;T&gt;&gt; | Yes | 在构建节点树时所需的无状态UI方法 [@Builder（ArkTS-Sta）](../../../ui/arkts-v1.2-user-defined-arktsNode-builderNode.md)封装的WrappedBuilder对象。 |
| arg | T | Yes | builder的入参。当前仅支持一个入参，且入参对象类型与@Builder定义的入参类型保持一致。 |
...
依照传入的带参数的[CustomBuilderT\&lt;T&gt;](../../../reference/apis-arkui/arkui-ts/ts-types.md#custombuildertt23)创建组件树，并持有组件树的根节点。同时，支持设置build相关的配置项。无状态的UI方法@Builder最多拥有一个根节点。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| builder | [WrappedBuilder](../arkts-components/arkts-arkui-wrappedbuilder-c.md)&lt;CustomBuilderT&lt;T&gt;&gt; | Yes | 用于构建对应节点树的无状态UI方法 [@Builder（ArkTS-Sta）](../../../ui/arkts-v1.2-user-defined-arktsNode-builderNode.md)封装的WrappedBuilder对象。 |
| arg | T | Yes | builder的入参。 |
| options | [BuildOptions](arkts-arkui-buildernode-buildoptions-i.md) | Yes | 该值无效，默认支持@Builder参数不一致，且行为与@Builder的行为保持一致。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| uiContext | [UIContext](arkts-arkui-arkui-uicontext-uicontext-c.md) | Yes | UI上下文，获取方式可参考 [UIContext获取方法](../../../reference/apis-arkui/js-apis-arkui-node.md#uicontext获取方法)。 |
| options | [RenderOptions](arkts-arkui-buildernode-renderoptions-i.md) | No | BuilderNode的构造可选参数。&lt;br/&gt;默认值：undefined |
...
| Type | Description |
| --- | --- |
| [FrameNode](arkts-arkui-framenode-c.md) | Returns a FrameNode inside the BuilderNode, or null if not contained. |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| enabled | boolean | Yes | BuilderNode对象是否设置为继承父组件中自定义组件的冻结策略。true为继承父组件中自定义组件的冻结策略，false为不继承父组件中自定义组件的冻结策略。 |
...
> 
> - 不建议同一个事件转发多次。&lt;!--Del--&gt;不支持[UIExtensionComponent](ui_extension_component)。&lt;!--DelEnd--&gt;
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| event | [InputEventType](arkts-arkui-inputeventtype-t.md) | Yes | 用于透传的输入事件。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| event | [InputEventType](arkts-arkui-inputeventtype-t.md) | Yes | 用于事件分发的输入事件。 |
...
> 
> - 不支持[UIExtensionComponent](ui_extension_component)。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| param | [RecordData](../../apis-basic-service-kit/arkts-apis/arkts-basicservices-recorddata-t.md) | No | 用于复用BuilderNode的参数。该参数将直接用于BuilderNode中所有顶层自定义组件的复用，应该包含每个自定义组件的构造函数参数所需内容，否则，会导致未定 义行为。调用此方法将同步触发内部自定义组件的 [aboutToReuse](../../../reference/apis-arkui/arkui-ts/ts-custom-component-lifecycle.md#abouttoreuse10)生命周期回调，并 将该参数作为回调的入参。默认值为undefined，此时BuilderNode中的自定义组件将直接使用构造时的数据源。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| arg | T | Yes | 用于更新BuilderNode的参数，和[build](arkts-arkui-buildernode-c.md)调用时传入的参数类型一致。 |
