...
This module provides application logging and event subscription capabilities, including event storage, event subscription, event clearance, and logging configuration. HiAppEvent records the events triggered during application running in [AppEventInfo](arkts-performanceanalysis-appeventinfo-i.md), and classifies the events into system events and application events.
...
Application events are defined by application developers and can be customized using the [Write](arkts-performanceanalysis-write-f.md#write-1) API as required.
...
| Name | Description |
| --- | --- |
| [domain](arkts-performanceanalysis-domain-n.md) | Provides domain name constants.\| Name\| Type \| Read Only \| Description \| \| --- \| ------ \| ------ \| ---------- \| \| OS \| string \| Yes\| System domain.\| |
| [event](arkts-performanceanalysis-event-n.md) | Provides event name constants, including system event name constants and application event name constants.<br>The application event name constants are optional custom event names reserved when you call Write for application event logging. |
| [param](arkts-performanceanalysis-param-n.md) | Provides parameter name constants.\| Name \| Type \| Read Only \| Description \| \| ------------------------------- \| ------ \| ------ \| ------------------ \| \| USER_ID \| string \| Yes\| Custom user ID. \| \| DISTRIBUTED_SERVICE_NAME \| string \| Yes\| Distributed service name. \| \| DISTRIBUTED_SERVICE_INSTANCE_ID \| string \| Yes\| Distributed service instance ID.\| |
...
| [setUserId](arkts-performanceanalysis-setuserid-f.md#setuserid-1) | Sets a user ID, which is used for association when a [Processor](arkts-performanceanalysis-processor-i.md) is configured. |
| [getUserId](arkts-performanceanalysis-getuserid-f.md#getuserid-1) | Obtains the value set through **setUserId**. |
| [setUserProperty](arkts-performanceanalysis-setuserproperty-f.md#setuserproperty-1) | Sets a user property, which is used for association when a [Processor](arkts-performanceanalysis-processor-i.md) is configured. |
...
| [TriggerCondition](arkts-performanceanalysis-triggercondition-i.md) | Defines the triggering condition parameters of the **onTrigger** callback of a [Watcher](arkts-performanceanalysis-watcher-i.md). |
| [AppEventFilter](arkts-performanceanalysis-appeventfilter-i.md) | Defines parameters of subscription filtering conditions of a [Watcher](arkts-performanceanalysis-watcher-i.md). This API is used to set event filtering conditions in the event watcher to ensure that only the events that meet the filtering conditions are subscribed to. |
| [AppEventGroup](arkts-performanceanalysis-appeventgroup-i.md) | Defines parameters of the event group returned by the subscription. This API can be used to obtain detail information about an event group, which is often used in the **onReceive** callback of [Watcher](arkts-performanceanalysis-watcher-i.md). |
...
| [CpuUsageHighPolicy](arkts-performanceanalysis-cpuusagehighpolicy-i.md) | Defines the configuration policy for the high CPU usage event. |
