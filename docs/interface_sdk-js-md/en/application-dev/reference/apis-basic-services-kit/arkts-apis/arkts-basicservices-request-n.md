...
| [uploadFile](arkts-basicservices-request-uploadfile-f.md) | Uploads a file. This API uses an asynchronous callback to return the result. HTTP is supported. You can use [on('complete'\|'fail')](arkts-basicservices-request-uploadtask-i.md#on_progressprogress) to obtain the upload success or error information. > **NOTE：**> > For details about how to obtain the context in the example, see > [Obtaining the Context of UIAbility](../../../application-models/uiability-usage.md#obtaining-the-context-of-uiability) > . |
...
| [TaskState](arkts-basicservices-request-taskstate-i.md) | Upload task information, which is the callback parameter of the [on('complete' \| 'fail')](arkts-basicservices-request-uploadtask-i.md#on_progressprogress) and [off('complete' \| 'fail')](arkts-basicservices-request-uploadtask-i.md#off_progressprogress) APIs. |
