...
HashMap is implemented using an array, linked lists, and red-black trees as its core, supporting efficient querying,
 ...
 HashMap is faster in accessing data than [TreeMap](arkts-util-treemap.md), because the former accesses the keys
 based on the hash codes, whereas the latter stores and accesses the keys in sorted order.
 [HashSet](arkts-util-hashset.md) is implemented based on HashMap. The input parameter of HashMap consists of
