# FlexWrap

FlexWrap enumeration description

**Since:** 23

**ArkTS mode:** ArkTS-Sta only, since version 23.

**Deprecated since:** -1

<!--Device-unnamed-export declare enum FlexWrap--><!--Device-unnamed-export declare enum FlexWrap-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## NoWrap

```TypeScript
NoWrap = 0
```

The Flex container has a single row/column layout of elements, and children are allowed to go beyond the container.

**Since:** 23

**ArkTS mode:** ArkTS-Sta only, since version 23.

**Deprecated since:** -1

**Model restriction:** This API can be used only in the stage model.

<!--Device-FlexWrap-NoWrap = 0--><!--Device-FlexWrap-NoWrap = 0-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## Wrap

```TypeScript
Wrap = 1
```

The elements of the Flex container are arranged in multiple rows or columns, and the sub-items are allowed to exceed the container.

**Since:** 23

**ArkTS mode:** ArkTS-Sta only, since version 23.

**Deprecated since:** -1

**Model restriction:** This API can be used only in the stage model.

<!--Device-FlexWrap-Wrap = 1--><!--Device-FlexWrap-Wrap = 1-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## WrapReverse

```TypeScript
WrapReverse = 2
```

The elements of the Flex container are arranged in reverse multiple rows/columns, and children are allowed to exceed the container.

**Since:** 23

**ArkTS mode:** ArkTS-Sta only, since version 23.

**Deprecated since:** -1

**Model restriction:** This API can be used only in the stage model.

<!--Device-FlexWrap-WrapReverse = 2--><!--Device-FlexWrap-WrapReverse = 2-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

