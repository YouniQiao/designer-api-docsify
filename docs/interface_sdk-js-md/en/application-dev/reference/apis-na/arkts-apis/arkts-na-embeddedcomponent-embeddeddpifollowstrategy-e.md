# EmbeddedDpiFollowStrategy

Enumeration of different types of EmbeddedDpiFollowStrategy.

**Since:** 26.0.0

**ArkTS mode:** ArkTS-Sta only, since version 26.0.0.

**Deprecated since:** -1

<!--Device-unnamed-export declare enum EmbeddedDpiFollowStrategy--><!--Device-unnamed-export declare enum EmbeddedDpiFollowStrategy-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## FOLLOW_HOST_DPI

```TypeScript
FOLLOW_HOST_DPI = 0
```

Follow the host application's DPI configuration.

**Since:** 26.0.0

**ArkTS mode:** ArkTS-Sta only, since version 26.0.0.

**Deprecated since:** -1

**Model restriction:** This API can be used only in the stage model.

<!--Device-EmbeddedDpiFollowStrategy-FOLLOW_HOST_DPI = 0--><!--Device-EmbeddedDpiFollowStrategy-FOLLOW_HOST_DPI = 0-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

## FOLLOW_UI_EXTENSION_ABILITY_DPI

```TypeScript
FOLLOW_UI_EXTENSION_ABILITY_DPI = 1
```

Follow the EmbeddedUIExtensionAbility's DPI configuration.

**Since:** 26.0.0

**ArkTS mode:** ArkTS-Sta only, since version 26.0.0.

**Deprecated since:** -1

**Model restriction:** This API can be used only in the stage model.

<!--Device-EmbeddedDpiFollowStrategy-FOLLOW_UI_EXTENSION_ABILITY_DPI = 1--><!--Device-EmbeddedDpiFollowStrategy-FOLLOW_UI_EXTENSION_ABILITY_DPI = 1-End-->

**System capability:** SystemCapability.ArkUI.ArkUI.Full

