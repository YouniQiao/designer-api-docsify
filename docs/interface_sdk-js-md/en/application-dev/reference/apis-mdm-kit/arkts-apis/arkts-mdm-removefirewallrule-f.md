...
If there is no rule with [Action](arkts-mdm-action-e.md) being **ALLOW** after the rule is removed, the **DENY** rules that are added by default with [addFirewallRule](arkts-mdm-addfirewallrule-f.md#addfirewallrule-1) will be removed.
...
| firewallRule | [FirewallRule](arkts-mdm-firewallrule-i.md) | No | Firewall rule to remove. If the value is empty, all firewall rules will be removed. |
