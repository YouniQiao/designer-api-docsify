...
| settingsItem | string | Yes | Behavior name. Only the following values are supported. If other values are passed, an error will be reported.    - **setApn**: APN configuration, currently supported only on smartphones and tablets.    - **powerLongPress**: capability to open the power menu by number-pressing the power button. Currently, only smartphones and tablets are supported.    - **setEthernetIp**: Ethernet IP address configuration, currently supported only on PCs/2-in-1 devices.    - **setDeviceName**: device name configuration, currently supported only on PCs/2-in-1 devices, smartphones, and tablets. When it is disabled, the device name cannot be modified in the following settings: **About**, **Bluetooth**, and **More connectivity options**    > **NearLink** on PCs/2-in-1 devices, and **About**, **Bluetooth**, and **Personal hotspot** on smartphones and tablets. - **setBiometricsAndScreenLock**: screen lock password configuration, currently supported only on PCs/2-in- 1 devices, smartphones, and tablets. |
