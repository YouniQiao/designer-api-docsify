...
After a rule with [Action](arkts-mdm-action-e.md) set to **ALLOW** is added, a rule with **Action** set to **DENY** is added by default to discard or intercept all packets for domain name resolution that do not meet the **ALLOW** rule.
...
| domainFilterRule | [DomainFilterRule](arkts-mdm-domainfilterrule-i.md) | Yes | Domain name filtering rule to add. |
