...
| isForce | boolean | Yes | Whether to forcibly enable Wi-Fi. The value **true** means to forcibly enable Wi-Fi. Once enabled, it cannot be disabled manually. You must call [turnOffWifi](arkts-mdm-wifimanager-turnoffwifi-f.md) instead. The value **false** means the opposite and the Wi-Fi can be disabled manually. |
