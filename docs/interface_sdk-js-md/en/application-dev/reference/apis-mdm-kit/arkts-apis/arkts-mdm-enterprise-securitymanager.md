...
| [getAppClipboardPolicy](arkts-mdm-securitymanager-getappclipboardpolicy-f.md) | Obtains the device clipboard policy. |
...
| [getExternalSourceExtensionsPolicy](arkts-mdm-securitymanager-getexternalsourceextensionspolicy-f.md) | Obtains the management policy for extensions from external sources. |
...
| [getPasswordPolicy](arkts-mdm-securitymanager-getpasswordpolicy-f.md) | Obtains the device screen lock password policy. |
...
| [installUserCertificate](arkts-mdm-securitymanager-installusercertificate-f.md) | Installs a user certificate. This API uses a promise to return the result. Enterprises can use this API to install certificates on devices in scenarios such as enterprise VPN connection, security authentication, and digital signatures, implementing enterprise-level secure communication and data protection. |
...
| [setWatermarkImage](arkts-mdm-securitymanager-setwatermarkimage-f.md) | Sets a watermark policy for a specified application of a specified user. Currently, a maximum of 100 policies can be saved. |
