...
| mode | [RunningMode](arkts-mdm-adminmanager-runningmode-e-sys.md) | Yes | Running mode. The value **DEFAULT** means the application runs under the default user (user after the first device power- on). The value **MULTI_USER** means the application runs under multiple users at the same time. |
