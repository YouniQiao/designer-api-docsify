...
动画插值起点。 **说明:** 会影响[onFrame](../arkts-components/arkts-arkui-imageframeinfo-i.md#ImageFrameInfo)回调的入参值。 默认值：0
...
动画插值终点。 **说明:** 会影响[onFrame](../arkts-components/arkts-arkui-imageframeinfo-i.md#ImageFrameInfo)回调的入参值。 默认值：1
...
动画播放次数。设置为0时不播放，设置为-1时无限次播放，设置大于0时为播放次数。 **说明:** 设置为除-1外其他负数视为无效取值，无效取值动画默认播放1次。
