...
| 参数名 | 类型 | 必填 |
| --- | --- | --- |
| value | [ResourceStr](../arkts-apis/arkts-arkui-resourcestr-t.md) \| [PixelMap](../../apis-na/arkts-apis/arkts-na-pixelmap-t.md) | 是 | 图片的数据源，支持本地图片和网络图片。 <br>使用网络图片时，需要申请权限ohos.permission.INTERNET。具体申请方式请参考 [声明权限](../../../security/AccessToken/declare-permissions.md)。 <br>当使用相对路径引用图片资源时，例如`ImageSpan("common/test.jpg")`，不支持跨包/跨模块调用该ImageSpan组件，建议使用`\$r`方式来管理需全局使用的图片资源。 <br>- 支持的图片格式包括png、jpg、bmp、svg、gif、webp和heif。 <br>- 支持`Base64`字符串。格式`data:image/[png\|jpeg\|bmp\|webp\|
