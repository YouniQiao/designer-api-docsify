...
Video组件用于播放视频文件并控制其播放状态，支持播放、暂停、进度控制、倍速播放、全屏切换等功能。 > **说明：** > > 该组件从API version 7开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。 > <br> > > Video组件只提供简单的视频播放功能，无法支撑复杂的视频播控场景。复杂开发场景推荐使用[AVPlayer](../../apis-media-kit/arkts-apis/arkts-media-media-avplayer-i.md#avplayer)播控API和 > XComponent组件开发。 > <br> > > Video组件在使用[expandSafeArea](arkts-arkui-commonmethod-c.md#expandsafearea)扩展安全区域时，组件视频显示内容区域不支持扩展。
...
| 参数名 | 类型 | 必填 |
| --- | --- | --- |
| value | [VideoOptions](arkts-arkui-videooptions-i.md) | 是 |
