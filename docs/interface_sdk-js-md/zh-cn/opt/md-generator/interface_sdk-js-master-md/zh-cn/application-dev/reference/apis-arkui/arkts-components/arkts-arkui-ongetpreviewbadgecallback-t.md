...
即将启动多选长按聚拢动画时，触发用于获取选中数量的回调。 返回true表示显示选中数量角标，对应Grid或List显示范围内选中item数量；false表示不显示角标。 返回数字时默认显示角标，该数字表示角标中需要显示的数量。取值范围：[0, 2&lt;sup&gt;31&lt;/sup&gt;-1]，超过取值范围时按返回true处理。 返回浮点数时，向下取整。
