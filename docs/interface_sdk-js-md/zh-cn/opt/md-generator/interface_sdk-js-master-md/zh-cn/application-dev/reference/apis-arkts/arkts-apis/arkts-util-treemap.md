...
TreeMap可用于存储具有关联关系的key-value键值对集合，存储元素中key值唯一，每个key对应一个value。
 ...
 TreeMap和[HashMap](arkts-arkts-util-hashmap-hashmap-c.md#HashMap)相比，HashMap依据键的hashCode存取数据，访问速度较快但不保证键的顺序；而TreeMap基于红黑树有序存取，访问效率较低，但支持有序遍历、范围查询和首末键及相邻键查找等有序操作。
