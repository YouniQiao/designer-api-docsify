...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| data | [DataBlob](../../apis-device-certificate-kit/arkts-apis/arkts-devicecertificate-datablob-i.md) | 是 | 表示最终要加密或解密的数据。 |
| callback | [AsyncCallback](../../apis-basic-service-kit/arkts-apis/arkts-basicservices-asynccallback-i.md)<DataBlob> | 是 | 回调函数。当加/解密成功时，err为undefined，data为加/解密结果DataBlob；否则为错误对象。 |
...
> **说明：**  
>  
> 1.对称加解密中，调用**doFinal**标志着一次加解密流程已经完成，即[Cipher](arkts-cryptoarchitecture-cipher-i.md)实例的状态被清除，  
...
- 对于GCM和CCM模式的对称加密：一次加密流程中，如果将每一次**update**和**doFinal**的结果拼接起来，会得到“密文+authTag”，即末尾的16字节（GCM模式）或12字节（CCM模式）是authTag，而其余部分均为密文。（也就是说，如果**doFinal**的data参数传入null，则**doFinal**的结果就是authTag）authTag需要填入解密时的[GcmParamsSpec](arkts-cryptoarchitecture-gcmparamsspec-i.md)或[CcmParamsSpec](arkts-cryptoarchitecture-ccmparamsspec-i.md)；密文则作为解密时的入参data。  
...
关于其他注意事项，请参见[doFinal()](arkts-cryptoarchitecture-cipher-i.md#dofinal-2)中的**说明：**。
...
| 类型 | 说明 |
| --- | --- |
| [DataBlob](../../apis-device-certificate-kit/arkts-apis/arkts-devicecertificate-datablob-i.md) | 加密或解密后的数据。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| itemType | [CipherSpecItem](arkts-cryptoarchitecture-cipherspecitem-e.md) | 是 | 用于指定需要获取的加解密参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| opMode | [CryptoMode](arkts-cryptoarchitecture-cryptomode-e.md) | 是 | 要执行的操作（加密或解密） |
| key | [Key](../../apis-input-kit/arkts-apis/arkts-input-key-i.md) | 是 | 用于加密或解密的密钥 |
| params | [ParamsSpec](arkts-cryptoarchitecture-paramsspec-i.md) | 是 | IV等算法参数 |
| callback | [AsyncCallback](../../apis-basic-service-kit/arkts-apis/arkts-basicservices-asynccallback-i.md)<void> | 是 | 回调函数。当加解密初始化成功时，err为undefined；否则为错误对象。 |
...
初始化加解密的[cipher](arkts-cryptoarchitecture-cipher-i.md)对象，使用callback异步回调获取结果。

init、update、doFinal为三段式接口，需要成组使用。其中init和doFinal必选，update可选。

必须在使用[createCipher](arkts-cryptoarchitecture-createcipher-f.md#createcipher-1)创建[Cipher](arkts-cryptoarchitecture-cipher-i.md)实例后，才能使用本函数。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| opMode | [CryptoMode](arkts-cryptoarchitecture-cryptomode-e.md) | 是 | 加密或者解密模式。 |
| key | [Key](../../apis-input-kit/arkts-apis/arkts-input-key-i.md) | 是 | 指定加密或解密的密钥。 |
| params | ParamsSpec \| null | 是 | 指定加密或解密的参数，对于ECB等没有参数的算法模式，请传入null。API 10之前只支持ParamsSpec， API 10之后增加支持null。 |
| callback | [AsyncCallback](../../apis-basic-service-kit/arkts-apis/arkts-basicservices-asynccallback-i.md)<void> | 是 | 回调函数。当加解密初始化成功，err为undefined，否则为错误对象。 |
...
初始化加解密的[cipher](arkts-cryptoarchitecture-cipher-i.md)对象，此API以同步方式返回结果。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| itemType | [CipherSpecItem](arkts-cryptoarchitecture-cipherspecitem-e.md) | 是 | 用于指定需要设置的加解密参数。 |
| itemValue | [Uint8Array](../../apis-arkts/arkts-apis/arkts-arkts-uint8array-c.md) | 是 | 用于指定加解密参数的具体值。 |
...
必须在对[Cipher](arkts-cryptoarchitecture-cipher-i.md)实例使用[init()](arkts-cryptoarchitecture-cipher-i.md#init-4)初始化后，才能使用本函数。
...
> 最后**doFinal**的时候，会把剩下的还没加/解密的数据，根据[createCipher](arkts-cryptoarchitecture-createcipher-f.md#createcipher-1)时设置的  
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| data | [DataBlob](../../apis-device-certificate-kit/arkts-apis/arkts-devicecertificate-datablob-i.md) | 是 | 需要进行加密或解密的数据。data不能为null。 |
| callback | [AsyncCallback](../../apis-basic-service-kit/arkts-apis/arkts-basicservices-asynccallback-i.md)<DataBlob> | 是 | 回调函数。当更新加/解密数据成功时，err为undefined，data为加密或解密结果DataBlob；否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| data | [DataBlob](../../apis-device-certificate-kit/arkts-apis/arkts-devicecertificate-datablob-i.md) | 是 | 加密或者解密的数据。data不能为null。 |
...
必须在对[Cipher](arkts-cryptoarchitecture-cipher-i.md)实例使用[initSync()](arkts-cryptoarchitecture-cipher-i.md#initsync-1)初始化后，才能使用本函数。
...
| 类型 | 说明 |
| --- | --- |
| [DataBlob](../../apis-device-certificate-kit/arkts-apis/arkts-devicecertificate-datablob-i.md) | 返回此次更新的加/解密结果DataBlob。 |
