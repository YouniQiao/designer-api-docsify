...
list.add(5);
list.add(7);
let listCb: ListForEachCb<int> = (value: int, index: int, list: List<int>) => {
  console.info("value: " + value, " index = " + index);
}
list.forEach(listCb);
```
...
参见 [removeByIndex](#removebyindex)
...
list.add(5);
list.add(7);
let listCb: ListReplaceCb<int> = (value: int, index: int, list: List<int>): int => {
