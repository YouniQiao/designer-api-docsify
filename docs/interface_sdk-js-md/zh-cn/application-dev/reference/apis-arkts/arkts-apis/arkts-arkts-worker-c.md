...
| listener | [EventListener](arkts-arkts-eventlistener-t.md) | 否 | listener 要移除的事件监听的回调函数。 |
...
| transfer | [ArrayBuffer](arkts-arkts-arraybuffer-c.md)[] | 是 | transfer 可转移的ArrayBuffer实例对象。transferList数组不可包含null。 |
...
| --- | --- | --- | --- |
| message | Object | 是 | 发送至Worker的数据。 |
| options | [PostMessageOptions](arkts-arkts-postmessageoptions-i.md) | 否 | 可为postMessage设置的选项。transferList数组不可包含null。 |
