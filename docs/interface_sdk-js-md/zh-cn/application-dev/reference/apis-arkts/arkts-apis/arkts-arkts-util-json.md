...
| [parseSendable](arkts-arkts-json-parsesendable-f.md) | 解析JSON字符串，生成可直接跨并发实例（Worker或TaskPool）传递、无需拷贝的Sendable对象图。当解析后的JSON数据需要跨线程共享时，使用本接口替代[parse](arkts-arkts-json-parse-f.md)：解析结果直接创建于共享堆，调用返回后即可被各并发实例访问。 |
