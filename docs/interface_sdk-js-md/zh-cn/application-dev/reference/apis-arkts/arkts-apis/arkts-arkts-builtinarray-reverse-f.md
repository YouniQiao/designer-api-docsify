# reverse

## 导入模块

```TypeScript
```

## reverse

```TypeScript
export function reverse(self: FixedArray<boolean>): FixedArray<boolean>
```

原地反转数组。数组的第一个元素变为最后一个，最后一个元素变为 第一个。

**起始版本：** 26.0.0

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为26.0.0。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-unnamed-export function reverse(self: FixedArray<boolean>): FixedArray<boolean>--><!--Device-unnamed-export function reverse(self: FixedArray<boolean>): FixedArray<boolean>-End-->

**系统能力：** SystemCapability.Utils.Lang

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| self | FixedArray&lt;boolean&gt; | 是 | 执行`reverse`操作的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| FixedArray&lt;boolean&gt; | 反转后的数组。 |


## reverse

```TypeScript
export function reverse(self: FixedArray<byte>): FixedArray<byte>
```

原地反转数组。数组的第一个元素变为最后一个，最后一个元素变为 第一个。

**起始版本：** 26.0.0

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为26.0.0。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-unnamed-export function reverse(self: FixedArray<byte>): FixedArray<byte>--><!--Device-unnamed-export function reverse(self: FixedArray<byte>): FixedArray<byte>-End-->

**系统能力：** SystemCapability.Utils.Lang

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| self | FixedArray&lt;byte&gt; | 是 | 执行`reverse`操作的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| FixedArray&lt;byte&gt; | 反转后的数组。 |


## reverse

```TypeScript
export function reverse(self: FixedArray<short>): FixedArray<short>
```

原地反转数组。数组的第一个元素变为最后一个，最后一个元素变为 第一个。

**起始版本：** 26.0.0

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为26.0.0。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-unnamed-export function reverse(self: FixedArray<short>): FixedArray<short>--><!--Device-unnamed-export function reverse(self: FixedArray<short>): FixedArray<short>-End-->

**系统能力：** SystemCapability.Utils.Lang

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| self | FixedArray&lt;short&gt; | 是 | 执行`reverse`操作的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| FixedArray&lt;short&gt; | 反转后的数组。 |


## reverse

```TypeScript
export function reverse(self: FixedArray<int>): FixedArray<int>
```

原地反转数组。数组的第一个元素变为最后一个，最后一个元素变为 第一个。

**起始版本：** 26.0.0

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为26.0.0。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-unnamed-export function reverse(self: FixedArray<int>): FixedArray<int>--><!--Device-unnamed-export function reverse(self: FixedArray<int>): FixedArray<int>-End-->

**系统能力：** SystemCapability.Utils.Lang

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| self | FixedArray&lt;int&gt; | 是 | 执行`reverse`操作的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| FixedArray&lt;int&gt; | 反转后的数组。 |


## reverse

```TypeScript
export function reverse(self: FixedArray<long>): FixedArray<long>
```

原地反转数组。数组的第一个元素变为最后一个，最后一个元素变为 第一个。

**起始版本：** 26.0.0

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为26.0.0。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-unnamed-export function reverse(self: FixedArray<long>): FixedArray<long>--><!--Device-unnamed-export function reverse(self: FixedArray<long>): FixedArray<long>-End-->

**系统能力：** SystemCapability.Utils.Lang

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| self | FixedArray&lt;long&gt; | 是 | 执行`reverse`操作的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| FixedArray&lt;long&gt; | 反转后的数组。 |


## reverse

```TypeScript
export function reverse(self: FixedArray<float>): FixedArray<float>
```

原地反转数组。数组的第一个元素变为最后一个，最后一个元素变为 第一个。

**起始版本：** 26.0.0

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为26.0.0。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-unnamed-export function reverse(self: FixedArray<float>): FixedArray<float>--><!--Device-unnamed-export function reverse(self: FixedArray<float>): FixedArray<float>-End-->

**系统能力：** SystemCapability.Utils.Lang

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| self | FixedArray&lt;float&gt; | 是 | 执行`reverse`操作的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| FixedArray&lt;float&gt; | 反转后的数组。 |


## reverse

```TypeScript
export function reverse(self: FixedArray<double>): FixedArray<double>
```

原地反转数组。数组的第一个元素变为最后一个，最后一个元素变为 第一个。

**起始版本：** 26.0.0

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为26.0.0。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-unnamed-export function reverse(self: FixedArray<double>): FixedArray<double>--><!--Device-unnamed-export function reverse(self: FixedArray<double>): FixedArray<double>-End-->

**系统能力：** SystemCapability.Utils.Lang

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| self | FixedArray&lt;double&gt; | 是 | 执行`reverse`操作的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| FixedArray&lt;double&gt; | 反转后的数组。 |


## reverse

```TypeScript
export function reverse(self: FixedArray<char>): FixedArray<char>
```

原地反转数组。数组的第一个元素变为最后一个，最后一个元素变为 第一个。

**起始版本：** 26.0.0

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为26.0.0。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-unnamed-export function reverse(self: FixedArray<char>): FixedArray<char>--><!--Device-unnamed-export function reverse(self: FixedArray<char>): FixedArray<char>-End-->

**系统能力：** SystemCapability.Utils.Lang

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| self | FixedArray&lt;char&gt; | 是 | 执行`reverse`操作的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| FixedArray&lt;char&gt; | 反转后的数组。 |

