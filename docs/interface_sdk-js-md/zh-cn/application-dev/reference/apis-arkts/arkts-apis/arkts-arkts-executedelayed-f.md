...
| task | [Task](../../apis-basic-service-kit/arkts-apis/arkts-basicservices-task-i.md) | 是 | 需要延时执行的任务。 |
| priority | [Priority](arkts-arkts-priority-e.md) | 否 | 延时执行的任务的优先级，默认值为**taskpool.Priority.MEDIUM**。 |
...
| --- | --- | --- | --- |
| delayTime | number | 是 | 延时时间，单位为ms。delayTime值必须要大于等于0。该值应为整数。<br>单位：毫秒。 |
| task | [GenericsTask](arkts-arkts-genericstask-c.md)<A, R> | 是 | 需要延时执行的泛型任务。 |
| priority | [Priority](arkts-arkts-priority-e.md) | 否 | 延时执行的任务的优先级，默认值为**taskpool.Priority.MEDIUM**。 |
