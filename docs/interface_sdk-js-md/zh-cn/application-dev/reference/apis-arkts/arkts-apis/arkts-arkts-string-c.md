...
anchor()方法创建将字符串嵌入带name属性的&lt;a&gt;元素（&lt;a name="..."&gt;str&lt;/a&gt;）的字符串。
...
at()方法接受一个整数值，返回由指定偏移位置处单个UTF-16码元 构成的新字符串。该方法允许传入正整数和负整数。 负整数表示从字符串最后一个字符开始倒数。
...
big()方法创建将字符串嵌入&lt;big&gt;元素（&lt;big&gt;str&lt;/big&gt;）的字符串， 使字符串以大号字体显示。
...
blink()方法创建将字符串嵌入&lt;blink&gt;元素（&lt;blink&gt;str&lt;/blink&gt;）的字符串， 使字符串以大号字体显示。
...
bold()方法创建将字符串嵌入&lt;b&gt;元素（&lt;b&gt;str&lt;/b&gt;）的字符串， 使字符串以大号字体显示。
...
charCodeAt()方法返回0到65535之间的整数，表示给定索引处的 UTF-16码元。
...
获取该字符串中指定索引处的码点。 与charAt(index)类似，但如果该索引处的字符是 代理对的起始，则返回该码点对应的int 表示；否则 返回该索引处的字符。
...
返回begin与end索引之间完整码点的数量。 范围之外的字符不计入，即使该范围 在代理对中间结束也是如此。
...
对该字符串与另一个字符串进行字典序比较。 如果该字符串排序在另一个字符串之前，则结果小于0； 相等时为0，否则大于0。
...
检查该字符串是否包含指定的字符串。 从指定索引开始查找（fromIndex为负数时等价于fromIndex = 0， fromIndex大于等于长度时表示无匹配）。
...
将给定的StringBuilder与该字符串进行比较。如果此刻 该StringBuilder的内容与该字符串相同，则结果为true， 否则为false。
...
fixed()方法创建将字符串嵌入&lt;tt&gt;元素（&lt;tt&gt;str&lt;/tt&gt;）的字符串， 使字符串以大号字体显示。
...
fontcolor()方法创建将字符串嵌入&lt;font&gt;元素（&lt;font color="..."&gt;str&lt;/font&gt;）的字符串， 使字符串以指定颜色显示。
...
fontsize()方法创建将字符串嵌入&lt;font&gt;元素（&lt;font size="..."&gt;str&lt;/font&gt;）的字符串， 使字符串以指定字号显示。
...
静态方法string.fromCharCode()返回由指定UTF-16码元创建的字符串。
...
静态方法string.fromCharCode()返回由指定的一组UTF-16码元创建的字符串。
...
静态方法string.fromCodePoint()返回由指定的一组码点创建的字符串。
...
获取表示该字符串中begin与end索引之间部分的byte序列， 该范围为 半开区间[begin, end)。
...
获取表示该字符串中begin与end索引之间部分的char序列， 该范围为 半开区间[begin, end)。
...
includes()方法执行区分大小写的查找，判断一个字符串是否 包含在另一个字符串中，并相应地返回true或false。
...
查找某个字符在该字符串中位置大于等于fromIndex处首次出现的位置。 fromIndex为负数时等价于0；fromIndex大于等于长度时表示无匹配。
...
string值的isWellFormed()方法返回一个boolean值，表示该字符串是否包含 孤立代理项。
...
italics()方法创建将字符串嵌入&lt;i&gt;元素（&lt;i&gt;str&lt;/i&gt;）的字符串， 使字符串以大号字体显示。
...
连接指定的字符串数组，并在所有元素之间插入 在每个元素前插入指定前缀、在每个元素后插入指定 后缀，并在所有元素之间插入指定的 分隔符。
...
查找某个字符在该字符串中位置小于等于fromIndex处最后一次出现的位置。 fromIndex大于等于长度时效果相同；fromIndex为负数时表示无匹配。
...
查找另一个字符串在该字符串中最后一次出现的位置。 如果fromIndex为undefined，则从字符串末尾开始查找。 如果fromIndex大于等于长度，则按长度处理，即从末尾开始查找。 如果fromIndex为负数，则返回-1，表示无匹配。
...
link()方法创建将字符串嵌入&lt;a&gt;元素（&lt;a href="..."&gt;str&lt;/a&gt;）的字符串， 用作指向另一个URL的超链接。
...
基于区域设置和选项比较该字符串与另一个字符串。 如果该字符串排序在另一个字符串之前，则结果为-1； 相等时为0，否则为1。
...
string值的normalize()方法返回该字符串的Unicode规范化形式。
...
padEnd()方法使用给定字符串（必要时重复）填充当前字符串， 使结果字符串达到给定长度。 填充从当前字符串的末尾开始。
...
创建指定长度的新字符串，其中 该字符串的开头以指定字符填充。 `padStart`是该方法的别名， 区别仅在于参数顺序。
...
创建指定长度的新字符串，其中 该字符串的末尾以指定字符填充。 `padEnd`是该方法的别名， 区别仅在于参数顺序。
...
padStart()方法使用另一个字符串（必要时重复多次）填充当前字符串， 直到结果字符串达到给定长度。 填充从当前字符串的开头开始。
...
将该字符串重复count次，例如： a = 'A'， a.repeat(2) == 'AA'。
...
将指定字符出现的所有位置替换为另一个 指定字符。如果该字符串中未找到 指定字符，则返回原始 字符串。
...
slice()方法提取字符串中的一段并以新字符串返回， 不修改原始字符串。
...
small()方法创建将字符串嵌入&lt;small&gt;元素（&lt;small&gt;str&lt;/small&gt;）的字符串， 使字符串以大号字体显示。
...
使用分隔符分割该字符串，并返回有序的子串数组。 结果数组的顺序与从头到尾遍历该字符串的 顺序一致。子串中不包含匹配模式本身。 数组长度受指定值限制。
...
检查该字符串中从指定索引开始的子串 是否以指定前缀开头。 `fromIndex`为负数时按0处理。当`prefix`为空时， 结果恒为true。
...
strike()方法创建将字符串嵌入&lt;strike&gt;元素（&lt;strike&gt;str&lt;/strike&gt;）的字符串， 使字符串以大号字体显示。
...
sub()方法创建将字符串嵌入&lt;sub&gt;元素（&lt;sub&gt;str&lt;/sub&gt;）的字符串， 使字符串以大号字体显示。
...
substr()方法从指定索引开始，返回字符串中 指定数量字符所构成的部分。
...
截取该字符串的子串，从指定索引开始， 到另一个指定索引之前结束。 如果'begin'小于0，则按0处理。 如果'begin'大于this.length，则按this.length处理。
...
截取该字符串的子串，从指定索引开始， 到另一个指定索引之前结束。 如果'begin'小于0，则按0处理。 如果'begin'大于this.length，则按this.length处理。 如果'end'小于0，则按0处理。 如果'end'大于this.length，则按this.length处理。 如果'begin'大于'end'，则二者互换。 如果'begin'等于'end'，则返回空字符串。
...
sup()方法创建将字符串嵌入&lt;sup&gt;元素（&lt;sup&gt;str&lt;/sup&gt;）的字符串， 使字符串以大号字体显示。
...
toLocaleLowerCase()方法返回调用方字符串转换为小写后的值， 转换时遵循区域设置相关的大小写映射规则。
...
toLocaleUpperCase()方法返回调用方字符串转换为大写后的值， 转换时遵循区域设置相关的大小写映射规则。
...
toString()方法以原对象副本的形式返回给定字符串的 字符串表示。
...
string值的toWellFormed()方法返回一个字符串，其中该字符串的所有孤立代理项 都被替换为Unicode替换字符U+FFFD。
...
trimEnd()方法去除字符串末尾的空白并返回新字符串， 不修改原始字符串。trimRight()是该方法的别名。
...
trimStart()方法去除字符串开头的空白并返回新字符串， 不修改原始字符串。trimLeft()是该方法的别名。
