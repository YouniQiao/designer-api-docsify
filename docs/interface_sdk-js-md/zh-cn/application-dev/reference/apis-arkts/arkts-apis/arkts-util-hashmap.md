...
HashMap底层采用数组、链表和红黑树实现，支持高效查询、插入和删除。HashMap存储内容基于键值对映射，不允许重复的key，且一个key只能对应一个value。
 HashMap和[TreeMap](arkts-util-treemap.md)相比，HashMap依据键的hashCode（哈希码）存取数据，访问速度较快，但不保证键的有序性。而TreeMap是有序存储和访问，查询效率较低。
 [HashSet](arkts-util-hashset.md)基于HashMap实现。HashMap的输入参数由key、value两个值组成。在HashSet中，只对value对象进行存储和管理。
