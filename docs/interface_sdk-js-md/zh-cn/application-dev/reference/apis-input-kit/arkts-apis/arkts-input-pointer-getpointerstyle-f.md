...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| windowId | ArkTS-Dyn: number  <br>ArkTS-Sta：int | 是 | 窗口ID。取值范围为大于等于-1的整数，取值为-1时表示全局窗口。&lt;br&gt;窗口ID合法并且对应窗口存在时，返回窗口的鼠标光标样式。&lt;br&gt;窗口ID合法但窗口不存在时，默认返回全局 鼠标光标样式。&lt;br&gt;如果通过[setPointerStyle](arkts-input-pointer-setpointerstyle-f.md#setpointerstyle)接口为不存 在的窗口设置了鼠标光标样式，使用本接口可以正常获取到该光标样式。 |
