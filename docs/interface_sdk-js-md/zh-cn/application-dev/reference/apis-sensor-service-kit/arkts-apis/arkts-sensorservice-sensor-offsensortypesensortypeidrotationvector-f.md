...
取消订阅旋转矢量传感器数据。off取消订阅必须与on订阅成对出现。 > **说明：** > > 从API version 8 开始支持，从API version 9 开始废弃，建议使用[sensor.on.ROTATION_VECTOR] > [off](arkts-sensorservice-sensor-offsensoridcolor-f-sys.md#off_SensorId.COLOR) > 替代。
