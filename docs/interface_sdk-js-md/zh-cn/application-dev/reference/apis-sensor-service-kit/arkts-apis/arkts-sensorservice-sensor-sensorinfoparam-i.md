...
指定目标传感器所属设备的ID。默认值：-1（表示本地设备）。可通过sensor.on('sensorStatusChange')或 [getSensorList](arkts-sensorservice-sensor-getsensorlist-f.md)获取远程设备ID。
...
指定目标传感器的索引，同一类型传感器可能有多个实例。默认值：0（表示设备上的默认传感器）。其它传感器索引需通过 [getSensorList](arkts-sensorservice-sensor-getsensorlist-f.md)或 sensor.on('sensorStatusChange')获取。
