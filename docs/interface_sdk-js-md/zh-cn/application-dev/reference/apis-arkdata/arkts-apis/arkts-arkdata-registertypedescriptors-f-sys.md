...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| typeDescriptors | [Array](../../apis-arkts/arkts-apis/arkts-arkts-array-c.md)<TypeDescriptor> | 是 | 待注册的标准化数据类型描述符列表。TypeDescriptor用于描述自定义数据类型的属性，包括类型标识、所属类型、文件扩展名、MIME类型等。列表不可为空，其中元素个数不超过50。单个应用注册的标准化数据类型描述符数量总计不超过200。 |
