...
在使用以下API前，请先通过[getRdbStore]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_方法获取RdbStore实例，并使用该实例调用对应接口方法。

在此基础上，建议优先使用[execute]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_方法完成数据库表结构和初始数据的初始化，以确保相关接口调用的前置条件已满足。
...
| cursor | ArkTS-Dyn: number  \_\_\_HTML\_TAG\_USD\_0\_\_\_ArkTS-Sta：long | 否 | 表示数据游标，不大于此游标的脏数据将被清理。整数类型，取值应大于0。当传入小于等于0的值时，会抛出异常，异常信息为无效的参数。当此参数不填时，清理当前表的所有脏数据。 |
...
手动执行按条件进行端云同步，使用callback异步回调。使用该接口需要实现云同步功能。
    **说明：**  
    ...
    从API version 18开始，手动执行端云同步时，设置谓词条件时新增支持指定资产下载能力。此时，同步模式需要设置为\_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_。  
    
    谓词中支持使用主键（必填）和资产（可选）作为同步条件：选择资产作为同步条件时，谓词仅支持[equalTo]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_；指定资产的数量较多时（最  
...
| predicates | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 表示同步数据的谓词条件。 |
...
| 类型 | 说明 |
| --- | --- |
| ArkTS-Dyn: Promise&lt;number&gt;  \_\_\_HTML\_TAG\_USD\_0\_\_\_ArkTS-Sta：Promise&lt;int&gt; | Promise对象。如果加锁成功，返回锁的有效时长；如果加锁失败，返回0，单位：ms。 |
...
[getValue]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_、[getString]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_等get方法时将无法成功获取数据，并可能导致操作失败或抛出异常。
...
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;ResultSet&gt; | 是 | 回调函数。返回ResultSet对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| predicates | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 表示查询的谓词条件。 |
...
| --- | --- | --- | --- |
| predicates | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 表示查询的谓词条件。 |
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;ResultSet&gt; | 是 | 回调函数。返回查询的结果集。 |
...
从副本关系型数据库文件恢复数据库，使用Promise异步回调。此接口仅供[HAMode]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_为MAIN\_REPLICA时使用，且不支持在事务中使用。
...
保留对应\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_分布式数据表中对应设备同步过来的数据，删除其他设备同步过来的数据，使用Promise异步回调。

不支持对\_\_\_MD\_LINK\_DESC\_USD\_1\_\_\_分布式数据表进行删除。
...
| callback | ArkTS-Dyn: \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;number&gt;  \_\_\_HTML\_TAG\_USD\_2\_\_\_ArkTS-Sta：\_\_\_MD\_LINK\_USD\_1\_\_\_&lt;long&gt; | 是 | 回调函数。返回受影响的行数。 |
...
要更新数据越多，执行所需的时间越长。
    **说明：**  
    ...
    入参predicates中若要传入[ORIGIN\_ORIDEVICE]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_，则只允许使用等于空或不等于空。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 指定要更新的分布式表的日志信息。 |
...
| 类型 | 说明 |
| --- | --- |
| ArkTS-Dyn: Promise&lt;number&gt;  \_\_\_HTML\_TAG\_USD\_0\_\_\_ArkTS-Sta：Promise&lt;long&gt; | Promise对象。返回更新的数据个数。 |
