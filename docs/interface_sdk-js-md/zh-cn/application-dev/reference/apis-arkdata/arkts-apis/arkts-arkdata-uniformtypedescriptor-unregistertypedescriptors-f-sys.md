...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| typeIds | Array&lt;string&gt; | 是 | 待注销的typeId列表。列表不可为空，其中元素个数不超过50。每项长度不超过127。<br> **typeId约束限制：**<br>1.typeId对应的标准化数据类型必须在系统中已注册；<br> 2.typeId必须以当前应用的包名开头；<br> 3.typeId对应的标准化数据类型必须已通过registerTypeDescriptors接口注册。 |
