...
单版本数据库，继承自[KVStore]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_数据库，提供查询数据和同步数据的方法。单版本数据库，不对数据所属设备进行区分，不同设备使用相同键写入数据会互相覆盖。比如，可以使用单版本数据库实现个人日历、联系人数据在不同设备间的数据同步。在调用SingleKVStore的方法前，需要先通过  
[getKVStore]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_构建一个SingleKVStore实例。
...
关闭由  
[SingleKVStore.getResultSet]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_返回的KvStoreResultSet对象，使用callback异步回调。
...
关闭由  
[SingleKVStore.getResultSet]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_返回的KvStoreResultSet对象，使用Promise异步回调。
