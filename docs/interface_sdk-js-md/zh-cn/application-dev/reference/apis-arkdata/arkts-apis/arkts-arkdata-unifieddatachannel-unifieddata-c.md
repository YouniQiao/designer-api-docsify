...
将当前统一数据对象中的所有数据记录取出。通过本接口取出的数据为UnifiedRecord类型，需通过[getType]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_获取数据类型后转为子类再使用。
...
检查当前统一数据对象中是否有指定的数据类型，检查范围包括使用[addEntry]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_函数添加的数据类型。
