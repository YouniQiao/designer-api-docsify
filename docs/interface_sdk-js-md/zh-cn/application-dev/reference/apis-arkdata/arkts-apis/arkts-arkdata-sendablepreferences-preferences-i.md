...
Preferences继承自\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_，可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，提供获取和修改存储数据的接口。

下列接口都需先使用[sendablePreferences.getPreferences]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_获取到Preferences实例，再通过此实例调用对应接口。
...
本接口提供给申请了[dataGroupId]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_的应用进行使用，未申请的应用不推荐使用，多进程操作可能会损坏持久化文件，导致数据丢失。
...
订阅数据变更，订阅的Key的值发生变更后，在执行flush方法后，触发callback回调。
    ...
    **选取建议：** 需要监听所有数据变更时使用on('change')；需要精确知道特定Key变化并获取新值时使用on('dataChange')。  
    ...
    当调用[removePreferencesFromCache]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_或者  
    [deletePreferences]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_后，订阅的数据变更会主动取消订阅，在重新  
    [getPreferences]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_后需要重新订阅数据变更。
...
订阅进程间数据变更，多个进程持有同一个首选项文件时，在任意一个进程（包括本进程）执行[flush]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_方法，持久化文件发生变更后，触发callback回调。

本接口提供给申请了[dataGroupId]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_的应用进行使用，未申请的应用不推荐使用，多进程操作可能会损坏持久化文件，导致数据丢失。
    **说明：**  
    
    同一持久化文件在当前进程订阅进程间数据变更的最大数量为50次，超过最大限制后会订阅失败。建议在触发callback回调后及时取消订阅。  
    
    当调用[removePreferencesFromCache]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_或者  
...
精确订阅数据变更，只有被订阅的Key值发生变更后，在执行[flush]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_方法后，触发callback回调。
    **说明：**  
    ...
    当调用[removePreferencesFromCache]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_或者  
