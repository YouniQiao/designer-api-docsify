...
对应沙箱路径下无数据库文件时，将创建数据库文件，文件创建位置详见[StoreConfig]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_。对应路径下已有数据库文件时，将打开已有数据库文件。

开发者在创建数据库时，应谨慎配置是否进行数据库加密的参数[encrypt]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_，数据库创建后，禁止对该参数进行修改。
