...
- [KVManager](arkts-arkdata-distributeddata-kvmanagerconfig-i.md#KVManagerConfig)：数据管理实例，用于获取KVStore的相关信息。  
- [KvStoreResultSet&lt;sup&gt;8+&lt;/sup&gt;](arkts-arkdata-distributeddata-kvstoreresultset-i.md#KvStoreResultSet)：提供获取KVStore数据库结果集的相关方法，包括查询和移动数据读取位置等。  
- [Query&lt;sup&gt;8+&lt;/sup&gt;](arkts-arkdata-distributeddata-query-c.md#Query)：使用谓词表示数据库查询，提供创建Query实例、查询数据库中的数据和添加谓词的方法。  
- [KVStore](arkts-arkdata-distributeddata-kvstoretype-e.md#KVStoreType)：KVStore数据库实例，提供增加数据、删除数据和订阅数据变更、订阅数据同步完成的方法。  
- [SingleKVStore](arkts-arkdata-distributeddata-singlekvstore-i.md#SingleKVStore)：单版本分布式数据库，继承自[KVStore](arkts-arkdata-distributeddata-kvstoretype-e.md#KVStoreType)，不对数据  
所属设备进行区分，提供查询数据和同步数据的方法。  
- [DeviceKVStore&lt;sup&gt;8+&lt;/sup&gt;](arkts-arkdata-distributeddata-devicekvstore-i.md#DeviceKVStore)：设备协同数据库，继承自  
[KVStore](arkts-arkdata-distributeddata-kvstoretype-e.md#KVStoreType)，以设备维度对数据进行区分，提供查询数据和同步数据的方法。
...
**替代接口：** [distributedKVStore](arkts-data-distributedkvstore.md#distributedKVStore)
...
| [SingleKVStore](arkts-arkdata-distributeddata-singlekvstore-i.md) | 单版本数据库，继承自[KVStore](arkts-arkdata-distributeddata-kvstoretype-e.md#KVStoreType)数据库，提供查询数据和同步数据的方法。单版本数据库，不对数据所属设备进行区分，不同设备使用相同键写入数据会互相覆盖。比如，可以使用单版本数据库实现个人日历、联系人数据在不同设备间的数据同步。在调用SingleKVStore的方法前，需要先通过  [getKVStore](distributedData.KVManager.getKVStore&lt;T extends KVStore&gt;(storeId: string, options: Options, callback: AsyncCallback&lt;T&gt;))构建一个SingleKVStore实例。 |
