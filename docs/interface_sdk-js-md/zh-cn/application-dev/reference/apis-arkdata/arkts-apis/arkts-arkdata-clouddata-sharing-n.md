...
| [allocResourceAndShare](arkts-arkdata-sharing-allocresourceandshare-f-sys.md#allocresourceandshare-2) | 根据谓词条件匹配的数据申请共享资源标识并发起共享，返回已共享资源的结果集。并根据指定的列字段，返回的结果集中同时包含对应列的字段值，使用callback异步回调。 |
...
| [unshare](arkts-arkdata-sharing-unshare-f-sys.md#unshare-1) | 根据指定的共享资源标识和共享参与者取消共享，使用Promise异步回调。 |
