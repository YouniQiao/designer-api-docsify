...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 作为延迟数据类型的标识，用于区分不同类型的数据。取值见 [UniformDataType]\_\_\_JSDOC\_LINK\_USD\_0\_\_\_， 如'general.plain-text'表示纯文本类型。  |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| \_\_\_MD\_LINK\_USD\_0\_\_\_ | 当延迟回调触发时，返回包含相应类型数据的UnifiedData对象，可用于跨应用数据共享和传输。  |
