...
移除规则后如果不存在[Action](arkts-mdm-action-e.md)为ALLOW规则后，会将[addDomainFilterRule](arkts-mdm-adddomainfilterrule-f.md#adddomainfilterrule-1)添加的默认DENY规则清空。
...
| domainFilterRule | [DomainFilterRule](arkts-mdm-domainfilterrule-i.md) | 否 | 移除域名过滤规则。值为空时，清空所有的域名规则。 |
