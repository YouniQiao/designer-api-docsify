...
移除域名过滤规则时非必填，当值为空时，表示清空所有的匹配[Action](arkts-mdm-action-e.md)规则的链，且domainName，appUid也必须传入空值。
...
移除域名过滤规则时非必填，当值为空时，表示清空所有的[Direction](arkts-mdm-direction-e.md)链，且domainName，appUid也必须传入空值。
