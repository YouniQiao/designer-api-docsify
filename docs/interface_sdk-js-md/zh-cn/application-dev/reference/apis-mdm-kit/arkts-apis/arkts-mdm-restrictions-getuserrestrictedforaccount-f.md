...
**替代接口：** [getUserRestrictedForAccount](arkts-mdm-restrictions-getuserrestrictedforaccount-f.md)(admin: Want | null, settingsItem: SettingsForAccount, accountId: number)
...
| accountId | number | 是 | 用户ID，取值范围：大于等于0。<br>accountId可以通过[getOsAccountLocalId](../../apis-basic-services-kit/arkts-apis/arkts-basicservices-osaccount-accountmanager-i.md#getosaccountlocalid)等接口来获取。<br>取值限定为整数。 |
...
| accountId | number | 是 | 用户ID，取值范围：大于等于0。<br>取值限定为整数。<br>accountId可以通过[getOsAccountLocalId](../../apis-basic-services-kit/arkts-apis/arkts-basicservices-osaccount-accountmanager-i.md#getosaccountlocalid)等接口来获取。 |
