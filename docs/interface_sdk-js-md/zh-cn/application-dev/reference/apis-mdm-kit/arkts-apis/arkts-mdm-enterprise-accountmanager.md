...
| [getDomainAccountPolicy](arkts-mdm-accountmanager-getdomainaccountpolicy-f.md) | 获取域账号策略。适用于企业管理场景，如查询当前域账号策略配置、策略合规性审计等。 |
| [isOsAccountAdditionDisallowed](arkts-mdm-accountmanager-isosaccountadditiondisallowed-f.md) | 查询是否禁止用户添加账号。适用于企业审计和合规检查场景，帮助管理员确认账号策略执行情况。 |
...
| [removeOsAccount](arkts-mdm-accountmanager-removeosaccount-f.md) | 移除系统账号。当前仅支持手机、平板设备使用，可以移除使用[createNormalOsAccount](arkts-mdm-accountmanager-createnormalosaccount-f.md)创建的普通系统账号（normal类型）和[addOsAccountAsync](arkts-mdm-accountmanager-addosaccountasync-f.md)创建的系统账号（admin、normal、guest类型），不可移除默认系统账号（ID为100）。 |
| [setDomainAccountPolicy](arkts-mdm-accountmanager-setdomainaccountpolicy-f.md) | 设置域账号策略。 |
...
| 名称 | 说明 |
| --- | --- |
| [addOsAccount](arkts-mdm-accountmanager-addosaccount-f-sys.md) | 后台添加账号。 |
| [disallowAddLocalAccount](arkts-mdm-accountmanager-disallowaddlocalaccount-f-sys.md) | 禁止设备创建本地账号。使用callback异步回调。 |
...
| [disallowAddOsAccountByUser](arkts-mdm-accountmanager-disallowaddosaccountbyuser-f-sys.md) | 禁止用户添加账号。 |
| [isAddOsAccountByUserDisallowed](arkts-mdm-accountmanager-isaddosaccountbyuserdisallowed-f-sys.md) | 查询是否禁止某用户添加账号。 |
<!--DelEnd-->
