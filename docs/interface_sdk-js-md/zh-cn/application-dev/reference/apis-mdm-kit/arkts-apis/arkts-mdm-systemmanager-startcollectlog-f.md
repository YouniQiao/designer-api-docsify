...
开始收集设备上已生成并存储至硬盘的[FaultType]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_类型的faultlog日志，不支持收集未存储至硬盘的faultlog日志、应用业务日志和系统运行日志。
...
- 任务执行完成后，通过  
[EnterpriseAdminExtensionAbility.onLogCollected]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_回调函数通知给MDM应用，系统将已收集的日志文件挂载到MDM应用沙箱路径，MDM应用可以在回调函数中读取已收集的日志。  
- 如果日志收集任务执行超过5分钟，  
[EnterpriseAdminExtensionAbility.onLogCollected]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_回调函数会返回日志收集任务失败。  
- 应用取走日志后，建议调用[systemManager.finishLogCollected]\_\_\_JSDOC\_LINK\_DESC\_USD\_3\_\_\_删除已收集到的日志。
