...
- 上传时，data是表单项数组Array<[FormItem](arkts-basicservices-formitem-i.md)>。从API version15开始，创建单个任务可以上传最多100个文件。默认为空。
...
超时时间自定义设置，连接超时时间默认60秒，总超时时间默认604800秒（1周）。当retry参数为true时，[timeout](arkts-basicservices-timeout-i.md)事件会触发立即重试，导致[timeout](arkts-basicservices-timeout-i.md)在外部观察中被重试动作所掩盖，但内部[timeout](arkts-basicservices-timeout-i.md)条件已实际触发。若需显性观察[timeout](arkts-basicservices-timeout-i.md)事件，需关闭retry参数。
...
任务令牌。查询带有token的任务需提供token并通过[request.agent.touch](arkts-basicservices-touch-f.md#touch-1)查询，否则无法查询到指定任务。其最小为8个字节，最大为2048个字节。默认为空。
