...
- factoryReset（普通恢复出厂，用于清除用户数据分区。详见\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_）。  
- forceFactoryReset（强制恢复出厂，用于清除用户数据分区并同步清除文件密钥。详见\_\_\_MD\_LINK\_DESC\_USD\_1\_\_\_）。  
- deepFactoryReset（深度恢复出厂，用于通过scope参数指定清除范围：DATA仅清除用户数据分区，DATA\_AND\_OS同时清除用户数据和操作系统分区。详见\_\_\_MD\_LINK\_DESC\_USD\_2\_\_\_）。
...
- 调用factoryReset，deepFactoryReset和getDeepFactoryResetInfo接口时，需要权限ohos.permission.FACTORY\_RESET。  
- 调用forceFactoryReset接口时，需要权限ohos.permission.FORCE\_FACTORY\_RESET。  
...
| 类型 | 说明 |
| --- | --- |
| \_\_\_MD\_LINK\_USD\_0\_\_\_ | 用于执行恢复出厂设置相关操作的工具类对象。 |
