...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| pipe | [USBDevicePipe](arkts-basicservices-usbdevicepipe-i.md) | 是 | 用于确定设备，需要调用[usbManager.connectDevice](arkts-basicservices-connectdevice-f.md#connectdevice-1)获取。 |
| endpoint | [USBEndpoint](arkts-basicservices-usbendpoint-i.md) | 是 | 用于确定传输的端口，需要调用[usbManager.getDevices](arkts-basicservices-getdevices-f.md#getdevices-1)获取设备信息列表以及endpoint，address用于确定端点地址，direction用于确定端点的方向，interfaceId用于确定所属接口，当前其他属性不做处理。 |
| buffer | [Uint8Array](../../apis-arkts/arkts-apis/arkts-arkts-uint8array-c.md) | 是 | 用于写入或读取数据的缓冲区。 |
