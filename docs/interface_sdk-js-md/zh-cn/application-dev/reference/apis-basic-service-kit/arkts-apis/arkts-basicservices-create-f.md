...
| type | [RunningLockType](arkts-basicservices-runninglocktype-e.md) | 是 | 要创建的锁的类型；该参数必须是一个枚举类。 |
| callback | [AsyncCallback](arkts-basicservices-asynccallback-i.md)<RunningLock> | 是 | 回调函数。当创建锁成功，err为undefined，data为创建的RunningLock；否则为错误对象；AsyncCallback封装了一个RunningLock类型的类。 |
