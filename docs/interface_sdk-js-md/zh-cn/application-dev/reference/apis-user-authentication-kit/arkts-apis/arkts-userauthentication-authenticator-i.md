...
| callback | AsyncCallback&lt;number&gt; | 是 | 回调函数。number表示认证结果，参见[AuthenticationResult](arkts-userauthentication-authenticationresult-e.md#authenticationresult)。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;number&gt; | 返回携带一个number的Promise。number 为认证结果，参见[AuthenticationResult](arkts-userauthentication-authenticationresult-e.md#authenticationresult)。 |
