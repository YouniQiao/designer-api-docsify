...
获取[UserAuthInstance]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_对象，执行用户身份认证，并支持使用统一用户身份认证控件。该接口用于创建一个用户认证实例，配置认证参数和界面参数后，可通过返回的实例对象启动认证、订阅认证结果等。
    **说明：**  
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| authParam | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 用户认证相关参数。包含挑战值、认证类型列表、认证可信等级、认证结果复用配置等。挑战值建议使用加解密算法库框架生成的随机数，认证类型可指定多种供用户选择，认证可信等级 应根据业务场景安全需求选择。 |
...
| 类型 | 说明 |
| --- | --- |
| \_\_\_MD\_LINK\_USD\_0\_\_\_ | 支持用户界面的认证器对象。获取实例后，需调用 [on('result')]{ |
