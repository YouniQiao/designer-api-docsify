...
当电池电量低于设备设置的低电量百分比值时，将会触发事件通知服务发布该系统公共事件。<!--Del-->设备设置低电量百分比值请参考 电量等级定制开发指导。<!--DelEnd-->
...
当设备热等级变化时，将会触发事件通知服务发布该系统公共事件。<!--Del-->设备热等级配置请参考 热等级定制开发指导。<!--DelEnd-->
...
与这个公共事件相关的接口：activateOsAccount, 为系统API，具体参看[@ohos.account.osAccount](arkts-account-osaccount.md)。

要订阅此事件，在API version 21之前，需要申请ohos.permission.MANAGE_LOCAL_ACCOUNTS权限；从API version 21开始，需要申请 ohos.permission.MANAGE_LOCAL_ACCOUNTS或ohos.permission.INTERACT_ACROSS_LOCAL_ACCOUNTS权限
...
与这个公共事件相关的接口：setOsAccountDistributedInfo、updateOsAccountDistributedInfo(已废弃)，这些为公共API， setOsAccountDistributedInfoByLocalId为系统API，具体参看分布式账号接口文档。
...
与这个公共事件相关的接口：createOsAccount、createOsAccountForDomain, 这些为系统API，具体参看 [@ohos.account.osAccount](arkts-account-osaccount.md)。
...
与这个公共事件相关的接口：removeOsAccount, 为系统API，具体参看[@ohos.account.osAccount](arkts-account-osaccount.md)。
...
与这个公共事件相关的接口：setOsAccountName、setOsAccountProfilePhoto, 这些为系统API，setOsAccountDistributedInfo为公共API，具体参看 [系统账号接口文档](arkts-account-osaccount.md)、 分布式账号接口文档。
...
> 具体枚举值及其对应的连接状态如下表所示：
> 
> | 枚举值 | 连接状态 | &gt; | ------ | ---------- | &gt; | 2 | 连接中 | &gt; | 3 | 已连接 | &gt; | 4 | 正在断开 | &gt; | 5 | 已断开 |。
