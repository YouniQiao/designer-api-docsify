...
| callback | (wallpaperType: WallpaperType, resourceType: WallpaperResourceType, uri?: string) =&gt; void | 否 | 表示要取消的壁纸变化回调，不填写该参数则取消订阅该type对应的所有回调。   - wallpaperType：壁纸类型。   - resourceType：壁纸资源类型。   - uri：壁纸资源地址。 |
