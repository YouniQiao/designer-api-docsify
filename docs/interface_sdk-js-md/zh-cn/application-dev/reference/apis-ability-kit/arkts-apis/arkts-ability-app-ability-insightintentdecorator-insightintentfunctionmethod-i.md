...
该注解与@InsightIntentFunction注解必须组合使用。 使用该注解来装饰类中的静态函数，同时使用@InsightIntentFunction注解来装饰静态函数所属的类，可以将对应的静态函数定义为意图，便于AI入口能够快速执行此函数。 该注解仅支持ArkTS-Sta模式，对应ArkTS-Dyn模式接口见 @InsightIntentFunctionMethod。
