...
当Want中存在URI授权Flag字段（即[FLAG_AUTH_READ_URI_PERMISSION](arkts-ability-flags-e.md)或[FLAG_AUTH_WRITE_URI_PERMISSION](arkts-ability-flags-e.md)），且同时存在PARAMS_STREAM字段时，该字段将不生效。
