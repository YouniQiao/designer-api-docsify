...
表示[EmbeddableUIAbility](arkts-ability-embeddableuiability-c.md)的显示模式，值为枚举类型
...
。该字段需要与文件URI读写[Flags](arkts-ability-flags-e.md)配合使用。
...
当Want中存在URI授权Flag字段（即[FLAG_AUTH_READ_URI_PERMISSION](arkts-ability-flags-e.md)或[FLAG_AUTH_WRITE_URI_PERMISSION](arkts-ability-flags-e.md)）
