...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| startupTasks | [Array](../../apis-arkts/arkts-apis/arkts-arkts-array-c.md)<string> | 是 | 表示准备执行的启动任务[StartupTask](arkts-ability-startuptask-c.md)的名称或预加载so名称的数组。 |
| config | [StartupConfig](arkts-ability-startupconfig-i.md) | 否 | 表示启动任务配置信息，包含启动框架超时时间与启动任务监听器配置。 |
...
| --- | --- | --- | --- |
| startupTasks | [Array](../../apis-arkts/arkts-apis/arkts-arkts-array-c.md)<string> | 是 | 表示准备执行的启动任务[StartupTask](arkts-ability-startuptask-c.md)的名称或预加载so名称的数组。 |
| context | common.AbilityStageContext | 是 | 表示执行启动任务[StartupTask](arkts-ability-startuptask-c.md)的AbilityStage上下文，作为入参传给启动任务的[init](@ohos.app.appstartup.StartupTask:StartupTask#init(context: AbilityStageContext))。 |
| config | [StartupConfig](arkts-ability-startupconfig-i.md) | 是 | 表示启动任务配置信息，包含启动框架超时时间与启动任务监听器配置。 |
