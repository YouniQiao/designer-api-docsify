...
开发者自定义参数，透传到子进程中。可以在[ChildProcess.onStart](arkts-ability-app-ability-childprocess-childprocess-c.md#onstart)方法中通过args.entryParams获取，不传入时子进程无法获取开发者自定义参数。entryParams通过IPC传输，IPC传输的数据量最大为200KB，其中部分由系统占用，建议entryParams传入数据量不超过150KB，否则可能导致创建子进程失败。
