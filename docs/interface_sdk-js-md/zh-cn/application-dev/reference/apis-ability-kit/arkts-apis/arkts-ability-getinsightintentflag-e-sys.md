...
查询[InsightIntentInfo](arkts-ability-insightintentinfo-i-sys.md)中的除entities以外的全量意图信息，详见下表。查询entities信息需要使用GET_ENTITY_INFO。
...
查询[EntityInfo](arkts-ability-entityinfo-i-sys.md)的信息，不可单独使用，必选结合GET_FULL_INSIGHT_INTENT或者GET_SUMMARY_INSIGHT_INTENT使用。例如`GET_FULL_INSIGHT_INTENT | GET_ENTITY_INFO`。
