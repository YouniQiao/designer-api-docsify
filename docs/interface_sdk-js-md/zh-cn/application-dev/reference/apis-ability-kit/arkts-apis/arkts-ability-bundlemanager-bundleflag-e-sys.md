...
用于获取去除分身应用而仅包含主应用的bundleInfo。它仅在 [getAllBundleInfo](arkts-ability-bundlemanager-getallbundleinfo-f-sys.md) 接口中生效。  
**系统API：** 该标记仅支持在系统API中使用。
...
用于获取启用端云文件同步能力或者端云结构化数据同步能力的应用的bundleInfo。它仅在 [getAllBundleInfo](arkts-ability-bundlemanager-getallbundleinfo-f-sys.md) 接口中生效。  
**系统API：** 该标记仅支持在系统API中使用。
