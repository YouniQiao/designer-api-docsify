...
整机可用内存级别，该类型为枚举，可配合UIAbility的[onMemoryLevel()]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_方法根据level执行不同内存级别的相应操作。
    **说明：**  
    ...
    - 当LRU发生变化时，后台应用会根据处于应用使用排序链表（LRU）的位置，触发对应MemoryLevel级别(MEMORY\_LEVEL\_BACKGROUND\_MODERATE、  
    MEMORY\_LEVEL\_BACKGROUND\_LOW、MEMORY\_LEVEL\_BACKGROUND\_CRITICAL)的onMemoryLevel回调。如果应用被冻结，则会在应用唤醒时收到对应的onMemoryLevel回  
    调，因此不建议在此回调接口中做耗时操作。
