...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| key | string | 是 | 目标UDMF数据唯一标识。key必须指定应用（即callerTokenId对应的应用）通过[unifiedDataChannel.insertData](../../apis-arkdata/arkts-apis/arkts-arkdata-insertdata-f.md#insertdata-1)创建，且写入的数据均为有权限授权的文件URI。<br>当前仅支持SYSTEM_SHARE、PICKER和MENU类型的[UDMF数据通路](../../apis-arkdata/arkts-apis/arkts-arkdata-intention-e.md)的key。key的创建与使用方法详见[标准化数据通路实现数据共享](../../../../database/unified-data-channels.md)。 |
