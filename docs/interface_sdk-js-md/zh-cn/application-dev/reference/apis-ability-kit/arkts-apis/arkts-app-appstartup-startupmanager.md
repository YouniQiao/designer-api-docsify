...
| [removeStartupTaskResult](arkts-ability-startupmanager-removestartuptaskresult-f.md) | 删除指定启动任务或so预加载任务的初始化结果。  - 输入为启动任务名时，删除指定启动任务的初始化结果。 - 输入为so文件时，将该so文件置为未加载，缓存中已加载的so文件不会被移除。 |
| [run](arkts-ability-startupmanager-run-f.md) | 执行启动框架启动任务或加载so文件。 |
| [run](arkts-ability-startupmanager-run-f.md) | 执行启动框架启动任务或加载so文件。支持指定[AbilityStageContext](arkts-ability-abilitystagecontext-c.md)用于启动任务的加载。使 用Promise异步回调。 |
