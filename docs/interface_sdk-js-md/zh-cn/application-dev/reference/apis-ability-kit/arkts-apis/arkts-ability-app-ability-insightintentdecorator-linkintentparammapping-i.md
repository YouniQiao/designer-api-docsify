...
若意图参数类别取值为[LINK](arkts-ability-app-ability-insightintentdecorator-linkparamcategory-e.md)，系统获取paramName字段对应的意图参数映射名称，并将该意图参数映射名称拼接到uri链接的末尾(以键值对的形式key=value，key为意图参数映射名称，value为意图参数值)。
