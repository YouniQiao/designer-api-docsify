...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| capturer | [AudioCapturer](arkts-audio-audiocapturer-i.md) | 是 | 目标音频捕获程序实例以打印快照。取值限定为整数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| loopback | [AudioLoopback](arkts-audio-audioloopback-i.md) | 是 | 目标音频环回实例以打印快照。取值限定为整数。 |
| fd | number | 是 | fd是文件描述符，表示快照信息的位置写入到。如果fd小于0或无可写，则快照信息将打印到运行日志，否则快照将写入文件。取值限定为整数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| renderer | [AudioRenderer](arkts-audio-audiorenderer-i.md) | 是 | 目标音频渲染器实例以打印快照。取值限定为整数。 |
| fd | number | 是 | fd是文件描述符，表示快照信息的位置写入到。如果fd小于0或无可写，则快照信息将打印到运行日志，否则快照将写入文件。取值限定为整数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| session | [AudioSessionManager](arkts-audio-audiosessionmanager-i.md) | 是 | 目标音频会话管理器实例以打印快照。取值限定为整数。 |
| fd | number | 是 | fd是文件描述符，表示快照信息的位置写入到。如果fd小于0或无可写，则快照信息将打印到运行日志，否则快照将写入文件。取值限定为整数。 |
