...
| [createAudioCapturer](arkts-audio-audio-createaudiocapturer-f.md) | 获取一个 [AudioCapturer](arkts-audio-audio-audiocapturer-i.md) 实例。此方法通过异步回调返回捕获器实例。 使用 [AudioCapturer](arkts-audio-audio-audiocapturer-i.md) 录制音频时，需要根据 options 参数中的不同 Sourcetype 申请权限， 例如大多数麦克风录音场景需要申请 MICROPHONE 权限。 |
...
| [createAsrProcessingController](arkts-audio-audio-createasrprocessingcontroller-f-sys.md) | 在一个音频捕获器上创建ASR处理控制器。 |
...
| [createTonePlayer](arkts-audio-audio-createtoneplayer-f-sys.md) | 获取一个 [TonePlayer](arkts-audio-audio-toneplayer-i-sys.md) 实例，此方法采用异步回调方式返回渲染器实例。 |
| [createTonePlayer](arkts-audio-audio-createtoneplayer-f-sys.md) | 创建DTMF播放器。使用Promise异步回调。 |
| [createTonePlayer](arkts-audio-audio-createtoneplayer-f-sys.md) | 获取一个 [TonePlayer](arkts-audio-audio-toneplayer-i-sys.md) 实例，此方法使用 Promise 返回渲染器实例。 |
<!--DelEnd-->
...
| [AudioDataCallbackResult](arkts-audio-audio-audiodatacallbackresult-e.md) | 表示音频数据回调结果的枚举。@enum { int } |
...
| [AudioLoopbackEqualizerPreset](arkts-audio-audio-audioloopbackequalizerpreset-e.md) | 表示返听均衡器类型的枚举。@enum { int } |
| [AudioLoopbackMode](arkts-audio-audio-audioloopbackmode-e.md) | 表示返听模式的枚举。 |
| [AudioLoopbackReverbPreset](arkts-audio-audio-audioloopbackreverbpreset-e.md) | 表示返听混响模式的枚举。@enum { int } |
| [AudioLoopbackStatus](arkts-audio-audio-audioloopbackstatus-e.md) | 表示返听状态的枚举。@enum { int } |
...
| [AudioVolumeMode](arkts-audio-audio-audiovolumemode-e.md) | 表示音量模式的枚举。@enum { int } |
...
| [CommunicationDeviceType](arkts-audio-audio-communicationdevicetype-e.md) | 表示用于通信的可用设备类型的枚举。@enum { int } |
...
| [DeviceUsage](arkts-audio-audio-deviceusage-e.md) | 表示音频设备类型的枚举（根据用途分类）。@enum { int } |
...
| 名称 | 说明 |
| --- | --- |
| [AsrAecMode](arkts-audio-audio-asraecmode-e-sys.md) | 枚举，自动语音识别（ASR）的声学回声消除（AEC）模式。@enum { int } |
| [AsrNoiseSuppressionMode](arkts-audio-audio-asrnoisesuppressionmode-e-sys.md) | 枚举，自动语音识别（ASR）的噪音抑制模式。@enum { int } |
...
| [ConnectType](arkts-audio-audio-connecttype-e-sys.md) | 枚举，设备连接类型。@enum { int } |
| [DeviceFlag](arkts-audio-audio-deviceflag-e-sys.md) | 枚举，可获取的设备种类。 |
| [DeviceType](arkts-audio-audio-devicetype-e-sys.md) | 表示设备类型的枚举。 |
| [EffectFlag](arkts-audio-audio-effectflag-e-sys.md) | 枚举，音效分类。@enum { int } |
...
| [PolicyType](arkts-audio-audio-policytype-e-sys.md) | 表示静音策略类型的枚举。@enum { int } |
| [RenderTarget](arkts-audio-audio-rendertarget-e-sys.md) | 枚举，音频渲染器的渲染目标。@enum { int } |
...
| [AudioRendererWriteDataCallback](arkts-audio-audio-audiorendererwritedatacallback-t.md) | 音频渲染器写入数据的回调函数类型定义。 |
