...
在需要管理或追踪当前应用已打开的企业DLP文件时调用该接口，可用于文件状态检查、资源管理等场景。
    **说明：**  
    ...
    - 该接口仅能查询调用方应用通过[generateDlpFileForEnterprise]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_生成的企业DLP文件，无法查询  
    其他应用生成的企业DLP文件。  
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 否 | 企业DLP文件的查询选项。当需要按分类标签筛选查询特定企业DLP文件时传入此参数，当需要查询所有企业DLP文件时可不传此参数。不传入或传入空 字符串时，查询所有企业DLP文件。 |
