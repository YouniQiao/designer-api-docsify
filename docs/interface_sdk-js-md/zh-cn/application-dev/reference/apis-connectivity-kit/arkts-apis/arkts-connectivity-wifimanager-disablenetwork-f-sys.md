...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| netId | ArkTS-Dyn: number  <br>ArkTS-Sta：int | 是 | Identifies the network to disable. The value of networkId cannot be less than 0. |
...
| --- | --- | --- | --- |
| netId | ArkTS-Dyn: number  <br>ArkTS-Sta：int | 是 | Identifies the network to disable. The value of networkId cannot be less than 0. |
| blockDuration | ArkTS-Dyn: number  <br>ArkTS-Sta：int | 是 | Indicates the duration of network disablement(unit is secondes), If the value is -1, means permanent disablement. |
