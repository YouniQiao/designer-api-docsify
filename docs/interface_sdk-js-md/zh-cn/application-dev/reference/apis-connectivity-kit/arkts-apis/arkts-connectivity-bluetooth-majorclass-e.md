# MajorClass

The enum of major class of a bluetooth device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass

<!--Device-bluetooth-enum MajorClass--><!--Device-bluetooth-enum MajorClass-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_MISC

```TypeScript
MAJOR_MISC = 0x0000
```

Miscellaneous device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_MISC

<!--Device-MajorClass-MAJOR_MISC = 0x0000--><!--Device-MajorClass-MAJOR_MISC = 0x0000-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_COMPUTER

```TypeScript
MAJOR_COMPUTER = 0x0100
```

Computer.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_COMPUTER

<!--Device-MajorClass-MAJOR_COMPUTER = 0x0100--><!--Device-MajorClass-MAJOR_COMPUTER = 0x0100-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_PHONE

```TypeScript
MAJOR_PHONE = 0x0200
```

Mobile phone.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_PHONE

<!--Device-MajorClass-MAJOR_PHONE = 0x0200--><!--Device-MajorClass-MAJOR_PHONE = 0x0200-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_NETWORKING

```TypeScript
MAJOR_NETWORKING = 0x0300
```

Network device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_NETWORKING

<!--Device-MajorClass-MAJOR_NETWORKING = 0x0300--><!--Device-MajorClass-MAJOR_NETWORKING = 0x0300-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_AUDIO_VIDEO

```TypeScript
MAJOR_AUDIO_VIDEO = 0x0400
```

Audio or video device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_AUDIO_VIDEO

<!--Device-MajorClass-MAJOR_AUDIO_VIDEO = 0x0400--><!--Device-MajorClass-MAJOR_AUDIO_VIDEO = 0x0400-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_PERIPHERAL

```TypeScript
MAJOR_PERIPHERAL = 0x0500
```

Peripheral device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_PERIPHERAL

<!--Device-MajorClass-MAJOR_PERIPHERAL = 0x0500--><!--Device-MajorClass-MAJOR_PERIPHERAL = 0x0500-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_IMAGING

```TypeScript
MAJOR_IMAGING = 0x0600
```

Imaging device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_IMAGING

<!--Device-MajorClass-MAJOR_IMAGING = 0x0600--><!--Device-MajorClass-MAJOR_IMAGING = 0x0600-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_WEARABLE

```TypeScript
MAJOR_WEARABLE = 0x0700
```

Wearable device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_WEARABLE

<!--Device-MajorClass-MAJOR_WEARABLE = 0x0700--><!--Device-MajorClass-MAJOR_WEARABLE = 0x0700-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_TOY

```TypeScript
MAJOR_TOY = 0x0800
```

Toy.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_TOY

<!--Device-MajorClass-MAJOR_TOY = 0x0800--><!--Device-MajorClass-MAJOR_TOY = 0x0800-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_HEALTH

```TypeScript
MAJOR_HEALTH = 0x0900
```

Health device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_HEALTH

<!--Device-MajorClass-MAJOR_HEALTH = 0x0900--><!--Device-MajorClass-MAJOR_HEALTH = 0x0900-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

## MAJOR_UNCATEGORIZED

```TypeScript
MAJOR_UNCATEGORIZED = 0x1F00
```

Unclassified device.

**起始版本：** 8

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为8。

**废弃版本：** 9

**替代接口：** ohos.bluetoothManager/bluetoothManager.MajorClass.MAJOR_UNCATEGORIZED

<!--Device-MajorClass-MAJOR_UNCATEGORIZED = 0x1F00--><!--Device-MajorClass-MAJOR_UNCATEGORIZED = 0x1F00-End-->

**系统能力：** SystemCapability.Communication.Bluetooth.Core

