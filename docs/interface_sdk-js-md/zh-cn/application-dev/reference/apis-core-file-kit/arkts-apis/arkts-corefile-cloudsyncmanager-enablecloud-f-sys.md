...
| switches | [Record](../../apis-arkts/arkts-apis/arkts-arkts-map-record-c.md)&lt;string, boolean&gt; | 是 | 应用的端云协同特性使能开关，string类型为应用包名，boolean类型为开关状态。true为打开；false为关闭。 |
...
| accountId | string | 是 | 账号Id。 |
| switches | [Record](../../apis-arkts/arkts-apis/arkts-arkts-map-record-c.md)&lt;string, boolean&gt; | 是 | 应用的端云协同特性使能开关，string类型为应用包名，boolean类型为开关状态。true为打开；false为关闭。 |
| callback | [AsyncCallback](../../apis-basic-services-kit/arkts-apis/arkts-basicservices-base-asynccallback-i.md)&lt;void&gt; | 是 | 回调函数。异步使能端云协同能力之后。 |
