...
云盘文件缓存对象移除由  
[on]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口添加的云文件批量缓存过程事件的监听。
...
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;MultiDownloadProgress&gt; | 否 | 回调函数。云文件批量缓存过程事件。如果填写此参数，将取消指定的回调函数；否则，将取消当前订阅的相同事件类型的所有回 调函数。 |
...
停止由[startBatch]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_启动的云文件批量缓存任务。使用Promise异步回调。
