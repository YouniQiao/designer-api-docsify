...
该接口为同步接口，包含耗时操作。为了确保性能，建议使用[addProcessorFromConfig]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_异步接口或者交由子线程执行。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| processor | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 上报事件的数据处理者。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| ArkTS-Dyn: number  \_\_\_HTML\_TAG\_USD\_0\_\_\_ArkTS-Sta：long | 所添加上报事件数据处理者的ID，标识唯一数据处理者，可用于移除数据处理者。 添加失败返回-1，添加成功返回大于0的值。 |
