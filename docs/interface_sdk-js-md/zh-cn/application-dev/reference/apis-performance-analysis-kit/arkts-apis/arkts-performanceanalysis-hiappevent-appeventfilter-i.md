...
提供设置[Watcher]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_的订阅过滤条件的参数选项。用于在事件观察者中设置事件过滤条件，确保只有满足过滤条件的事件才会被监听处理。
    **说明：**  
    
    不同类型应用上，系统事件的订阅规格不同，具体规格可参见\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_。
...
需要订阅的事件领域。可以是系统事件领域（hiAppEvent.domain.OS）或开发者在使用[Write]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口时传入的自定义事件信息（[AppEventInfo]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_）中的事件领域。
