...
需要订阅的事件领域。可以是系统事件领域（hiAppEvent.domain.OS）或开发者在使用[Write](arkts-performanceanalysis-write-f.md#write-1)接口时传入的自定义事件信息（[AppEventInfo](arkts-performanceanalysis-appeventinfo-i.md)）中的事件领域。
