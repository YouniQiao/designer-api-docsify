...
@brief 输入法切换列表弹窗控件的build函数，开发者无需实现。
...
@brief 输入法模式选项配置。仅系统预置输入法支持。 <br>使用场景：系统预置输入法需要支持模式切换功能（如单手模式、全屏模式等）时传入此参数，配置模式图标资源和切换回调。 <br>默认值：不传入时，控件仅显示输入法列表，不提供模式切换功能。 <br>说明：三方输入法应用不可使用此参数。
