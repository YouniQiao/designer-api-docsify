...
**相似接口差异点及选取原则：** 与[inputMethod.switchInputMethod](arkts-ime-switchinputmethod-f.md#switchinputmethod-1)接口相比，本控件提供了可视化的输入法选择界面，适用于需要交互式选择界面的场景；switchInputMethod接口适用于程序化切换输入法的场景，无需用户手动选择。
