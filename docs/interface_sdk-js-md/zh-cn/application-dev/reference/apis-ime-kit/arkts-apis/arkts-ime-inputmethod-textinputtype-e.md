...
@brief NONE。 <br> <br>使用场景：当编辑框不希望指定特定输入类型时使用，输入法将使用默认键盘布局。
...
@brief 文本类型。 <br> <br>使用场景：适用于普通文本输入框，如聊天、备忘录等，输入法显示全功能键盘。
...
@brief 多行类型。 <br> <br>使用场景：适用于需要多行文本输入的场景，如长文本编辑、评论框等。
...
@brief 数字类型。 <br> <br>使用场景：适用于仅需要输入数字的场景，如数量输入、年龄输入等，输入法显示数字键盘。
...
@brief 电话号码类型。 <br> <br>使用场景：适用于电话号码输入框，输入法显示电话号码键盘（包含数字和常用电话符号）。
...
@brief 日期类型。 <br> <br>使用场景：适用于日期时间输入框，输入法显示日期相关的键盘布局。
...
@brief 邮箱地址类型。 <br> <br>使用场景：适用于邮箱输入框，输入法键盘会突出显示"@""."等常用邮箱符号。
...
@brief 链接类型。 <br> <br>使用场景：适用于网址输入框，输入法键盘会突出显示"/""."等常用URL符号。
...
@brief 密码类型。 <br> <br>使用场景：适用于密码输入框，输入法显示可见密码键盘，不进行自动建议。
...
@brief 数字密码类型。 <br> <br>使用场景：适用于仅需输入数字密码的场景，如PIN码输入。
...
@brief 锁屏密码类型。 <br> <br>使用场景：适用于锁屏界面的密码输入框。
...
@brief 用户名类型。 <br> <br>使用场景：适用于用户名输入框，输入法可根据用户名特点优化建议。
...
@brief 新密码类型。 <br> <br>使用场景：适用于设置新密码的输入框，输入法可提供密码强度提示。
...
@brief 带小数点的数字类型。 <br> <br>使用场景：适用于需要输入带小数点数字的场景，如金额输入。
...
@brief 验证码类型。 <br> <br>使用场景：适用于验证码输入框，输入法可优化验证码输入体验。
