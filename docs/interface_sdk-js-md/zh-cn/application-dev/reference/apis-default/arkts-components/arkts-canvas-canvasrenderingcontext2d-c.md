...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| settings | [RenderingContextSettings](arkts-canvas-renderingcontextsettings-c.md) | 否 | 用来配置CanvasRenderingContext2D对象的参数， 见RenderingContextSettings。 异常值undefined和null按RenderingContextSettings的默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| drawingContext | [DrawingRenderingContext](arkts-canvas-drawingrenderingcontext-c.md) | 是 | 一个DrawingRenderingContext类型的对象。 |
| options | [RenderingContextOptions](arkts-canvas-renderingcontextoptions-i.md) | 否 | 渲染上下文的配置选项。 默认值：{ antialias: false }。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [CanvasRenderingContext2D](arkts-canvas-canvasrenderingcontext2d-c.md) | 返回一个CanvasRenderingContext2D对象， 其与入参的DrawingRenderingContext绑定了相同的Canvas组件。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [ImageAnalyzerConfig](../../apis-arkui/arkts-apis/arkts-arkui-imagecommon-imageanalyzerconfig-i.md) | 是 | 执行AI分析所需要的入参，用于配置AI分析功能。 异常值undefined或null按无效值处理。 |
