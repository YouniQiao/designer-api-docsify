...
| animationMode | [SwiperAnimationMode](arkts-swiper-swiperanimationmode-e.md) \| boolean | 否 | 设置翻页至指定页面时的动效模式。<br/>默认值： SwiperAnimationMode.NO_ANIMATION<br/> **说明：** <br/>当传入true时有动效，等同于SwiperAnimationMode.DEFAULT_ANIMATION；当传入 false时无动效，等同于SwiperAnimationMode.NO_ANIMATION。<br/>true：有动效翻页；false：无动效翻页。 |
...
翻至下一页。翻页带动效切换过程，时长通过Swiper的[duration](arkts-swiper-attribute.md#duration)属性设置。
...
翻至上一页。翻页带动效切换过程，时长通过Swiper的[duration](arkts-swiper-attribute.md#duration)属性设置。
