...
设置评分组件作为指示器使用，值为true时，不可改变评分，值为false时，可进行评分。默认值：false  
...
设置并接收评分值。取值为undefined时，按默认值处理。默认值：0取值范围： [0, stars]小于0取0，大于stars取最大值stars。该参数支持[\$\$](../../../ui/state-management/arkts-two-way-sync.md)双向绑定变量。  
**卡片能力（仅ArkTS-Dyn）：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  
**原子化服务API（仅ArkTS-Dyn）：** 从API version 11开始，该接口支持在原子化服务中使用。
