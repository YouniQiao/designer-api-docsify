...
文本选择器弹窗的参数继承自[TextPickerOptions](arkts-textpicker-textpickeroptions-i.md)。

**继承/实现关系：** TextPickerDialogOptionsExt extends [TextPickerOptions](arkts-textpicker-textpickeroptions-i.md)
...
**类型：** [DialogAlignment](../arkts-apis/arkts-alertdialog-dialogalignment-e.md)
...
**说明：**设置为BlurStyle.NONE即可关闭背景虚化。当设置了backgroundBlurStyle为非NONE值时，则不要设置backgroundColor， 否则显示的颜色将不符合预期效果。
...
**说明：**当设置了backgroundColor为非透明色时，backgroundBlurStyle需要设置为BlurStyle.NONE，否则显示的颜色将不符合预期效果。
...
设置选择器中选项的高度。number/double类型取值范围：0, +∞)，string类型仅支持number/double类型取值的字符串形式，例如"56"。默认值：选中项56vp，非选中项36vp。设置该参数后，选中项与非选中项的高度均为所设置的值。
...
<!--Device-TextPickerDialogOptionsExt-defaultPickerItemHeight?: double | string--><!--Device-TextPickerDialogOptionsExt-defaultPickerItemHeight?: double | string-End-->
...
**类型：** [TextPickerTextStyle](arkts-textpicker-textpickertextstyle-i.md)
...
默认值：false  
**说明：**设置为true时，滑动过程中无字号、字重、字体颜色等变化动效，且文本均显示为defaultTextStyle属性设置的样式。 如未设置defaultTextStyle，则显示为Text组件默认样式。
...
设置边缘项（以选中项为基准向上或向下的第二项）的文本颜色、字号、字体粗细、最大字号、最小字号、超长文本截断方式。默认值： { color: '#ff182431', font: {size: '14fp', weight: FontWeight.Regular }, minFontSize: 0, maxFontSize: 0, overflow: TextOverflow.CLIP }
...
设置悬停态下弹窗默认展示区域。默认值：HoverModeAreaType.BOTTOM_SCREEN
...
**类型：** [Callback](../../apis-arkui/arkts-components/arkts-arkui-callback-i.md)&lt;[TextPickerResult](arkts-textpicker-textpickerresult-i.md)&gt;
...
滑动弹窗中的选择器后，选项归位至选中项位置时，触发该回调。回调会在滑动动画结束后触发，如果需要快速获取索引值变化，建议使用onEnterSelectedArea接口。
...
**说明：**在多列联动场景中，不建议使用该回调，由于该回调标识的是滑动过程中选项进入分割线区域内的节点，而跟随变化的选项并不涉及滑动，因此， 回调的返回值中，仅当前滑动列的值会正常变化，其余未滑动列的值保持不变。
...
设置选中项背景样式。默认值： { color: \$r('sys.color.comp_background_tertiary'), borderRadius: \$r('sys.float.corner_radius_level12') }

**类型：** [PickerBackgroundStyle](arkts-textpicker-pickerbackgroundstyle-i.md)
...
设置选中项的文本颜色、字号、字体粗细、最大字号、最小字号、超长文本截断方式。默认值： { color: '#ff007dff', font: { size: '20fp', weight: FontWeight.Medium }, minFontSize: 0, maxFontSize: 0, overflow: TextOverflow.CLIP }
...
设置待选项（以选中项为基准向上或向下的第一项）的文本颜色、字号、字体粗细、最大字号、最小字号、超长文本截断方式。默认值：{ color: '#ff182431', font: { size: '16fp', weight: FontWeight.Regular }, minFontSize: 0, maxFontSize: 0, overflow: TextOverflow.CLIP }
