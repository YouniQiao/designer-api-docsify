...
| ignoreSize | boolean | 是 | 设置是否忽略导航点本身大小，默认false。<br/>true表示可以将导航点更靠近Swiper底部；false表示忽略导航点本身大小。<br/>使用方法可以参考 [示例9演示导航点space与bottom](../../../reference/apis-arkui/arkui-ts/ts-container-swiper copy.md#示例9演示导航点space与bottom) 。<br/> 说明：[数字导航点](arkts-swiper-digitindicator-c.md)ignoreSize属性，不生效的场景如下：<br/> ? 当 [vertical](arkts-swiper-attribute.md#vertical) 设置为false，且bottom &gt; 0。<br/> ? 当 [vertical](arkts-swiper-attribute.md#vertical) 设置为true时：<br/>1、bottom &gt; 0 时。<br/> 2、bottom设为undefined。 <br/> 3、 isSidebarMiddle设置为false时。 |
...
| 类型 | 说明 |
| --- | --- |
| [DigitIndicator](arkts-swiper-digitindicator-c.md) | 数字指示器对象，用于设置Swiper组件的数字导航样式。 |
...
| 类型 | 说明 |
| --- | --- |
| [DotIndicator](arkts-swiper-dotindicator-c.md) | 圆点指示器对象，用于设置Swiper组件的圆点导航样式。 |
