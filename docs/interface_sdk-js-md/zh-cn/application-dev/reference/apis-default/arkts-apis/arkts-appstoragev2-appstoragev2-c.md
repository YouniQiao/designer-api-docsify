...
将键值对数据存储在应用内存中。如果给定的key已经存在于AppStorageV2中，返回对应 的值；否则，通过获取默认值的构造器构造默认值，存储后返回。
...
| key | string | 是 | 指定的key。 |
| defaultCreator | [StorageDefaultCreator](arkts-storagedefaultcreator-t.md)&lt;T&gt; | 否 | 获取默认值的构造器，默认值为undefined。 |
...
将键值对数据存储在应用内存中。如果给定的ttype已经存在于AppStorageV2中，返回 对应的值；否则，通过获取默认值的构造器构造默认值，存储后返回。
...
将指定的键值对数据从AppStorageV2里面删除。如果指定的键值不存在于 AppStorageV2中，将删除失败。
