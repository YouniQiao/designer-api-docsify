...
该接口不支持拉起启动模式为\_\_\_MD\_LINK\_DESC\_USD\_2\_\_\_的UIAbility。

使用规则：

- 调用方应用位于后台时，使用该接口启动Ability需申请\_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_权限。  
- 跨应用场景下，目标Ability的exported属性若配置为false，调用方应用需申请\_\_\_INLINE\_CODE\_DESC\_USD\_1\_\_\_权限。  
- 同设备与跨设备场景下，该接口的使用规则存在差异，详见：\_\_\_MD\_LINK\_DESC\_USD\_3\_\_\_。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 传入需要启动的Ability的信息，包含abilityName、moduleName、bundleName、deviceId、parameters(可选)，parameters缺省或为 空表示后台启动Ability。 |
...
该接口不支持拉起启动模式为\_\_\_MD\_LINK\_DESC\_USD\_4\_\_\_的UIAbility。

使用规则：

- 跨用户场景下，Call调用目标Ability时，调用方应用需同时申请\_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_与\_\_\_INLINE\_CODE\_DESC\_USD\_1\_\_\_权限。  
- 调用方应用位于后台时，使用该接口启动Ability需申请\_\_\_INLINE\_CODE\_DESC\_USD\_2\_\_\_权限。  
- 跨应用场景下，目标Ability的exported属性若配置为false，调用方应用需申请\_\_\_INLINE\_CODE\_DESC\_USD\_3\_\_\_权限。  
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 传入需要启动的Ability的信息，包含abilityName、moduleName、bundleName、deviceId(可选)、parameters(可选)，其中deviceId 缺省或为空表示启动本地Ability，parameters缺省或为空表示后台启动Ability。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| wantList | Array&lt;\_\_\_MD\_LINK\_USD\_0\_\_\_&gt; | 是 | 需要被同时拉起的多个UIAbility的启动参数列表，最多支持传入4个Want。启动参数Want不支持隐式启动、跨用户启动、分布式、免安装和按需加载，不指明分身的 情况下默认启动主应用。 |
...
启动一个新的[UIServiceExtensionAbility]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_。使用Promise异步回调。
    **说明：**  
