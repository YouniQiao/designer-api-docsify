...
重新绘制DrawableDescriptor。当前仅支持 [PictureDrawableDescriptor](arkts-arkuidrawabledescriptor-picturedrawabledescriptor-c.md)类型，其他DrawableDescriptor子类型触发后无效果。 若DrawableDescriptor未绑定任何组件，则不会执行任何操作。
...
查询DrawableDescriptor是否已被释放。返回true表示已释放，此时调用 [getPixelMap](#getpixelmap)、 [getForeground](arkts-arkuidrawabledescriptor-layereddrawabledescriptor-c.md#getforeground)、 [getBackground](arkts-arkuidrawabledescriptor-layereddrawabledescriptor-c.md#getbackground)、 [getMask](arkts-arkuidrawabledescriptor-layereddrawabledescriptor-c.md#getmask)、 [loadSync](#loadsync)、 [load](#load)等接口，返回undefined或默认异常值；返回false表示未释放，对象可正常使用。
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[DrawableDescriptorLoadedResult](arkts-arkuidrawabledescriptor-drawabledescriptorloadedresult-i.md)&gt; | 图片资源的加载结果。 |
...
| 类型 | 说明 |
| --- | --- |
| [DrawableDescriptorLoadedResult](arkts-arkuidrawabledescriptor-drawabledescriptorloadedresult-i.md) | 图片资源的加载结果。 |
...
释放DrawableDescriptor持有的资源。调用release后，该对象将不可用，再调用 [getPixelMap](#getpixelmap)、 [getForeground](arkts-arkuidrawabledescriptor-layereddrawabledescriptor-c.md#getforeground)、 [getBackground](arkts-arkuidrawabledescriptor-layereddrawabledescriptor-c.md#getbackground)、 [getMask](arkts-arkuidrawabledescriptor-layereddrawabledescriptor-c.md#getmask)、 [loadSync](#loadsync)、 [load](#load)等接口，返回undefined或默认异常值。重复调用release不会崩溃。
