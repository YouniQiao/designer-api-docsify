...
重新绘制DrawableDescriptor。当前仅支持 [PictureDrawableDescriptor](arkts-arkui-drawabledescriptor-picturedrawabledescriptor-c.md)类型，其他DrawableDescriptor子类型触发后无效果。 若DrawableDescriptor未绑定任何组件，则不会执行任何操作。
...
查询DrawableDescriptor是否已被释放。返回true表示已释放，此时调用 [getPixelMap](#getpixelmap)、 [getForeground](arkts-arkui-drawabledescriptor-layereddrawabledescriptor-c.md#getforeground)、 [getBackground](arkts-arkui-drawabledescriptor-layereddrawabledescriptor-c.md#getbackground)、 [getMask](arkts-arkui-drawabledescriptor-layereddrawabledescriptor-c.md#getmask)、 [loadSync](#loadsync)、 [load](#load)等接口，返回undefined或默认异常值；返回false表示未释放，对象可正常使用。
...
> **说明：**
> 
> DrawableDescriptor对象通过[release](#release)释放后，本接口返回imageWidth和imageHeight均为 &gt; -1的Promise结果。
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[DrawableDescriptorLoadedResult](arkts-arkui-drawabledescriptor-drawabledescriptorloadedresult-i.md)&gt; | 图片资源的加载结果。 |
...
> **说明：**
> 
> DrawableDescriptor对象通过[release](#release)释放后，本接口返回imageWidth和imageHeight均为 &gt; -1的结果。
...
| 类型 | 说明 |
| --- | --- |
| [DrawableDescriptorLoadedResult](arkts-arkui-drawabledescriptor-drawabledescriptorloadedresult-i.md) | 图片资源的加载结果。 |
...
释放DrawableDescriptor持有的资源。调用release后，该对象将不可用，再调用 [getPixelMap](#getpixelmap)、 [getForeground](arkts-arkui-drawabledescriptor-layereddrawabledescriptor-c.md#getforeground)、 [getBackground](arkts-arkui-drawabledescriptor-layereddrawabledescriptor-c.md#getbackground)、 [getMask](arkts-arkui-drawabledescriptor-layereddrawabledescriptor-c.md#getmask)、 [loadSync](#loadsync)、 [load](#load)等接口，返回undefined或默认异常值。重复调用release不会崩溃。
