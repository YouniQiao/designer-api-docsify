...
| 名称 | 说明 |
| --- | --- |
| [SelectionContainer](arkts-arkui-components-selectioncontainer-selectioncontainer-f.md) | 创建一个SelectionContainer组件。 |
| [SelectionContainer](arkts-arkui-components-selectioncontainer-selectioncontainer-f.md) | 创建一个SelectionContainer组件。需要在组件属性设置开始时调用setSelectionContainerOptions，并在组件属性设置结束时调用applyAttributesFinish。 |
...
| 名称 | 说明 |
| --- | --- |
| [OnMenuItemClickWithTextCallback](arkts-onmenuitemclickwithtextcallback-t.md) | 点击菜单项时触发，可拦截系统默认菜单项（如复制、粘贴菜单项）的执行行为。 |
