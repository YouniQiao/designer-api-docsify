...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| builder | [WrappedBuilder](arkts-builder-wrappedbuilder-c.md)&lt;[CustomBuilder](arkts-custombuilder-t.md)&gt; | 是 | 包含要执行`times`次的@Builder函数的 `WrappedBuilder`。每次执行应创建一个或多个@Reusable /@ReusableV2组件。 |
