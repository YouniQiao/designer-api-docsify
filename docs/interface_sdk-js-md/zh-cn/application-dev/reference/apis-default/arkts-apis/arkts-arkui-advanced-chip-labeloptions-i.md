...
操作块激活时的文字颜色。默认值：\$r('sys.color.ohos_id_color_text_primary_contrary')值为undefined时，按默认值处理。
...
文字字号，不支持百分比。默认值：\$r('sys.float.ohos_id_text_size_button2')值为undefined时，按默认值处理。
...
文本与左右侧图标之间间距。默认值：size为ChipSize.SMALL时，默认值：{ left: 4, right: 4 }size为ChipSize.NORMAL时，默认值：{ left: 6, right: 6 }单位：vp值为undefined时，按默认值处理。

**类型：** [LabelMarginOptions](../../apis-arkui/arkts-apis/arkts-arkui-arkui-advanced-chip-labelmarginoptions-i.md)
