...
| template | [PluginComponentTemplate](arkts-plugincomponent-plugincomponenttemplate-i.md) | 是 | 组件模板。 |
| data | [KVObject](arkts-plugincomponentmanager-kvobject-t.md) | 是 | push事件中传递的数据内容，以键值对形式存储，键和值类型由业务定义。 |
| extraData | [KVObject](arkts-plugincomponentmanager-kvobject-t.md) | 是 | push事件中传递的附加数据，以键值对形式存储，键和值类型由业务定义。 |
