# BorderRadiusMode

边框圆角模式枚举，用于控制分段按钮的圆角计算方式。

| 名称 |值 | 说明 | | ------- | - |-------------------------------- | | DEFAULT | 0 |默认模式，框架自动计算边框圆角。 | | CUSTOM | 1 |自定义模式，开发者设置边框圆角。 |

**起始版本：** 23

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为23。

<!--Device-unnamed-declare enum BorderRadiusMode--><!--Device-unnamed-declare enum BorderRadiusMode-End-->

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

## DEFAULT

```TypeScript
DEFAULT = 0
```

Default mode, radius is auto calculated by framework.

**起始版本：** 23

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为23。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-BorderRadiusMode-DEFAULT = 0--><!--Device-BorderRadiusMode-DEFAULT = 0-End-->

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

## CUSTOM

```TypeScript
CUSTOM = 1
```

Custom mode, radius can be set by developers.

**起始版本：** 23

**ArkTS模式：** 同时支持ArkTS-Dyn、ArkTS-Sta，起始版本为23。

**模型约束：** 此接口仅可在Stage模型下使用。

<!--Device-BorderRadiusMode-CUSTOM = 1--><!--Device-BorderRadiusMode-CUSTOM = 1-End-->

**系统能力：** SystemCapability.ArkUI.ArkUI.Full

