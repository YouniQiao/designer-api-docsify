...
ArcSwiper自定义切换动画超时时间。从页面执行默认动画（页面滑动）至移出视窗外的第一帧开始计时，如果到达该时间后，开发者仍未调用 SwiperContentTransitionProxy 的[finishTransition](arkts-arkuiarcswiper-arcswipercontenttransitionproxy-i.md#finishtransition)接口通知ArcSwiper组件此页面的自定义动画已结束，那么组件就会认为此页面的 自定义动画已结束，立即在该页面节点下渲染树。<br/>单位：ms<br/>默认值：0。
...
**类型：** Callback&lt;[ArcSwiperContentTransitionProxy](arkts-arkuiarcswiper-arcswipercontenttransitionproxy-i.md)&gt;
