...
依照传入的CustomBuilder创建组件树，并持有组件树的根节点。支持响应式状态管理。
...
将输入事件分发到ReactiveBuilderNode管理的目标节点。offsetA为builderNode相对于父组件的偏移，offsetB为命中位置相对于builderNode的偏移，offsetC为offsetA+offsetB，最终输入给postInputEvent当中。
