...
文件映射对象，在调用FileMapping的方法前，需要先通过[mmap()](arkts-fileio-mmap-f.md)或方法[mmapSync()](arkts-fileio-mmapsync-f.md)构建一个FileMapping实例。
...
翻转文件映射区，将写入准备状态切换为读取准备状态。调用后，limit被设置为当前position的值，position被重置为0。推荐在一系列 [write()](arkts-fileio-stream-i.md#write) 操作完成后，调用此方法准备后续的 [read()](arkts-fileio-stream-i.md#read) 操作。
