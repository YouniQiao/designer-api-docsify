...
| [off](arkts-avsession-avsession-off-f-sys.md#offsessiondestroy) | 注销会话销毁事件监听。注销后，不再监听该事件。 |
| [off](arkts-avsession-avsession-off-f-sys.md#offtopsessionchange) | 注销最新播放会话变更事件监听。注销后，不再进行该事件的监听。 |
| [off](arkts-avsession-avsession-off-f-sys.md#offsessionservicedie) | 取消会话服务死亡监听，取消后，不再进行服务死亡监听。 |
| [off](arkts-avsession-avsession-off-f-sys.md#offdistributedsessionchange) | 取消最新分布式远端会话变更的监听事件，取消后，不再进行该事件的监听。 |
| [off](arkts-avsession-avsession-off-f-sys.md#offdeviceavailable) | 取消设备发现回调的监听。 |
| [off](arkts-avsession-avsession-off-f-sys.md#offdeviceoffline) | 取消设备下线回调的监听。 |
| [off](arkts-avsession-avsession-off-f-sys.md#offdevicelogevent) | 取消监听日志事件的回调。 |
| [off](arkts-avsession-avsession-off-f-sys.md#offdevicestatechanged) | 取消投播设备连接状态的监听。 |
...
| [on](arkts-avsession-avsession-on-f-sys.md#onsessioncreate) | 会话的创建事件监听。 使用callback异步回调。 |
| [on](arkts-avsession-avsession-on-f-sys.md#onsessiondestroy) | 会话的销毁事件监听。使用callback异步回调。 |
| [on](arkts-avsession-avsession-on-f-sys.md#ontopsessionchange) | 最新播放会话变更的事件监听。使用callback异步回调。 |
| [on](arkts-avsession-avsession-on-f-sys.md#onsessionservicedie) | 监听会话的服务死亡事件。通知应用清理资源。 |
| [on](arkts-avsession-avsession-on-f-sys.md#ondistributedsessionchange) | 最新分布式远端会话变更的监听事件。 |
| [on](arkts-avsession-avsession-on-f-sys.md#ondeviceavailable) | 设备发现回调监听。 |
| [on](arkts-avsession-avsession-on-f-sys.md#ondeviceoffline) | 设备下线回调监听。 |
| [on](arkts-avsession-avsession-on-f-sys.md#ondevicelogevent) | 监听日志事件的回调。 |
| [on](arkts-avsession-avsession-on-f-sys.md#ondevicestatechanged) | 投播设备连接状态的回调函数。 |
...
| [AVCastController](arkts-avsession-avsession-avcastcontroller-i.md) | 在投播建立后，调用[avSession.AVSession.getAVCastController](arkts-avsession-avsession-avsession-i.md#getavcastcontroller)后，返回会话控制器实例。控制器可查看会话ID，并可完成对会话发送命令及事件，获取会话元数据，播放状态信息等操作。 |
...
| 名称 | 说明 |
| --- | --- |
| [AVCastController](arkts-avsession-avsession-avcastcontroller-i-sys.md) | 在投播建立后，调用[avSession.AVSession.getAVCastController](arkts-avsession-avsession-avsession-i.md#getavcastcontroller)后，返回会话控制器实例。控制器可查看会话ID，并可完成对会话发送命令及事件，获取会话元数据，播放状态信息等操作。 |
