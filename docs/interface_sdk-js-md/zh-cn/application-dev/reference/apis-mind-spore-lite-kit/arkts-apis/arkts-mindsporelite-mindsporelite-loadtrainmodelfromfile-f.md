...
| 错误码ID | 错误信息 |
| --- | --- |
| 1000009 | Failed to create native training model from path. Possible causes: 1. The model file is incorrect; 2. The training configuration is incorrect.  **ArkTS模式：** 该错误码仅适用于ArkTS-Sta。 |
| 1000008 | Invalid model path in training. Possible causes: 1. The model path is null; 2. The model path does not exist.  **ArkTS模式：** 该错误码仅适用于ArkTS-Sta。 |
