...
| 1000002 | Failed to create native model. Possible causes: 1. Insufficient permission to access the model path; 2. The model file is corrupted.  **ArkTS模式：** 该错误码仅适用于ArkTS-Sta。 |
...
| 1000000 | Model path error. Possible causes: 1. The model path is null; 2. The model path does not exist.  **ArkTS模式：** 该错误码仅适用于ArkTS-Sta。 |
