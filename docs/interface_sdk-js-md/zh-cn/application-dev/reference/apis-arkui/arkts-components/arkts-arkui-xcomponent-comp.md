...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [XComponentOptions](arkts-arkui-xcomponent-comp-xcomponentoptions-i.md) | 是 | 表示XComponent的选项。 |
...
| 名称 | 说明 |
| --- | --- |
| [NativeXComponentParameters](arkts-arkui-xcomponent-comp-nativexcomponentparameters-i.md) | 定义XComponent在Native侧使用的具体配置参数。通过这种构造参数创建的XComponent，可以将其对应的FrameNode对象传递至Native侧，使用NDK接口进行Surface生命周期的相关设置和添加事件监听。 |
| [SurfaceConfig](arkts-arkui-xcomponent-comp-surfaceconfig-i.md) | Surface配置。 |
| [SurfaceRect](arkts-arkui-xcomponent-comp-surfacerect-i.md) | 描述XComponent所持有的surface的矩形。 |
| [SurfaceRotationOptions](arkts-arkui-xcomponent-comp-surfacerotationoptions-i.md) | 定义屏幕旋转时是否锁定当前XComponent所持有的surface的方向。 |
| [XComponentOptions](arkts-arkui-xcomponent-comp-xcomponentoptions-i.md) | 定义XComponent的具体配置参数。 |

### 类型

| 名称 | 说明 |
| --- | --- |
| [OnNativeLoadCallback](arkts-arkui-xcomponent-comp-onnativeloadcallback-t.md) | XComponent的Native加载完成后回调事件，用于向开发者传递XComponent实例对象的context。与onSurfaceCreated的区别：onLoad回调参数为context对象，适用于设置libraryname参数的场景；onSurfaceCreated回调参数为surfaceId，适用于未设置libraryname参数的场景。onLoad触发时机早于onSurfaceCreated。 |

### 枚举

| 名称 | 说明 |
| --- | --- |
| [HdrType](arkts-arkui-xcomponent-comp-hdrtype-e.md) | 设置XComponent的HDR类型。 |
...
从API version 20开始，该示例通过调用[lockCanvas](arkts-arkui-xcomponent-comp-xcomponentcontroller-c.md#lockcanvas)返回画布对象，通过画布对象调用对应的绘制接口，再调用[unlockCanvasAndPost](arkts-arkui-xcomponent-comp-xcomponentcontroller-c.md#unlockcanvasandpost)在XComponent上绘制内容。
