...
| 类型 | 说明 |
| --- | --- |
| [LayoutManager](../arkts-apis/arkts-arkui-textcommon-layoutmanager-i.md) \| undefined | 布局管理器对象。<br/>返回undefined时表示controller未与组件绑定。 |
...
| 类型 | 说明 |
| --- | --- |
| [PreviewText](../arkts-apis/arkts-arkui-textcommon-previewtext-i.md) \| undefined | 预上屏信息。<br/>返回undefined时表示controller未与组件绑定。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| range | [TextRange](../arkts-apis/arkts-arkui-textcommon-textrange-i.md) | 否 | 可视范围。 如果参数无效，该方法将不产生效果。 |
...
支持设置组件内的内容选中，选中部分背板高亮。selectionStart和selectionEnd均为-1时表示全选，均为0时可以清空选中区。未获焦时调用该接口不产生选中效果。从API version 12开始，在2in1设备中，无论options取何值，调用setSelection接口都不会弹出菜单，此外，如果组件中已经存在菜单，调用setSelection接口会关闭菜单。在非2in1设备中，options取值为MenuPolicy.DEFAULT时，遵循以下规则：
...
| options | [SelectionOptions](arkts-arkui-selectionoptions-i.md) | 否 | 选择项配置。<br/>缺省时，按照 [SelectionOptions](arkts-arkui-selectionoptions-i.md)中的默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| styledString | [StyledString](../arkts-apis/arkts-arkui-styledstring-styledstring-c.md) | 是 | 设置属性字符串样式的提示文本，其优先级高于[placeholder](arkts-arkui-richeditor-attribute.md#placeholder)属 性设置的提示文本。<br>提示文本不支持触发属性字符串[GestureStyle](../arkts-apis/arkts-arkui-gesturestyle-c.md)样式 绑定的手势事件，以及[UrlStyle](../arkts-apis/arkts-arkui-urlstyle-c.md)样式的超链接跳转能力。 |
