...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [PolygonOptions](../arkts-apis/arkts-arkui-polygon-polygonoptions-i.md) | 否 | Polygon options |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [PolygonOptions](../arkts-apis/arkts-arkui-polygon-polygonoptions-i.md) | 否 | Polygon组件的配置选项，用于定义绘制区域的宽度和高度。需要指定多边形尺寸时传入此参数，不传入时使用默认宽度和高度（均为0）。当传入undefined 或null时，参数设置无效，组件属性维持原值。 |
