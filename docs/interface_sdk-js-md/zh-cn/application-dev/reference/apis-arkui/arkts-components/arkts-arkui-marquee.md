...
> **说明：** > > 为了不影响滚动帧率，建议在滚动类组件中Marquee的个数不超过4个，或者使用 > > 对于Marquee组件动态帧率的场景，可以使用[MarqueeDynamicSyncScene](../../apis-default/arkts-apis/arkts-arkuiuicontext-uicontext-c.md)接口实现。 > > 在文本宽度小于跑马灯组件宽度时，使用属性动画实现滚动。
