...
`wrapBuilder`用于封装全局[@Builder](Builder)，可以将全局`@Builder`函数作为参数传递，实现按引用传递和动态调用，提升代码复用性。开发指南见：[wrapBuilder：封装全局@Builder](../../../ui/state-management/arkts-wrapBuilder.md)。
...
| 类型 | 说明 |
| --- | --- |
| [WrappedBuilder](arkts-arkui-wrappedbuilder-c.md)&lt;Args&gt; | `WrappedBuilder<Args>`的实例，用于在组件之间复用或传递全局`@Builder`函数。该实例封装了指定的全局`@Builder`函数，可通过其`builder`属性调用被封装的构建函数，便于在组件间作为参数传递或赋值给变量。@stagemodelonly@crossplatform@atomicservice |
