...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| modifier | ContentModifier&lt;[ToggleConfiguration](arkts-arkui-toggleconfiguration-i.md)&gt; | 是 | 在Toggle组件上，定制内容区的方法。<br/>modifier：内容修改器，开发者需要自定义class实现 ContentModifier接口。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| color | ResourceColor | 是 | Switch类型的圆形滑块颜色。<br/>默认值：\\$r('sys.color.ohos_id_color_foreground_contrary')<br/> **说明：**<br/>同时设置了systemMaterial新材质时，设置此属性后会出现点光源效果，点光源颜色跟随此属性的设置。 |
