...
aboutToAppear函数在创建自定义组件的新实例后，在其build()函数执行前调用。允许在aboutToAppear函数中改变[状态变量](../../../ui/state-management/arkts-state-management-glossary.md#state-variables状态变量)，更改将在后续执行build()函数中生效。实现[自定义布局](arkts-arkui-common-comp-layoutable-i.md)的自定义组件的aboutToAppear生命周期在布局过程中触发。具体使用说明，详见[自定义组件生命周期指南](../../../ui/state-management/arkts-page-custom-components-lifecycle.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| selfLayoutInfo | [GeometryInfo](arkts-arkui-common-comp-geometryinfo-i.md) | 是 | 计算自定义组件大小后的自身布局信息。<br>**说明：** <br>第一次布局时以自身设置的属性为准。 |
| children | Array&lt;[Measurable](arkts-arkui-common-comp-measurable-i.md)&gt; | 是 | 计算子组件大小后的子组件布局信息。<br>**说明：** <br>如果没有设置子组件的布局信息，子组件会维持上一次的布局信息，当子组件从来没有设置过尺寸时，尺寸默认为0。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| selfLayoutInfo | [GeometryInfo](arkts-arkui-common-comp-geometryinfo-i.md) | 是 | 计算自定义组件大小后的自身布局信息。 |
| children | Array&lt;[Layoutable](arkts-arkui-common-comp-layoutable-i.md)&gt; | 是 | 计算子组件大小后的子组件布局信息。 |
| constraint | [ConstraintSizeOptions](../arkts-apis/arkts-arkui-constraintsizeoptions-i.md) | 是 | 自定义组件的布局约束信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| theme | [Theme](arkts-arkui-common-comp-theme-t.md) | 是 | 自定义组件当前生效的Theme对象，可在回调中通过该对象获取主题配色等资源，用于更新组件的样式变量。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isInner | [Optional](arkts-arkui-common-comp-optional-t.md)&lt;boolean&gt; | 是 | true：向内查询最近的，且在栈内的NavDestinationInfo的详细信息。<br>false：向外查询最近的，且在栈内的NavDestinationInfo的详细信息。 |
