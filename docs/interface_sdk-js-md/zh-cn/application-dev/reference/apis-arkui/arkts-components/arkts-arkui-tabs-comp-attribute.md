...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | boolean | 是 | 是否为纵向Tabs。<br>默认值：false，横向Tabs，为true时纵向Tabs。<br>当横向Tabs设置height为auto时，Tabs组件高度自适应子组件高度，即为[tabBar](arkts-arkui-tabcontent-comp-attribute.md#tabbar)高度+ divider线宽+TabContent高度+Tabs组件的上下padding值+Tabs组件的上下border宽度。<br>当纵向Tabs设置width为auto时，Tabs组件宽度自适应子组件宽度，即为tabBar宽度+divider线宽+ TabContent宽度+左右padding值+左右border宽度。<br>尽量保持每一个页面中的子组件尺寸大小一致，避免滑动页面时出现页面切换动画跳动现象。 |
