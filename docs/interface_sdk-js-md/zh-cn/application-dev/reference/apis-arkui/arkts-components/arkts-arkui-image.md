...
Image加载成功且组件不设置宽高时，其显示大小自适应父组件。
    **说明：**  
    ...
    - 当Image组件入参为[PixelMap]\_\_\_JSDOC\_LINK\_DESC\_USD\_5\_\_\_类型时，只有当PixelMap对象发生变化（即指向一个新的PixelMap实例），  
    Image组件才能感知到数据的变化。仅修改PixelMap对象的内容（如像素值）而不更换对象引用，无法触发数据变化的感知。  
    
    - Image组件入参为Base64字符串时，Base64字符串通用格式为\_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_，其中subtype为类型声明，Base64  
    EncodedData为数据对应的base64编码，其他为固定字符串。例如：png图像对应的入参为\_\_\_INLINE\_CODE\_DESC\_USD\_1\_\_\_。  
    
    1. image/subType用于声明数据内容的类型。从API版本26.0.0开始，Image组件接受任意\_\_\_INLINE\_CODE\_DESC\_USD\_2\_\_\_格式的Base64字符串，具体图片类  
    ...
    2. Image组件从API版本26.0.0开始支持通过\_\_\_INLINE\_CODE\_DESC\_USD\_3\_\_\_的通配写法，对于API版本26.0.0之前版本，Image组件不支持  
    \_\_\_INLINE\_CODE\_DESC\_USD\_4\_\_\_的通配写法，subType必须显式声明具体的图片类型。  
