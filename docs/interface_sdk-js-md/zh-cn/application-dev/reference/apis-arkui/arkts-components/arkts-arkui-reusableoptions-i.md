...
可复用自定义组件的内存优化策略。该参数在创建可复用自定义组件时设定，不支持动态修改。传入[ENABLE\_AUTO\_CACHE\_OPTIMIZATION]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_时可启用自动内存优化，在应用退后台、组件不可见或整机低内存等场景下自动释放复用池中的组件；不传入时使用默认值[DEFAULT]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_（无内存优化策略）。
