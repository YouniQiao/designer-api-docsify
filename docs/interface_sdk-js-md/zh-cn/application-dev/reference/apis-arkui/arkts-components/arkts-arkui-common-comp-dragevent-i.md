...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| customDropAnimation | [Callback](arkts-arkui-common-comp-callback-i.md)&lt;void&gt; | 是 | 在此回调函数中实现自定义落位动效。<br> **说明：** <br>1. 该接口仅在onDrop回调中使用有效。<br> 2.使用前需设置useCustomDropAnimation为true，否则该接口不生效。<br> 3. 不要在动画callback中实现与动效无关的逻辑，避免影响执行效率。 |
...
| 类型 | 说明 |
| --- | --- |
| [UnifiedData](arkts-arkui-common-comp-unifieddata-t.md) | 从DragEvent中获取拖拽相关数据。数据获取结果请参考错误码说明。 |
...
获取当前拖拽事件发生时所在的屏幕ID，不支持在[onDragEnd](arkts-arkui-common-comp-commonmethod-c.md#ondragend)阶段使用。
...
| 类型 | 说明 |
| --- | --- |
| [Rectangle](arkts-arkui-common-comp-rectangle-i.md) | 拖拽预览图相对于当前窗口的位置，以及预览图尺寸信息，单位vp，其中x和y代表预览图左上角的窗口坐标，width和height代表预览图的尺寸。 |
...
| 类型 | 说明 |
| --- | --- |
| [DragResult](arkts-arkui-common-comp-dragresult-e.md) | 从DragEvent中获取的拖拽结果。 |
...
| 类型 | 说明 |
| --- | --- |
| [Summary](arkts-arkui-common-comp-summary-t.md) | 拖拽相关数据的概要。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| unifiedData | [UnifiedData](arkts-arkui-common-comp-unifieddata-t.md) | 是 | 拖拽相关的数据。 |
...
设置起拖方延迟提供数据。使用此方法向系统提供数据加载参数，而不是直接提供完整的数据对象。当用户在目标应用程序上落入时，系统将使用此参数从起拖方请求实际数据。与[setData](#setdata)方法同时使用，以最后调用的方法为准。该接口仅在[onDragStart](arkts-arkui-common-comp-commonmethod-c.md#ondragstart)回调中生效。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dataLoadParams | [DataLoadParams](arkts-arkui-common-comp-dataloadparams-t.md) | 是 | 落入操作时使用的数据加载参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dragResult | [DragResult](arkts-arkui-common-comp-dragresult-e.md) | 是 | 拖拽结果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [DataSyncOptions](arkts-arkui-common-comp-datasyncoptions-t.md) | 是 | 获取拖拽数据时的参数，包含目标路径、文件冲突选项、进度条类型等。数据传输过程中可使用[cancelDataLoading](../arkts-apis/arkts-arkui-arkui-uicontext-dragcontroller-c.md#canceldataloading)接口取消数据加载。 |
...
仅在[onDragStart](arkts-arkui-common-comp-commonmethod-c.md#ondragstart)回调中设置生效。拖拽成功发起后，系统会在显示拖拽预览窗口前隐藏目标组件。
...
**类型：** [DragBehavior](arkts-arkui-common-comp-dragbehavior-e.md)
