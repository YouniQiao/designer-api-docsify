...
> 针对侧边栏子组件设置通用属性宽高时，宽高都不生效。

> 针对侧边栏内容区设置通用属性宽高时，宽高都不生效，默认占满SideBarContainer的剩余空间。
...
分割线的宽度不支持百分比设置。优先级低于[通用属性height，超过通用属性设置大小时，按照通用属性进行裁切。部分设备硬件中存在1像素取整后分割线不显示问题，建议使用2像素。
