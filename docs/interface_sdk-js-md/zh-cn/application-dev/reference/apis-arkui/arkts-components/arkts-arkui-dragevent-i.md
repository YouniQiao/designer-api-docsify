...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [DataSyncOptions](arkts-arkui-datasyncoptions-t.md) | 是 | 获取拖拽数据时的参数，包含目标路径、文件冲突选项、进度条类型等。数据传输过程中可使用 [cancelDataLoading](../../apis-default/arkts-apis/arkts-arkuiuicontext-dragcontroller-c.md#canceldataloading)接口取消数据加载。 |
...
组件的uniqueId可通过[UIContext.getFrameNodeById()](../../apis-default/arkts-apis/arkts-arkuiuicontext-uicontext-c.md#getframenodebyid) 配合[FrameNode.getUniqueId()](../arkts-apis/arkts-arkui-framenode-c.md#getuniqueid)获取。
