...
设置自定义落位动效的执行函数，仅在[useCustomDropAnimation](arkts-arkui-dragevent-i.md#usecustomdropanimation)为true时有效。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| customDropAnimation | [Callback](../../apis-basic-service-kit/arkts-apis/arkts-basicservices-callback-i.md)<void> | 是 | 在此回调函数中实现自定义落位动效。<br/> **说明：** <br/>1. 该接口仅在onDrop回调中使用有效。<br/> 2.使用前需设置useCustomDropAnimation为true，否则该接口不生效。<br/> 3. 不要在动画callback中实现与动效无关的逻辑，避免影响执行效率。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| keys | [Array](../../apis-arkts/arkts-apis/arkts-arkts-array-c.md)<string> | 是 | 获取功能键按压状态。报错信息请参考以下错误码。支持功能键 'Ctrl' \| 'Alt' \| 'Shift'。<br/>**说明：**<br/>此接口不支持在手写笔场景下使用。 |
...
| 类型 | 说明 |
| --- | --- |
| [Summary](../../apis-arkdata/arkts-apis/arkts-arkdata-summary-c.md) | 拖拽相关数据的概要。 |
...
设置起拖方延迟提供数据。使用此方法向系统提供数据加载参数，而不是直接提供完整的数据对象。当用户在目标应用程序上落入时，系统将使用此参数从起拖方请求实际数据。与[setData](arkts-arkui-dragevent-i.md#setdata-1)方法同时使用，以最后调用的方法为准。该接口仅在[onDragStart](arkts-arkui-commonmethod-c.md#ondragstart-1)回调中生效。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dataLoadParams | [DataLoadParams](../../apis-arkdata/arkts-apis/arkts-arkdata-dataloadparams-i.md) | 是 | 落入操作时使用的数据加载参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [DataSyncOptions](arkts-arkui-datasyncoptions-t.md) | 是 | 获取拖拽数据时的参数，包含目标路径、文件冲突选项、进度条类型等。数据传输过程中可使用[cancelDataLoading](../arkts-apis/arkts-arkui-dragcontroller-c.md#canceldataloading-1)接口取消数据加载。 |
...
组件的uniqueId可通过[UIContext.getFrameNodeById()](../arkts-apis/arkts-arkui-uicontext-c.md#getframenodebyid-1)配合[FrameNode.getUniqueId()](../arkts-apis/arkts-arkui-framenode-c.md#getuniqueid-1)获取。

开发者应在[onDragEnd](arkts-arkui-commonmethod-c.md#ondragend-1)或[onDrop](arkts-arkui-commonmethod-c.md#ondrop-1)中恢复组件显示状态。
...
当不配置或设置为false时，系统默认落位动效生效，当[setResult](arkts-arkui-dragevent-i.md#setresult-1)设置为DRAG_SUCCESSFUL时，落位为缩小消失动效，不为DRAG_SUCCESSFUL时，则为放大消失动效。
