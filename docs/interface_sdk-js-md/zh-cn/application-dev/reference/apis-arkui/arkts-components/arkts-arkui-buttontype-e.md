...
> 
> - 设置Button的[border](arkts-arkui-commonmethod-c.md#border)时，会有默认的borderRadius值。如果同时使用`border`和`borderRadius`，需将`borderRadius`放在`border`之后，以确保`borderRadius`不会被`border`中的默认`radius`覆盖。
