...
参数类型为[Length](Length)时，统一设置四条边框的向外延伸距离。

参数类型为[EdgeWidths](../arkts-apis/arkts-arkui-units-edgewidths-i.md#EdgeWidths)时：
...
参数类型为[LocalizedEdgeWidths](LocalizedEdgeWidths)&lt;sup&gt;12+&lt;/sup&gt;时：
...
参数类型为[Length](Length)时，统一设置四个角的宽高。
...
参数类型为[Length](Length)时，统一设置四条边框的宽度。
