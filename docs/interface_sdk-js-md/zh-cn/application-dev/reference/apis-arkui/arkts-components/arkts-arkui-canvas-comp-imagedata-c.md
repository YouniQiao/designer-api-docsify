...
| unit | LengthMetricsUnit | 否 | 用来配置ImageData对象的单位模式，配置后无法动态更改，配置方法同[CanvasRenderingContext2D](arkts-arkui-canvas-comp-canvasrenderingcontext2d-c.md)。当需要使用vp单位实现响应式布局或适配不同屏幕密度时传入此参数。<br>异常值undefined、NaN和Infinity按默认值处理。<br>默认值：DEFAULT。 |
