...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| dataSource | [IDataSource](../arkts-apis/arkts-arkui-lazyforeach-idatasource-i.md) | 是 | LazyForEach数据源，需要开发者实现相关接口。 |
...
| options | [LazyForEachOptions](../arkts-apis/arkts-arkui-lazyforeach-lazyforeachoptions-i.md) | 否 | 开发者配置项，用于使能自定义组件冻结和配置内存优化策略、资源释放策略。使用此配置项时，必须设置键值生成函数，否则将编译失败。不传入时使用默认配置（ 自定义组件冻结模式默认为AUTO，资源释放策略默认为BATCH，内存优化策略默认为DEFAULT）。 |
