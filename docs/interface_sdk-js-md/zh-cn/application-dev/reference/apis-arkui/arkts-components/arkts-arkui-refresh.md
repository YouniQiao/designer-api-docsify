...
> **说明：**>> - 该组件从API version 12开始支持与垂直滚动的Swiper和> Web的联动。当Swiper设置> loop属性为true时，Refresh无法和Swiper产生联动。>> - Refresh和内容大小小于组件自身的List组件嵌套使用并且中间还有其他组件时，手势可能会被中间组件响应，导致Refresh未产生下拉刷新效果。此时可以将> alwaysEnabled参数设为true，List会响应手势并通过嵌套滚动带动Refresh组件产生下拉刷新效果。具体可以参考> 示例9（不满一屏场景实现下拉刷新）。>> - 组件内部已绑定手势实现跟手滚动等功能，需要增加自定义手势操作时请参考手势拦截增强进行处理。>> - 组件无法通过鼠标按下拖动操作进行下拉刷新。
...
| 参数名 | 类型 | 必填 |
| --- | --- | --- |
| value | [RefreshOptions](arkts-arkui-refreshoptions-i.md) | 是 |
