...
取消[on('afterPanEnd')](#on_navdestinationupdatenavdestinationupdate)监听Pan手势 onActionEnd事件执行后的callback回调。
...
取消[on('afterPanStart')](#on_navdestinationupdatenavdestinationupdate)监听Pan手势 onActionStart事件执行后的callback回调。
...
取消[on('beforePanEnd')](#on_navdestinationupdatenavdestinationupdate)监听Pan手势 onActionEnd事件执行前的callback回调。
...
取消[on('beforePanStart')](#on_navdestinationupdatenavdestinationupdate)监听Pan手势 onActionStart事件执行前的callback回调。
...
| --- | --- | --- | --- |
| type | 'densityUpdate' | 是 | 监听事件，固定为'densityUpdate'，即屏幕像素密度变化。 |
| callback | [Callback](../../apis-basic-services-kit/arkts-apis/arkts-basicservices-base-callback-i.md)&lt;observer.DensityInfo&gt; | 是 | 回调函数。携带 [DensityInfo](../../apis-na/arkts-apis/arkts-na-uiobserver-densityinfo-c.md)，返回变化后的屏幕像素密度。 |
