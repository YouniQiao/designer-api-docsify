...
| 类型 | 说明 |
| --- | --- |
| [AppResponse](arkts-arkui-systemapp-appresponse-i.md) | 定义AppResponse信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [RequestFullWindowOptions](arkts-arkui-systemapp-requestfullwindowoptions-i.md) | 否 | Transition time from non-full window to full window, in milliseconds. By default, the value is in direct proportion to the distance between the non-full window and the full window. |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [ScreenOnVisibleOptions](arkts-arkui-systemapp-screenonvisibleoptions-i.md) | 否 | 当启动保活时，锁屏时将阻止系统返回桌面显示，以保持屏幕唤醒时应用可见。 |
