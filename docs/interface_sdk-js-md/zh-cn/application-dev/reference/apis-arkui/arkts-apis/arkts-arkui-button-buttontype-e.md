...
> 
> - 按钮圆角通过
> [通用属性borderRadius](CommonMethod#borderRadius(value: Length | BorderRadiuses | LocalizedBorderRadiuses))设置。
> 
...
> 
> - 按钮文本通过[fontSize](arkts-arkui-button-buttonattribute-i.md#fontSize)、[fontColor](arkts-arkui-button-buttonattribute-i.md#fontColor)、[fontStyle](arkts-arkui-button-buttonattribute-i.md#fontStyle)、
> [fontFamily](arkts-arkui-button-buttonattribute-i.md#fontFamily)、[fontWeight](arkts-arkui-button-buttonattribute-i.md#fontWeight)进行设置。
> 
> - 设置[颜色渐变](common)需先设置[backgroundColor](CommonMethod#backgroundColor(value: ResourceColor))为透明色。
> 
> - 在不设置borderRadius时，圆角矩形按钮的圆角大小保持默认值不变。圆角大小不会随按钮高度变化而变化，和controlSize属性有关，controlSize为NORMAL时圆角大小20vp，controlSize为
> SMALL时圆角大小14vp。
> 
> - 设置Button的[border](CommonMethod#border)时，会有默认的
> [borderRadius](CommonMethod#borderRadius(value: Length | BorderRadiuses | LocalizedBorderRadiuses))值。如果同时使用
