...
使用默认曝光的HDR颜色创建ColorMetrics类。使用默认曝光（0.0表示对数，1.0表示线性）创建HDR颜色值。当没有指定曝光值时，RGB通道值可以超过1.0以实现HDR亮度。这与iOS UIColor行为匹配，其中RGB值  
    1.0启用HDR渲染。
...
检查ColorMetrics是否代表HDR颜色。如果颜色是使用createHDRColorWithXx()创建的，或者RGB值  
    1.0，则返回true。
