...
**继承/实现关系：** TextPickerDialogOptionsExt extends [TextPickerOptions](textpicker-textpickeroptions-i.md)

**起始版本：** 23

**ArkTS模式：** 仅支持ArkTS-Sta，起始版本为23。

<!--Device-unnamed-export declare interface TextPickerDialogOptionsExt extends TextPickerOptions--><!--Device-unnamed-export declare interface TextPickerDialogOptionsExt extends TextPickerOptions-End-->
...
\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_，呈现效果依然是胶囊型按钮  
...
设置选择器中选项的高度。number/double类型取值范围：[0, +∞)，string类型仅支持number/double类型取值的字符串形式，例如"56"。
...
设置为true时，滑动过程中无字号、字重、字体颜色等变化动效，且文本均显示为defaultTextStyle属性设置的样式。如未设置defaultTextStyle，则显示为[Text]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_组件默认样式。
...
默认值： { color: \$r('sys.color.comp\_background\_tertiary'), borderRadius: \$r('sys.float.corner\_radius\_level12') }
