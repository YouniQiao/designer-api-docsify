...
`CustomComponentContext`类提供对组件级服务的访问，包括复用池。通过 [UIUtils.getCustomComponentContext](arkts-arkui-arkuistatemanagement-uiutils-c.md#getcustomcomponentcontext)获取实例。
...
| 类型 | 说明 |
| --- | --- |
| [IReusePool](arkts-arkui-arkuistatemanagement-ireusepool-i.md) \| undefined | 当前组件配置全局复用池时，返回复用池信息，否则返回`undefined`。 |
