...
窗口内的可绘制区域尺寸，其中左边界上边界是相对于窗口左上顶点计算。在Stage模型下，需要在调用[loadContent()](arkts-arkui-window-i.md#loadcontent-1)或[setUIContent()](arkts-arkui-window-i.md#setuicontent-1)加载页面内容后获取该属性。
...
当前存在主窗使用[getWindowProperties()](arkts-arkui-window-i.md#getwindowproperties-1)接口返回type不准确的问题，开发者在创建窗口时已指明窗口类型，无需通过getWindowProperties()接口获取窗口类型。
...
**替代接口：** [windowType](arkts-arkui-windowproperties-i.md#windowtype)
