...
默认情况下，系统以[STANDARD](arkts-arkui-image-dynamicrangemode-e.md)模式进行截图。如果知道被截图组件使用的动态范围模式，可通过`dynamicRangeMode`字段指定具体的动态范围模式，并将`isAuto`设置为false，以达到预期的截图效果。
...
取值范围：[DynamicRangeMode](arkts-arkui-image-dynamicrangemode-e.md) 枚举值。

默认值：STANDARD 

如果值为undefined、null或未设置，则使用默认值截图；其他异常值会导致截图失败，返回错误码160003。

**类型：** [DynamicRangeMode](arkts-arkui-image-dynamicrangemode-e.md)
