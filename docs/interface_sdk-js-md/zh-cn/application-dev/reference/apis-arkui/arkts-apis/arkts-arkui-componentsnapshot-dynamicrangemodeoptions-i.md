...
默认情况下，系统以[STANDARD](DynamicRangeMode)模式进行截图。如果知道被截图组件使用的动态范围模式，可通过`dynamicRangeMode`字段指定具体的动态范围模式，并将`isAuto`设置为false，以达到预期的截图效果。
...
取值范围：[DynamicRangeMode](DynamicRangeMode) 枚举值。
