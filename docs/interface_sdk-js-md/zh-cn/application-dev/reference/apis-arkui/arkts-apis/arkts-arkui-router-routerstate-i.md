...
表示当前页面携带的参数。  
**说明：** params参数只能传递可序列化的参数，不能传递方法和系统接口返回的对象（例如，媒体接口定义和返回的PixelMap对象）。建议开发者提取系统接口返回的对象中需要被传递的基础类型属性，自行构造object类型对象进行传递。从API version 12开始，该接口支持在原子化服务中使用。此接口仅可在Stage模型下使用。
