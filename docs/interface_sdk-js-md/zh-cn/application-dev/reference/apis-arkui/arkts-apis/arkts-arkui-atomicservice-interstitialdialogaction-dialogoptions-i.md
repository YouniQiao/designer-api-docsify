...
弹框距离底部偏移类型，需根据是否存在菜单栏选择对应值。默认值为BottomOffset.OFFSET\_FOR\_NONE。
...
弹框副标题文本颜色。默认为\$r('sys.color.ohos\_id\_color\_text\_secondary\_contrary')。
...
弹框主标题文本颜色。默认为\$r('sys.color.ohos\_id\_color\_text\_primary\_contrary')。
