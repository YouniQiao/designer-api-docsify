...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| enable | boolean \| undefined | 是 | 是否启用AI分析功能。&lt;br&gt;true：开启AI分析；false：关闭AI分析。 &lt;br&gt;默认值：false&lt;br&gt;ArkTS-Sta模式下可不传，不传时使用默认值false。&lt;br&gt;**说明：**仅type为SURFACE或TEXTURE时该功能有效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isSecure | boolean \| undefined | 是 | 是否开启隐私图层模式。&lt;br&gt;true：开启隐私图层模式；false：关闭隐私图层模式。&lt;br&gt;默认值：false。 &lt;br&gt;ArkTS-Sta模式下可不传，不传时使用默认值false。&lt;br&gt;**说明：**仅type为SURFACE时有效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| brightness | double \| undefined | 是 | HDR视频的亮度。&lt;br&gt;默认值：1.0&lt;br&gt;取值范围：[0.0, 1.0]。 小于0.0的值按0.0处理，大于1.0的值按1.0处理，其他异常值按1.0处理。&lt;br&gt;0.0表示视频按照SDR亮度显示，1.0表示视频按照当前允许的最高HDR亮度显示。 &lt;br&gt;**说明：**仅type为SURFACE时有效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| brightness | double \| undefined | 是 | HDR内容的亮度。&lt;br&gt;默认值：1.0&lt;br&gt;取值范围：[0.0, 1.0]。 小于0.0的值按0.0处理，大于1.0的值按1.0处理，其他异常值按1.0处理。&lt;br&gt;0.0表示内容按照SDR亮度显示，1.0表示内容按照当前允许的最高HDR亮度显示。 &lt;br&gt;ArkTS-Sta模式下可不传，不传时使用默认值1.0。 |
| type | [HdrType](arkts-arkui-xcomponent-hdrtype-e.md) | 否 | 显示HDR内容时的HDR类型。&lt;br&gt;默认值：HdrType.DEFAULT |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [VoidCallback](arkts-arkui-voidcallback-t.md) \| undefined | 是 | 插件卸载完成时回调事件。&lt;br&gt;ArkTS-Sta模式下可不传。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [VoidCallback](arkts-arkui-voidcallback-t.md) \| undefined | 是 | 插件加载完成时回调事件，用于获取XComponent实例对象的context。&lt;br&gt;ArkTS-Sta模式下可不传。 |
