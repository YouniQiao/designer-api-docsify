...
文本颜色。默认值：\$r('sys.color.font_primary')。当[shaderStyle](arkts-arkui-richeditor-richeditorparagraphstyle-i.md)同时设置时，shaderStyle优先级高于fontColor。
...
设置字体列表，当前支持'HarmonyOS Sans'字体和[注册自定义字体](arkts-font.md)。默认字体:'HarmonyOS Sans'。
...
字体粗细。number类型取值[100,900]，取值间隔为100，默认为400，取值越大，字体越粗。超出范围时按默认值400生效。string类型仅支持number类型取值的字符串形式，例如"400"，以及"bold"、"bolder"、"lighter"、"regular"、"medium"分别对应FontWeight中相应的枚举值。默认值：FontWeight.Normal。
...
设置文本的文本行高。默认值：不设置时自适应字体大小。number类型取值范围：(0, +∞)，设置值不大于0时，不限制文本行高，自适应字体大小。number类型时单位为fp，不支持设置百分比字符串。 当lineHeight设置值小于当前字号下文本渲染出的实际高度时，[fallbackLineSpacing](arkts-arkui-richeditor-richeditorattribute-i.md#fallbacklinespacing)属性将生效。
...
文本描边宽度。如果LengthMetrics的unit值是[PERCENT](arkts-arkui-graphics-lengthunit-e.md)，当前设置不生效，作为0处理。值小于0时为实体字，大于0时为轮廓字，等于0时无描边效果。默认值：0。单位：LengthMetrics类型时跟随LengthMetrics，number类型时是vp。取值范围：(-∞, +∞)与[shaderStyle](arkts-arkui-richeditor-richeditorparagraphstyle-i.md)同时设置时，shaderStyle不生效。  
**模型约束：** 此接口仅可在Stage模型下使用。
...
文本背景样式。默认值：{color: Color.Transparent, radius: 0}
...
设置文字阴影效果。默认值：undefined，不设置文字阴影效果。该接口支持以数组形式入参，实现多重文字阴影。  
**说明：**仅支持设置阴影模糊半径、颜色和偏移量，不支持智能取色。
