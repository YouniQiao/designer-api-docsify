...
- 当物理屏不存在HDR资源时，与[save](arkts-arkui-save-f-sys.md#save-1)接口返回一个SDR的PixelMap不同，该接口返回包含一个SDR的PixelMap数组。同时该接口不具备[save](arkts-arkui-save-f-sys.md#save-1)接口的裁剪、拉伸、旋转功能。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [HdrScreenshotOptions](arkts-arkui-hdrscreenshotoptions-i-sys.md) | 否 | 要截取的HDR图像信息。默认为空。 |
