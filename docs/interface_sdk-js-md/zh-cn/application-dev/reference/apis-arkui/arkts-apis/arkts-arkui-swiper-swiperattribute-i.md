...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | boolean \| undefined | 是 | 子组件是否自动播放。&lt;br/&gt;true：自动播放；false：不自动播放。&lt;br/&gt;传入非法值时，按false处理。&lt;br/&gt;取值为undefined时 ，按默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| autoPlay | boolean \| undefined | 是 | 子组件是否自动播放。&lt;br/&gt;true：自动播放；false：不自动播放。&lt;br/&gt;传入非法值时，按false处理。&lt;br/&gt;取值为 undefined时，按默认值处理。 |
| options | [AutoPlayOptions](arkts-arkui-swiper-autoplayoptions-i.md) \| undefined | 是 | 配置手指或鼠标按下屏幕时子组件是否停止自动播放。当stopWhenTouched设置为true时，多指按下场景中任意一个手指抬起 后，将自动继续播放。&lt;br/&gt;默认值：{ stopWhenTouched: true }，停止自动播放。&lt;br/&gt;取值为undefined时，按默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| count | int \| undefined | 是 | 预加载子组件个数。&lt;br/&gt;默认值：1&lt;br/&gt;取值范围：[0, +∞)，设置小于0的值时，按照默认值处理。&lt;br/&gt;取值为undefined时，按默认值处理。 |
| isShown | boolean \| undefined | 是 | 预加载范围内的节点是否进行绘制，不下渲染树。&lt;br/&gt;true：预加载范围内的节点进行绘制；false：预加载范围内的节点不进行绘制。&lt;br/&gt;传入 非法值以及undefined时，按false处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| count | int \| undefined | 是 | 预加载子组件个数。&lt;br/&gt;取值范围：[0, +∞)，设置小于0的值时，按照1处理。&lt;br/&gt;取值为undefined时，按照1处理。 |
| options | [CachedCountOptions](arkts-arkui-swiper-cachedcountoptions-i.md) \| undefined | 是 | 预加载子组件的配置选项。&lt;br/&gt;取值为undefined时，按CachedCountOptions中的默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ArrowStyle](arkts-arkui-swiper-arrowstyle-i.md) \| boolean \| undefined | 是 | 支持设置箭头和底板样式，异常场景使用ArrowStyle对象中的默认值。&lt;br/&gt;true：显示默认的箭头和底板样式； false：不显示箭头和底板。&lt;br/&gt;默认值：false&lt;br/&gt;取值为undefined时，按默认值处理。 |
| isHoverShow | boolean \| undefined | 否 | 设置鼠标悬停时是否显示箭头。&lt;br/&gt;默认值：false&lt;br/&gt;true：有导航点时鼠标悬停在导航点和箭头范围内显示箭头，无导航点时鼠 标悬停在Swiper显示范围内显示箭头；false：常驻显示箭头。&lt;br/&gt;取值为undefined时，按默认值处理。&lt;br/&gt;**说明：**&lt;br/&gt;箭头显示时，支持点击翻页。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | boolean \| undefined | 是 | 导航点是否可交互。&lt;br/&gt;true：导航点可交互；false：导航点不可交互。&lt;br/&gt;传入参数非法时，按true处理。&lt;br/&gt;取值为 undefined时，按true处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| enabled | boolean \| undefined | 是 | 设置显示区域上方或前方插入或删除数据时是否要保持可见内容位置不变。&lt;br/&gt;默认值：false，显示区域上方或前方插入或删除数据时可见内容位置会跟随 变化。 &lt;br/&gt;true：显示区域上方或前方插入或删除数据时可见内容位置不变。如果改变数据源是在动画过程中，由于目标索引变化会导致动画停止。&lt;br/&gt;取值为undefined时，按默认值处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](arkts-arkui-length-t.md) \| undefined | 是 | 后边距。不支持设置百分比。&lt;br/&gt; &lt;br&gt;按默认值处理。 |
| ignoreBlank | boolean \| undefined | 否 | 非loop场景下尾页不显示nextMargin。&lt;br/&gt;true：尾页不显示空白的nextMargin，尾页的右边缘与Swiper视窗 右边缘对齐；false：尾页显示空白nextMargin，尾页的右边缘与Swiper视窗右边缘的距离为nextMargin。&lt;br/&gt; &lt;br&gt;默认值： false。 &lt;br&gt;按默认值处理. &lt;br/&gt; **说明：**&lt;br/&gt;尾页场景下，prevMargin和nextMargin的值相加作为左边边距显示前一个页面。 默认值： false。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | [ContentDidScrollCallback](arkts-arkui-contentdidscrollcallback-t.md) \| undefined | 是 | Swiper滑动时触发的回调。&lt;br/&gt;取值为undefined时，不使用回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | [ContentWillScrollCallback](arkts-arkui-contentwillscrollcallback-t.md) \| undefined | 是 | Swiper滑动时触发的回调。&lt;br/&gt;取值为undefined时，不使用回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [Callback](../arkts-components/arkts-arkui-callback-i.md)&lt;[ScrollState](../arkts-components/arkts-arkui-scrollstate-e.md)&gt; \| undefined | 是 | 滑动状态变化的回调。&lt;br/&gt;取值为undefined时，不使用回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [Callback](../arkts-components/arkts-arkui-callback-i.md)&lt;int&gt; \| undefined | 是 | 当前选中元素的索引。&lt;br/&gt;取值为undefined时，不使用回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [Callback](../arkts-components/arkts-arkui-callback-i.md)&lt;int&gt; \| undefined | 是 | 将要隐藏元素的索引。&lt;br/&gt;取值为undefined时，不使用回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mode | [PageFlipMode](arkts-arkui-pageflipmode-e.md) \| undefined | 是 | page flip mode on mouse wheel event. The default value is PageFlipMode.CONTINUOUS. |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](arkts-arkui-length-t.md) \| undefined | 是 | 前边距。不支持设置百分比。&lt;br/&gt; &lt;br&gt;按默认值处理。 |
| ignoreBlank | boolean \| undefined | 否 | 非loop场景下首页不显示prevMargin。&lt;br/&gt;true：首页不显示空白的prevMargin，首页的左边缘与Swiper视窗 左边缘对齐；false：首页显示空白prevMargin，首页的左边缘与Swiper视窗左边缘的距离为prevMargin。&lt;br/&gt; &lt;br&gt;默认值： false。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| controller | [SwiperController](arkts-arkui-swiper-swipercontroller-c.md) | 否 | Swiper构造函数选项 |
