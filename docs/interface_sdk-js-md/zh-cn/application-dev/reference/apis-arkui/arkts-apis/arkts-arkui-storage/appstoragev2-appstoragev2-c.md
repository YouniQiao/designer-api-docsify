...
将键值对数据存储在应用内存中。如果给定的key已经存在于\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_中，返回对应的值；否则，通过获取默认值的构造器构造默认值，存储后返回。
    **说明：**
...
| ttype | Class | 是 | 指定的类型。 |
| key | string | 是 | 指定的key。 |
| defaultCreator | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;T&gt; | 否 | 获取默认值的构造器，默认值为undefined。 |
...
将键值对数据存储在应用内存中。如果给定的ttype已经存在于\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_中，返回对应的值；否则，通过获取默认值的构造器构造默认值，存储后返回。
    **说明：**
...
获取\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_中的所有key。
    **说明：**
...
将指定的键值对数据从\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_里面删除。如果指定的键值不存在于AppStorageV2中，将删除失败。
    **说明：**
