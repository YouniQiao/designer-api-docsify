...
当应用退后台时、Repeat所在组件不可见时（visibility属性设置为Visible以外的值，或组件面积为0，不考虑遮 挡）、整机低内存时（[MemoryLevel](../../apis-ability-kit/arkts-apis/arkts-ability-abilityconstant-memorylevel-e.md)达到MEMORY_LEVEL_LOW或 MEMORY_LEVEL_CRITICAL），释放[缓存池](../../../ui/rendering-control/arkts-new-rendering-control-repeat.md#节点更新复用能力说明)内的所有 节点。
