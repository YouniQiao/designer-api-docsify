...
> [RenderNode](../../apis-default/arkts-apis/arkts-rendernode-c.md)对象也会与后端节点解引用。即从ReactiveBuilderNode中获取的FrameNode对象不对应任何一个节点。
> 
> - ReactiveBuilderNode对象会持有实体节点的引用。如果不需要使用ReactiveBuilderNode前端对象管理后端节点，可以调用
> [dispose](../../apis-default/arkts-apis/arkts-buildernode-reactivebuildernode-c.md#dispose)接口，实现前后端对象的解绑。
...
| config | [BuildOptions](../../apis-default/arkts-apis/arkts-buildernode-buildoptions-i.md) | 是 | 用于配置Builder的构建行为，BuildOptions中所有属性都是可选的，各属性默认值请参见BuildOptions的说明。 |
...
用于构造ReactiveBuilderNode类。当将ReactiveBuilderNode生成的内容嵌入到其它[RenderNode](../../apis-default/arkts-apis/arkts-rendernode-c.md)中显示时，需要显式指定 [RenderOptions](../../apis-default/arkts-apis/arkts-buildernode-renderoptions-i.md)中的[selfIdealSize](../../apis-default/arkts-apis/arkts-buildernode-renderoptions-i.md)，否则ReactiveBuilderNode内的节点默认父组件布局约束为 [0, 0]。调用此接口，若不设置selfIdealSize则认为ReactiveBuilderNode中子树的根节点大小为[0, 0]。
...
| options | [RenderOptions](../../apis-default/arkts-apis/arkts-buildernode-renderoptions-i.md) | 否 | ReactiveBuilderNode的构造可选参数，参数用于构造节点的理想大小和节点的渲染类型。 <br>默认值：undefined |
...
将输入事件分发到ReactiveBuilderNode管理的目标节点。适用于在自定义NodeContainer中将父组件接收的触摸、鼠标或轴事件转发给ReactiveBuilderNode内部组件，使内部组件能够响应相应交互的场 景。
...
> 
> 如果是开发者构造的事件，必填字段必须赋值，比如触摸事件的touches字段、轴事件的scrollStep字段，同时要保证事件的完整，比如触摸事件的TouchType中DOWN和UP字段都要
> 有，防止出现未定义行为。
> 
> [webview](../../apis-default/arkts-apis/arkts-web-webview.md)已经处理过坐标系变换，可以将事件直接下发。
> 
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [InputEventType](../../apis-default/arkts-apis/arkts-inputeventtype-t.md) | 是 | 待分发的输入事件。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [InputEventType](../../apis-default/arkts-apis/arkts-inputeventtype-t.md) | 是 | 用于事件分发的输入事件。 |
...
将原始事件派发到某个ReactiveBuilderNode创建的FrameNode上。适用于在自定义NodeContainer中将父组件接收的触摸事件转发给ReactiveBuilderNode内部组件，使内部组件能够响应触摸交互 的场景。
...
> 
> 在[webview](../../apis-default/arkts-apis/arkts-web-webview.md)中，内部已经处理过坐标系变换，可以将TouchEvent事件直接下发。
> 
...
ReactiveBuilderNode通过[reuse](../../apis-default/arkts-apis/arkts-buildernode-reactivebuildernode-c.md#reuse)和recycle完成其内外自定义组件之间的复用事件传递，具体使用场景请参见 [BuilderNode调用reuse和recycle接口实现节点复用能力](../../../ui/arkts-user-defined-arktsNode-builderNode.md#buildernode调用reuse和recycle接口实现节点复用能力)。
...
ReactiveBuilderNode通过reuse和[recycle](../../apis-default/arkts-apis/arkts-buildernode-reactivebuildernode-c.md#recycle)完成其内外自定义组件之间的复用事件传递，具体使用场景请参见 [BuilderNode调用reuse和recycle接口实现节点复用能力](../../../ui/arkts-user-defined-arktsNode-builderNode.md#buildernode调用reuse和recycle接口实现节点复用能力)。
