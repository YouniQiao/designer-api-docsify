...
JsGeolocation适用于Web组件中网页主动请求地理位置权限的场景。应用需先注册[onGeolocationShow事件](arkts-arkweb-web-comp-attribute.md#ongeolocationshow)，当网页发起地理位置权限请求时，该事件回调会将JsGeolocation对象传递给应用，应用在回调中调用invoke方法完成授权响应。使用时还需配置"ohos.permission.LOCATION"、"ohos.permission.APPROXIMATELY_LOCATION"权限。
...
JsGeolocation的构造函数。构造函数本身不直接被应用调用，通常通过[onGeolocationShow事件](arkts-arkweb-web-comp-attribute.md#ongeolocationshow)回调获取JsGeolocation实例。
...
设置网页地理位置权限状态。该方法需在[onGeolocationShow事件](arkts-arkweb-web-comp-attribute.md#ongeolocationshow)回调中调用，用于对发起地理位置权限请求的网页进行授权响应。
