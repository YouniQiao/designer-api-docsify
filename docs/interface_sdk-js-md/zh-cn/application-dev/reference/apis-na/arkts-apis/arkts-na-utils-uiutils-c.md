...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| valueCallback | [MonitorValueCallback](arkts-na-monitorvaluecallback-t.md) \| [MonitorValueCallback](arkts-na-monitorvaluecallback-t.md)[] | 是 | 返回被监听状态变量的箭头函数或箭头函数数组。 |
| monitorCallback | [MonitorCallback](arkts-na-monitorcallback-t.md) | 是 | 触发监听时调用的回调函数。 |
| options | [MonitorOptions](arkts-na-utils-monitoroptions-i.md) | 否 | 设置函数的行为，默认行行为详见[MonitorOptions]{ |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [IMonitorDecoratedVariable](arkts-na-decorator-imonitordecoratedvariable-i.md) | 指代监听关系的句柄。 |

**错误码：**

| 错误码ID | 错误信息 |
| --- | --- |
| [130000](../../apis-arkui/errorcode-stateManagement.md#130000-addmonitorclearmonitor非法目标对象) | options.owner is not ComponentV2 struct. |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| source | T | 是 | 输入一个数据对象，判断其是否可被观察。 &lt;/br&gt;具体使用规则，详见 canBeObserved接口：判断对象是否为可被观察对象。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [ObservedResult](arkts-na-utils-observedresult-i.md) | 返回对象是否可被观察的结果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| monitor | [IMonitorDecoratedVariable](arkts-na-decorator-imonitordecoratedvariable-i.md) | 是 | 指代监听关系的句柄。 |
...
| 类型 | 说明 |
| --- | --- |
| [CustomComponentContext](arkts-na-utils-customcomponentcontext-i.md) | 传入自定义组件的上下文信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [CustomComponentLifecycle](arkts-na-customcomponent-customcomponentlifecycle-i.md) | 自定义组件的生命周期实例。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| getter | [GetterCallback](arkts-na-gettercallback-t.md)&lt;T&gt; | 是 | 获取值的回调函数，每次访问值时重新执行以获取最新值。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [Binding](arkts-na-utils-binding-c.md)&lt;T&gt; | 包含一个value属性，用于获取当前绑定的值，且只能读取，不能修改。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| getter | [GetterCallback](arkts-na-gettercallback-t.md)&lt;T&gt; | 是 | 获取值的回调函数，每次访问值时重新执行。 |
| setter | [SetterCallback](arkts-na-settercallback-t.md)&lt;T&gt; | 是 | 定义如何更新值，当.value被修改时调用。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [MutableBinding](arkts-na-utils-mutablebinding-c.md)&lt;T&gt; | 包含一个value属性，支持读取和修改数据，设置值时检查类型是否匹配泛型`T`。 |
