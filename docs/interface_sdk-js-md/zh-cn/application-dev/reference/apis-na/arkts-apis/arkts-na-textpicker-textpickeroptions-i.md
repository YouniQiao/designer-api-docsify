...
设置选中项在数据选择列表中的索引值，索引从0开始。 默认值：0 **说明：** 1. 单列数据选择器使用int类型。 2. 多列数据选择器使用int[]类型。 3. 从API version 23开始，该参数支持\$\$双向绑定变量。

**类型：** int \| int[] \| [Bindable](arkts-na-common-bindable-i.md)&lt;int&gt; \| [Bindable](arkts-na-common-bindable-i.md)&lt;int[]&gt;
...
设置选中项的值，优先级低于selected。 默认值：数据选择列表中第一个元素的值。 **说明：** 1. 从API version 23开始，该参数支持\$\$双向绑定变量。 2. 只有显示文本列表时该值有效。显示图片或图文混排的列表时，该值无效。 3. 单列数据选择器使用ResourceStr类型。 4. 多列数据选择器使用ResourceStr[]类型。

**类型：** [BindableResourceStr](arkts-na-bindableresourcestr-t.md) \| [BindableResourceStrArray](arkts-na-bindableresourcestrarray-t.md)
