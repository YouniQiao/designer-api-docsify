...
| --- | --- | --- | --- |
| color | [common2D.Color](arkts-arkgraphics2d-common2d-color-i.md) | 是 | ARGB格式的颜色，每个颜色通道的取值范围为[0, 255]的整数。 |
| blendMode | [BlendMode](arkts-arkgraphics2d-drawing-blendmode-e.md) | 否 | 颜色混合模式，用于指定绘制颜色与画布已有内容的混合方式。当需要自定义颜色叠加效果时传入此参数，不传入时默认模式为SRC_OVER。 |
...
| font | [Font](arkts-arkgraphics2d-drawing-font-c.md) | 是 | 用于绘图的字体。 |
...
| samplingOptions | [SamplingOptions](arkts-arkgraphics2d-drawing-samplingoptions-c.md) | 否 | 采样选项对象，默认为不使用任何参数构造的原始采样选项对象。<br>**适用版本：** 12 |
...
| colors | Array&lt;number&gt; \| null | 是 | 颜色数组，在每个顶点指定一种颜色，每个颜色值用16进制ARGB格式的32位无符号整数表示，例如：0xAARRGGBB，可为null，大小必须为(meshWidth+1) * (meshHeight+1) + colorOffset。<br>**适用版本：** 20 |
...
| features | Array&lt;[FontFeature](arkts-arkgraphics2d-drawing-fontfeature-i.md)&gt; | 是 | 字体特征对象数组。参数为空数组时使用TTF（TrueType Font）文件中预设的字体特征。 |
