...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colorCount | ArkTS-Dyn: number  <br>ArkTS-Sta：int | 是 | 需要获取的颜色个数，向下取整。在OpenHarmony 6.1之前，取值范围为[1, 10]， 取色个数大于10视为取前10个；从OpenHarmony 6.1开始，取值范围为[1, 20]，取色个数大于20视为取前20个。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| color | ArkTS-Dyn: number  <br>ArkTS-Sta：long | 是 | 需要判断是否黑白灰色的颜色值，格式为0xAARRGGBB，取值范围为[0x0, 0xFFFFFFFF]。 |
