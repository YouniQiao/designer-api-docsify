...
| config | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | P12文件的解析配置。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| \_\_\_MD\_LINK\_USD\_0\_\_\_ | 表示解析后的P12数据。 |
...
function doTestParsePkcs12() {
  try {
    ...
    let p12: cert.Pkcs12Data = cert.parsePkcs12(p12_cert, conf);
    console.info("parsePKCS12 result: success.");
    if (p12 != undefined && p12.privateKey != undefined) {
      console.info("privateKey:" + p12?.privateKey?.toString());
    }
  } catch (err) {
    let e: BusinessError = err as BusinessError;
    console.error(`parsePKCS12 failed, ${e.code}, ${e.message}`);
  }
}
```
...
async function doTestParsePkcs12() {
  try {
    ...
    console.error('parsePKCS12 failed, errCode: ' + e.code + ', errMsg: ' + e.message);
  }
}
```
