...
true表示图片为高动态范围（HDR），false表示图片非高动态范围（SDR）。对于[ImageSource](arkts-image-imagesource-i.md)，代表源图片是否为HDR；对于[PixelMap](arkts-image-pixelmap-i.md)，代表解码后的PixelMap是否为HDR。
