...
媒体资源就绪通知，系统在资源准备就绪时回调此方法。若资源准备出错，回调的data为undefined。资源请求与回调一一对应。 T支持ArrayBuffer，[ImageSource](../../apis-image-kit/arkts-apis/arkts-image-image-imagesource-i.md#imagesource)， [MovingPhoto](arkts-medialibrary-photoaccesshelper-movingphoto-i.md#movingphoto)和boolean四种数据类型。其中，ArrayBuffer表示图片/视频资源数据， [ImageSource](../../apis-image-kit/arkts-apis/arkts-image-image-imagesource-i.md#imagesource)表示图片源， [MovingPhoto](arkts-medialibrary-photoaccesshelper-movingphoto-i.md#movingphoto)表示动态照片对象，boolean表示图片/视频资源是否成功写入应用沙箱，true表示成功，false表示失败。 map支持返回的信息： | map键名 | 值说明 | |----------|-------| | 'quality' | 图片质量。高质量为'high'，低质量为'low'。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| data | T | 是 | 已就绪的图片资源数据。泛型，支持ArrayBuffer, [ImageSource](../../apis-image-kit/arkts-apis/arkts-image-image-imagesource-i.md#imagesource), [MovingPhoto](arkts-medialibrary-photoaccesshelper-movingphoto-i.md#movingphoto)和boolean四种数据类型。 |
