...
| off | 取消订阅卡片新增事件。使用callback异步回调。 |
| off | 取消订阅卡片删除事件。使用callback异步回调。 |
| off | 取消订阅通知卡片可见的事件。使用callback异步回调。 |
| off | 取消订阅通知卡片不可见事件。使用callback异步回调。 |
| off | 取消订阅卡片router事件。使用callback异步回调，返回触发router事件的卡片信息。 |
| off | 取消订阅卡片message事件。使用callback异步回调，返回触发message事件的卡片的信息。 |
| off | 取消订阅卡片call事件。使用callback异步回调，返回触发call事件的卡片信息。 |
| on | 订阅卡片新增事件。使用callback异步回调，返回当前新增卡片的信息。 |
| on | 订阅卡片新增事件。使用callback异步回调，返回指定卡片使用方应用新增卡片的信息。 |
| on | 订阅卡片删除事件。使用callback异步回调，返回当前删除卡片的信息。 |
| on | 订阅卡片删除事件。使用callback异步回调，返回指定卡片使用方应用被删除卡片的信息。 |
| on | 订阅通知卡片可见的事件。使用callback异步回调。 |
| on | 订阅通知卡片可见的事件。使用callback异步回调。 |
| on | 订阅通知卡片不可见的事件。使用callback异步回调。 |
| on | 订阅通知卡片不可见的事件。使用callback异步回调。 |
| on | 订阅卡片router事件。使用callback异步回调，返回触发router事件的卡片信息。 |
| on | 订阅指定卡片使用方的卡片router事件。使用callback异步回调，返回触发router事件的卡片信息。 |
| on | 订阅卡片message事件。使用callback异步回调，返回触发message事件的卡片信息。 |
| on | 订阅指定卡片使用方的卡片message事件。使用callback异步回调，返回触发message事件的卡片信息。 |
| on | 订阅卡片call事件。使用callback异步回调，返回触发call事件的卡片信息。 |
| on | 订阅指定卡片使用方的卡片call事件。使用callback异步回调，返回触发call事件的卡片信息。 |
<!--DelEnd-->
